...
class GraphModule(torch.nn.Module):
    def forward(
        ...
        cache_position = torch.arange(0, 50, device=device(type="cuda", index=0))
        ...
        causal_mask = None
        arange_1 = torch.arange(51, device=device(type="cuda", index=0))
        ...
        arange_2 = torch.arange(
            50, dtype=torch.int64, device=device(type="cuda", index=0)
        )
        ...
        position_bias_2 = torch.zeros(
            (1, 6, 50, 50), device=device(type="cuda", index=0), dtype=torch.float32
        )
