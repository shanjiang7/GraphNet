...
class GraphModule(paddle.nn.Layer):
    ...
    def forward(
        ...
        # pd_op.slice: (-1xf32) <- (-1x6xf32, 1xi64, 1xi64)
        ...
        # pd_op.distribute_fpn_proposals: ([-1x4xf32, -1x4xf32, -1x4xf32, -1x4xf32], [-1xi32, -1xi32, -1xi32, -1xi32], -1x1xi32) <- (-1x4xf32, 1xi32)
        (
            ...
            paddle._C_ops.distribute_fpn_proposals(
                slice_0, data_2, 2, 5, 4, 224, False
            ),
        ...
        roi_align_0 = paddle._C_ops.roi_align(
            data_3, split_0, split_4, 14, 14, float("0.25"), 0, True
        )
        del data_3, split_0, split_4

        # pd_op.roi_align: (-1x256x14x14xf32) <- (1x256x-1x-1xf32, -1x4xf32, -1xi32)
        roi_align_1 = paddle._C_ops.roi_align(
            data_4, split_1, split_5, 14, 14, float("0.125"), 0, True
        )
        del data_4, split_1, split_5

        # pd_op.roi_align: (-1x256x14x14xf32) <- (1x256x-1x-1xf32, -1x4xf32, -1xi32)
        roi_align_2 = paddle._C_ops.roi_align(
            data_5, split_2, split_6, 14, 14, float("0.0625"), 0, True
        )
        del data_5, split_2, split_6

        # pd_op.roi_align: (-1x256x14x14xf32) <- (1x256x-1x-1xf32, -1x4xf32, -1xi32)
        roi_align_3 = paddle._C_ops.roi_align(
            data_6, split_3, split_7, 14, 14, float("0.03125"), 0, True
        )
        del data_6, split_3, split_7
        ...
        gather_0 = paddle._C_ops.gather(concat_0, distribute_fpn_proposals_2, full_1)
        del concat_0, distribute_fpn_proposals_2, full_1
        ...
        del full_2, full_3, slice_2

        # pd_op.cast: (-1xi32) <- (-1xi64)
        cast_3 = paddle._C_ops.cast(arange_0, paddle.int32)
        del arange_0

        # builtin.combine: ([-1xi32, -1xi32]) <- (-1xi32, -1xi32)
        combine_1 = [cast_3, cast_2]
        del cast_2

        # pd_op.broadcast_tensors: ([-1xi32, -1xi32]) <- ([-1xi32, -1xi32])
        broadcast_tensors_0 = paddle._C_ops.broadcast_tensors(combine_1)
        del combine_1

        # builtin.split: (-1xi32, -1xi32) <- ([-1xi32, -1xi32])
        (
            split_8,
            split_9,
        ) = broadcast_tensors_0
        ...
        # pd_op.stack: (-1x2xi32) <- ([-1xi32, -1xi32])
        stack_0 = paddle._C_ops.stack(combine_2, -1)
        del combine_2

        # pd_op.gather_nd: (-1x28x28xf32) <- (-1x2x28x28xf32, -1x2xi32)
        gather_nd_0 = paddle._C_ops.gather_nd(add_5, stack_0)
        del add_5, stack_0

        # pd_op.shape64: (3xi64) <- (-1x28x28xf32)
        shape64_1 = paddle._C_ops.shape64(gather_nd_0)
        ...
        shape64_2 = paddle._C_ops.shape64(cast_3)
        del cast_3
        ...
        combine_3 = [slice_4, full_4, full_4]
        del full_4, slice_4
        ...
        # pd_op.reshape: (-1x28x28xf32) <- (-1x28x28xf32, 3xi64)
