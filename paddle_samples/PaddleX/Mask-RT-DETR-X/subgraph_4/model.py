...
class GraphModule(paddle.nn.Layer):
    ...
    def forward(
        ...
        # pd_op.add: (1x400x384xf32) <- (1x400x384xf32, 1x400x384xf32)
        ...
        # pd_op.matmul: (1x400x384xf32) <- (1x400x384xf32, 384x384xf32)
        matmul_0 = paddle._C_ops.matmul(add_0, slice_0, False, False)

        # pd_op.add: (1x400x384xf32) <- (1x400x384xf32, 384xf32)
        add_1 = paddle._C_ops.add(matmul_0, slice_1)

        # pd_op.full_int_array: (4xi64) <- ()
        full_int_array_2 = [0, 0, 8, 48]

        # pd_op.reshape: (1x400x8x48xf32) <- (1x400x384xf32, 4xi64)
        reshape_0 = paddle._C_ops.reshape(add_1, full_int_array_2)

        # pd_op.transpose: (1x8x400x48xf32) <- (1x400x8x48xf32)
        ...
        # pd_op.matmul: (1x8x400x400xf32) <- (1x8x400x48xf32, 1x8x400x48xf32)
        ...
        # pd_op.matmul: (1x8x400x48xf32) <- (1x8x400x400xf32, 1x8x400x48xf32)
        matmul_4 = paddle._C_ops.matmul(softmax_0, transpose_2, False, False)

        # pd_op.transpose: (1x400x8x48xf32) <- (1x8x400x48xf32)
        transpose_3 = paddle._C_ops.transpose(matmul_4, [0, 2, 1, 3])
        del matmul_4

        # pd_op.full_int_array: (3xi64) <- ()
        full_int_array_5 = [0, 0, 384]

        # pd_op.reshape: (1x400x384xf32) <- (1x400x8x48xf32, 3xi64)
        ...
        # pd_op.layer_norm: (1x400x384xf32, 1x400xf32, 1x400xf32) <- (1x400x384xf32, 384xf32, 384xf32)
        ...
        # pd_op.matmul: (1x400x2048xf32) <- (1x400x384xf32, 384x2048xf32)
        matmul_6 = paddle._C_ops.matmul(layer_norm_1, parameter_5, False, False)
        del parameter_5

        # pd_op.add: (1x400x2048xf32) <- (1x400x2048xf32, 2048xf32)
        add_6 = paddle._C_ops.add(matmul_6, parameter_4)
        del parameter_4

        # pd_op.gelu: (1x400x2048xf32) <- (1x400x2048xf32)
        gelu_0 = paddle._C_ops.gelu(add_6, False)

        # pd_op.matmul: (1x400x384xf32) <- (1x400x2048xf32, 2048x384xf32)
