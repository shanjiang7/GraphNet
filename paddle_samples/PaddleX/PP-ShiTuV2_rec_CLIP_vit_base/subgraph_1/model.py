...
class GraphModule(paddle.nn.Layer):
    ...
    def forward(
        ...
        conv2d_0 = paddle._C_ops.conv2d(
            data_4, parameter_150, [16, 16], [0, 0], "EXPLICIT", [1, 1], 1, "NCHW"
        )
        del data_4, parameter_150
        ...
        slice_36 = paddle._C_ops.slice(
            add_72, [1], full_int_array_3, full_int_array_7, [1], []
        )
        ...
        layer_norm_75, layer_norm_76, layer_norm_77 = (lambda x, f: f(x))(
            paddle._C_ops.layer_norm(
                slice_36, parameter_3, parameter_2, float("1e-05"), 2
            ),
            lambda out: out if isinstance(out, (list, tuple)) else (out, None, None),
        )
        ...
        # pd_op.mean: (32x512xf32) <- (32x196x512xf32, 1xi64)
        mean_0 = paddle._C_ops.mean(matmul_72, full_int_array_3, False)
        ...
        scale_13 = paddle._C_ops.scale(square_2, full_2, float("1"), True)
        del square_2
        ...
        # pd_op.scale: (32x159xf32) <- (32x159xf32, 1xf32)
        scale_14 = paddle._C_ops.scale(scale_13, full_3, float("1e-06"), True)
        del scale_13
        ...
        # pd_op.scale: (32x159xf32) <- (32x159xf32, 1xf32)
        scale_15 = paddle._C_ops.scale(matmul_74, full_4, float("0"), True)
        ...
        # pd_op.scale: (32x159xf32) <- (32x159xf32, 1xf32)
        scale_16 = paddle._C_ops.scale(sqrt_2, full_5, float("0"), True)

        # pd_op.subtract: (32x159xf32) <- (32x159xf32, 32x159xf32)
        subtract_0 = paddle._C_ops.subtract(scale_15, scale_16)

        # pd_op.scale: (32x159xf32) <- (32x159xf32, 1xf32)
        scale_17 = paddle._C_ops.scale(matmul_74, full_3, float("-0.0397339"), True)
        ...
        greater_than_0 = paddle._C_ops.greater_than(matmul_74, full_6)
        del full_6
        ...
        # pd_op.scale: (32x159xf32) <- (32x159xf32, 1xf32)
        scale_18 = paddle._C_ops.scale(cast_0, full_2, float("1"), True)
        ...
        # pd_op.scale: (32x159xf32) <- (32x159xf32, 1xf32)
        scale_19 = paddle._C_ops.scale(squeeze_0, full_2, float("1"), True)
        ...
        # pd_op.scale: (32x159xf32) <- (32x159xf32, 1xf32)
        scale_0 = paddle._C_ops.scale(add_75, full_8, float("0"), True)
        del (
            add_0,
            add_1,
            add_10,
