import paddle


class GraphModule(paddle.nn.Layer):
    def __init__(self):
        super().__init__()

    def forward(
        self,
        parameter_0,
        parameter_1,
        parameter_2,
        parameter_3,
        parameter_4,
        parameter_5,
        parameter_6,
        parameter_7,
        parameter_8,
        parameter_9,
        parameter_10,
        parameter_11,
        parameter_12,
        parameter_13,
        parameter_14,
        parameter_15,
        parameter_16,
        parameter_17,
        parameter_18,
        parameter_19,
        parameter_20,
        parameter_21,
        parameter_22,
        parameter_23,
        parameter_24,
        parameter_25,
        parameter_26,
        parameter_27,
        parameter_28,
        parameter_29,
        parameter_30,
        parameter_31,
        parameter_32,
        parameter_33,
        parameter_34,
        parameter_35,
        parameter_36,
        parameter_37,
        parameter_38,
        parameter_39,
        parameter_40,
        parameter_41,
        parameter_42,
        parameter_43,
        parameter_44,
        parameter_45,
        parameter_46,
        parameter_47,
        parameter_48,
        parameter_49,
        parameter_50,
        parameter_51,
        parameter_52,
        parameter_53,
        parameter_54,
        parameter_55,
        parameter_56,
        parameter_57,
        parameter_58,
        parameter_59,
        parameter_60,
        parameter_61,
        parameter_62,
        parameter_63,
        data_0,
        data_1,
        data_2,
        data_3,
        data_4,
    ):
        # pd_op.transpose: (-1x1x96xf32) <- (-1x96x1xf32)
        transpose_1 = paddle._C_ops.transpose(data_4, [0, 2, 1])
        del data_4

        # pd_op.transpose: (-1x96x1xf32) <- (-1x1x96xf32)
        transpose_2 = paddle._C_ops.transpose(transpose_1, [0, 2, 1])
        del transpose_1

        # pd_op.full_int_array: (1xi64) <- ()
        full_int_array_0 = [1]

        # pd_op.mean: (-1x1x1xf32) <- (-1x96x1xf32, 1xi64)
        mean_0 = paddle._C_ops.mean(transpose_2, full_int_array_0, True)

        # pd_op.share_data_: (-1x1x1xf32) <- (-1x1x1xf32)
        share_data__0 = mean_0.detach()
        del mean_0

        # pd_op.mean: (-1x1x1xf32) <- (-1x96x1xf32, 1xi64)
        mean_1 = paddle._C_ops.mean(transpose_2, full_int_array_0, True)

        # pd_op.subtract: (-1x96x1xf32) <- (-1x96x1xf32, -1x1x1xf32)
        subtract_0 = paddle._C_ops.subtract(transpose_2, mean_1)
        del mean_1

        # pd_op.pow: (-1x96x1xf32) <- (-1x96x1xf32)
        pow_0 = paddle._C_ops.pow(subtract_0, float("2"))
        del subtract_0

        # pd_op.sum: (-1x1x1xf32) <- (-1x96x1xf32, 1xi64)
        sum_0 = paddle._C_ops.sum(pow_0, full_int_array_0, paddle.float32, True)
        del pow_0

        # pd_op.numel: (xi64) <- (-1x96x1xf32)
        numel_0 = paddle._C_ops.numel(transpose_2)

        # pd_op.cast: (xi64) <- (xi64)
        cast_0 = paddle._C_ops.cast(numel_0, paddle.int64)
        del numel_0

        # pd_op.numel: (xi64) <- (-1x1x1xf32)
        numel_1 = paddle._C_ops.numel(sum_0)

        # pd_op.cast: (xi64) <- (xi64)
        cast_1 = paddle._C_ops.cast(numel_1, paddle.int64)
        del numel_1

        # pd_op.cast: (xf32) <- (xi64)
        cast_2 = paddle._C_ops.cast(cast_0, paddle.float32)
        del cast_0

        # pd_op.cast: (xf32) <- (xi64)
        cast_3 = paddle._C_ops.cast(cast_1, paddle.float32)
        del cast_1

        # pd_op.divide: (xf32) <- (xf32, xf32)
        divide_0 = paddle._C_ops.divide(cast_2, cast_3)
        del cast_2, cast_3

        # pd_op.divide: (-1x1x1xf32) <- (-1x1x1xf32, xf32)
        divide_1 = paddle._C_ops.divide(sum_0, divide_0)
        del divide_0, sum_0

        # pd_op.full: (1xf32) <- ()
        full_0 = paddle._C_ops.full(
            [1], float("1"), paddle.float32, paddle.core.CPUPlace()
        )

        # pd_op.scale: (-1x1x1xf32) <- (-1x1x1xf32, 1xf32)
        scale_0 = paddle._C_ops.scale(divide_1, full_0, float("1e-05"), True)
        del divide_1

        # pd_op.sqrt: (-1x1x1xf32) <- (-1x1x1xf32)
        sqrt_0 = paddle._C_ops.sqrt(scale_0)
        del scale_0

        # pd_op.share_data_: (-1x1x1xf32) <- (-1x1x1xf32)
        share_data__1 = sqrt_0.detach()
        del sqrt_0

        # pd_op.subtract: (-1x96x1xf32) <- (-1x96x1xf32, -1x1x1xf32)
        subtract_1 = paddle._C_ops.subtract(transpose_2, share_data__0)
        del transpose_2

        # pd_op.divide: (-1x96x1xf32) <- (-1x96x1xf32, -1x1x1xf32)
        divide_2 = paddle._C_ops.divide(subtract_1, share_data__1)
        del subtract_1

        # pd_op.transpose: (-1x1x96xf32) <- (-1x96x1xf32)
        transpose_3 = paddle._C_ops.transpose(divide_2, [0, 2, 1])
        del divide_2

        # pd_op.full_int_array: (2xi64) <- ()
        full_int_array_1 = [3, 4]

        # pd_op.unsqueeze: (-1x1x96x1x1xf32) <- (-1x1x96xf32, 2xi64)
        unsqueeze_0 = paddle._C_ops.unsqueeze(transpose_3, full_int_array_1)
        del transpose_3

        # pd_op.full_int_array: (6xi64) <- ()
        full_int_array_2 = [0, 0, 0, 0, 0, 8]

        # pd_op.pad3d: (-1x1x104x1x1xf32) <- (-1x1x96x1x1xf32, 6xi64)
        pad3d_0 = paddle._C_ops.pad3d(
            unsqueeze_0, full_int_array_2, "replicate", float("0"), "NCDHW"
        )
        del full_int_array_2, unsqueeze_0

        # pd_op.squeeze: (-1x1x104xf32) <- (-1x1x104x1x1xf32, 2xi64)
        squeeze_0 = paddle._C_ops.squeeze(pad3d_0, full_int_array_1)
        del full_int_array_1, pad3d_0

        # pd_op.shape64: (3xi64) <- (-1x1x104xf32)
        shape64_0 = paddle._C_ops.shape64(squeeze_0)

        # pd_op.full_int_array: (1xi64) <- ()
        full_int_array_3 = [0]

        # pd_op.slice: (xi64) <- (3xi64, 1xi64, 1xi64)
        slice_0 = paddle._C_ops.slice(
            shape64_0, [0], full_int_array_3, full_int_array_0, [1], [0]
        )
        del shape64_0

        # pd_op.full: (xi64) <- ()
        full_1 = paddle._C_ops.full(
            [], float("1"), paddle.int64, paddle.core.CPUPlace()
        )

        # pd_op.full: (xi64) <- ()
        full_2 = paddle._C_ops.full(
            [], float("16"), paddle.int64, paddle.core.CPUPlace()
        )

        # pd_op.full_int_array: (3xi64) <- ()
        full_int_array_4 = [0, 0, 0]

        # builtin.combine: ([xi64, xi64, xi64]) <- (xi64, xi64, xi64)
        combine_0 = [slice_0, full_1, full_2]

        # pd_op.stack: (3xi64) <- ([xi64, xi64, xi64])
        stack_0 = paddle._C_ops.stack(combine_0, 0)
        del combine_0

        # pd_op.slice: (-1x1x16xf32) <- (-1x1x104xf32, 3xi64, 3xi64)
        slice_1 = paddle._C_ops.slice(
            squeeze_0, [0, 1, 2], full_int_array_4, stack_0, [-1, 1, 1], []
        )
        del full_int_array_4, stack_0

        # pd_op.full: (xi64) <- ()
        full_3 = paddle._C_ops.full(
            [], float("24"), paddle.int64, paddle.core.CPUPlace()
        )

        # pd_op.full_int_array: (3xi64) <- ()
        full_int_array_5 = [0, 0, 8]

        # builtin.combine: ([xi64, xi64, xi64]) <- (xi64, xi64, xi64)
        combine_1 = [slice_0, full_1, full_3]
        del full_3

        # pd_op.stack: (3xi64) <- ([xi64, xi64, xi64])
        stack_1 = paddle._C_ops.stack(combine_1, 0)
        del combine_1

        # pd_op.slice: (-1x1x16xf32) <- (-1x1x104xf32, 3xi64, 3xi64)
        slice_2 = paddle._C_ops.slice(
            squeeze_0, [0, 1, 2], full_int_array_5, stack_1, [-1, 1, 1], []
        )
        del full_int_array_5, stack_1

        # pd_op.full: (xi64) <- ()
        full_4 = paddle._C_ops.full(
            [], float("32"), paddle.int64, paddle.core.CPUPlace()
        )

        # pd_op.full_int_array: (3xi64) <- ()
        full_int_array_6 = [0, 0, 16]

        # builtin.combine: ([xi64, xi64, xi64]) <- (xi64, xi64, xi64)
        combine_2 = [slice_0, full_1, full_4]
        del full_4

        # pd_op.stack: (3xi64) <- ([xi64, xi64, xi64])
        stack_2 = paddle._C_ops.stack(combine_2, 0)
        del combine_2

        # pd_op.slice: (-1x1x16xf32) <- (-1x1x104xf32, 3xi64, 3xi64)
        slice_3 = paddle._C_ops.slice(
            squeeze_0, [0, 1, 2], full_int_array_6, stack_2, [-1, 1, 1], []
        )
        del full_int_array_6, stack_2

        # pd_op.full: (xi64) <- ()
        full_5 = paddle._C_ops.full(
            [], float("40"), paddle.int64, paddle.core.CPUPlace()
        )

        # pd_op.full_int_array: (3xi64) <- ()
        full_int_array_7 = [0, 0, 24]

        # builtin.combine: ([xi64, xi64, xi64]) <- (xi64, xi64, xi64)
        combine_3 = [slice_0, full_1, full_5]
        del full_5

        # pd_op.stack: (3xi64) <- ([xi64, xi64, xi64])
        stack_3 = paddle._C_ops.stack(combine_3, 0)
        del combine_3

        # pd_op.slice: (-1x1x16xf32) <- (-1x1x104xf32, 3xi64, 3xi64)
        slice_4 = paddle._C_ops.slice(
            squeeze_0, [0, 1, 2], full_int_array_7, stack_3, [-1, 1, 1], []
        )
        del full_int_array_7, stack_3

        # pd_op.full: (xi64) <- ()
        full_6 = paddle._C_ops.full(
            [], float("48"), paddle.int64, paddle.core.CPUPlace()
        )

        # pd_op.full_int_array: (3xi64) <- ()
        full_int_array_8 = [0, 0, 32]

        # builtin.combine: ([xi64, xi64, xi64]) <- (xi64, xi64, xi64)
        combine_4 = [slice_0, full_1, full_6]
        del full_6

        # pd_op.stack: (3xi64) <- ([xi64, xi64, xi64])
        stack_4 = paddle._C_ops.stack(combine_4, 0)
        del combine_4

        # pd_op.slice: (-1x1x16xf32) <- (-1x1x104xf32, 3xi64, 3xi64)
        slice_5 = paddle._C_ops.slice(
            squeeze_0, [0, 1, 2], full_int_array_8, stack_4, [-1, 1, 1], []
        )
        del full_int_array_8, stack_4

        # pd_op.full: (xi64) <- ()
        full_7 = paddle._C_ops.full(
            [], float("56"), paddle.int64, paddle.core.CPUPlace()
        )

        # pd_op.full_int_array: (3xi64) <- ()
        full_int_array_9 = [0, 0, 40]

        # builtin.combine: ([xi64, xi64, xi64]) <- (xi64, xi64, xi64)
        combine_5 = [slice_0, full_1, full_7]
        del full_7

        # pd_op.stack: (3xi64) <- ([xi64, xi64, xi64])
        stack_5 = paddle._C_ops.stack(combine_5, 0)
        del combine_5

        # pd_op.slice: (-1x1x16xf32) <- (-1x1x104xf32, 3xi64, 3xi64)
        slice_6 = paddle._C_ops.slice(
            squeeze_0, [0, 1, 2], full_int_array_9, stack_5, [-1, 1, 1], []
        )
        del full_int_array_9, stack_5

        # pd_op.full: (xi64) <- ()
        full_8 = paddle._C_ops.full(
            [], float("64"), paddle.int64, paddle.core.CPUPlace()
        )

        # pd_op.full_int_array: (3xi64) <- ()
        full_int_array_10 = [0, 0, 48]

        # builtin.combine: ([xi64, xi64, xi64]) <- (xi64, xi64, xi64)
        combine_6 = [slice_0, full_1, full_8]
        del full_8

        # pd_op.stack: (3xi64) <- ([xi64, xi64, xi64])
        stack_6 = paddle._C_ops.stack(combine_6, 0)
        del combine_6

        # pd_op.slice: (-1x1x16xf32) <- (-1x1x104xf32, 3xi64, 3xi64)
        slice_7 = paddle._C_ops.slice(
            squeeze_0, [0, 1, 2], full_int_array_10, stack_6, [-1, 1, 1], []
        )
        del full_int_array_10, stack_6

        # pd_op.full: (xi64) <- ()
        full_9 = paddle._C_ops.full(
            [], float("72"), paddle.int64, paddle.core.CPUPlace()
        )

        # pd_op.full_int_array: (3xi64) <- ()
        full_int_array_11 = [0, 0, 56]

        # builtin.combine: ([xi64, xi64, xi64]) <- (xi64, xi64, xi64)
        combine_7 = [slice_0, full_1, full_9]
        del full_9

        # pd_op.stack: (3xi64) <- ([xi64, xi64, xi64])
        stack_7 = paddle._C_ops.stack(combine_7, 0)
        del combine_7

        # pd_op.slice: (-1x1x16xf32) <- (-1x1x104xf32, 3xi64, 3xi64)
        slice_8 = paddle._C_ops.slice(
            squeeze_0, [0, 1, 2], full_int_array_11, stack_7, [-1, 1, 1], []
        )
        del full_int_array_11, stack_7

        # pd_op.full: (xi64) <- ()
        full_10 = paddle._C_ops.full(
            [], float("80"), paddle.int64, paddle.core.CPUPlace()
        )

        # pd_op.full_int_array: (3xi64) <- ()
        full_int_array_12 = [0, 0, 64]

        # builtin.combine: ([xi64, xi64, xi64]) <- (xi64, xi64, xi64)
        combine_8 = [slice_0, full_1, full_10]
        del full_10

        # pd_op.stack: (3xi64) <- ([xi64, xi64, xi64])
        stack_8 = paddle._C_ops.stack(combine_8, 0)
        del combine_8

        # pd_op.slice: (-1x1x16xf32) <- (-1x1x104xf32, 3xi64, 3xi64)
        slice_9 = paddle._C_ops.slice(
            squeeze_0, [0, 1, 2], full_int_array_12, stack_8, [-1, 1, 1], []
        )
        del full_int_array_12, stack_8

        # pd_op.full: (xi64) <- ()
        full_11 = paddle._C_ops.full(
            [], float("88"), paddle.int64, paddle.core.CPUPlace()
        )

        # pd_op.full_int_array: (3xi64) <- ()
        full_int_array_13 = [0, 0, 72]

        # builtin.combine: ([xi64, xi64, xi64]) <- (xi64, xi64, xi64)
        combine_9 = [slice_0, full_1, full_11]
        del full_11

        # pd_op.stack: (3xi64) <- ([xi64, xi64, xi64])
        stack_9 = paddle._C_ops.stack(combine_9, 0)
        del combine_9

        # pd_op.slice: (-1x1x16xf32) <- (-1x1x104xf32, 3xi64, 3xi64)
        slice_10 = paddle._C_ops.slice(
            squeeze_0, [0, 1, 2], full_int_array_13, stack_9, [-1, 1, 1], []
        )
        del full_int_array_13, stack_9

        # pd_op.full: (xi64) <- ()
        full_12 = paddle._C_ops.full(
            [], float("96"), paddle.int64, paddle.core.CPUPlace()
        )

        # pd_op.full_int_array: (3xi64) <- ()
        full_int_array_14 = [0, 0, 80]

        # builtin.combine: ([xi64, xi64, xi64]) <- (xi64, xi64, xi64)
        combine_10 = [slice_0, full_1, full_12]
        del full_12

        # pd_op.stack: (3xi64) <- ([xi64, xi64, xi64])
        stack_10 = paddle._C_ops.stack(combine_10, 0)
        del combine_10

        # pd_op.slice: (-1x1x16xf32) <- (-1x1x104xf32, 3xi64, 3xi64)
        slice_11 = paddle._C_ops.slice(
            squeeze_0, [0, 1, 2], full_int_array_14, stack_10, [-1, 1, 1], []
        )
        del full_int_array_14, stack_10

        # pd_op.full: (xi64) <- ()
        full_13 = paddle._C_ops.full(
            [], float("104"), paddle.int64, paddle.core.CPUPlace()
        )

        # pd_op.full_int_array: (3xi64) <- ()
        full_int_array_15 = [0, 0, 88]

        # builtin.combine: ([xi64, xi64, xi64]) <- (xi64, xi64, xi64)
        combine_11 = [slice_0, full_1, full_13]
        del full_1, full_13, slice_0

        # pd_op.stack: (3xi64) <- ([xi64, xi64, xi64])
        stack_11 = paddle._C_ops.stack(combine_11, 0)
        del combine_11

        # pd_op.slice: (-1x1x16xf32) <- (-1x1x104xf32, 3xi64, 3xi64)
        slice_12 = paddle._C_ops.slice(
            squeeze_0, [0, 1, 2], full_int_array_15, stack_11, [-1, 1, 1], []
        )
        del full_int_array_15, squeeze_0, stack_11

        # builtin.combine: ([-1x1x16xf32, -1x1x16xf32, -1x1x16xf32, -1x1x16xf32, -1x1x16xf32, -1x1x16xf32, -1x1x16xf32, -1x1x16xf32, -1x1x16xf32, -1x1x16xf32, -1x1x16xf32, -1x1x16xf32]) <- (-1x1x16xf32, -1x1x16xf32, -1x1x16xf32, -1x1x16xf32, -1x1x16xf32, -1x1x16xf32, -1x1x16xf32, -1x1x16xf32, -1x1x16xf32, -1x1x16xf32, -1x1x16xf32, -1x1x16xf32)
        combine_12 = [
            slice_1,
            slice_2,
            slice_3,
            slice_4,
            slice_5,
            slice_6,
            slice_7,
            slice_8,
            slice_9,
            slice_10,
            slice_11,
            slice_12,
        ]
        del (
            slice_1,
            slice_10,
            slice_11,
            slice_12,
            slice_2,
            slice_3,
            slice_4,
            slice_5,
            slice_6,
            slice_7,
            slice_8,
            slice_9,
        )

        # pd_op.stack: (-1x1x16x12xf32) <- ([-1x1x16xf32, -1x1x16xf32, -1x1x16xf32, -1x1x16xf32, -1x1x16xf32, -1x1x16xf32, -1x1x16xf32, -1x1x16xf32, -1x1x16xf32, -1x1x16xf32, -1x1x16xf32, -1x1x16xf32])
        stack_12 = paddle._C_ops.stack(combine_12, -1)
        del combine_12

        # pd_op.shape64: (4xi64) <- (-1x1x16x12xf32)
        shape64_1 = paddle._C_ops.shape64(stack_12)

        # pd_op.slice: (xi64) <- (4xi64, 1xi64, 1xi64)
        slice_13 = paddle._C_ops.slice(
            shape64_1, [0], full_int_array_3, full_int_array_0, [1], [0]
        )
        del shape64_1

        # pd_op.transpose: (-1x1x12x16xf32) <- (-1x1x16x12xf32)
        transpose_4 = paddle._C_ops.transpose(stack_12, [0, 1, 3, 2])
        del stack_12

        # pd_op.matmul: (-1x1x12x16xf32) <- (-1x1x12x16xf32, 16x16xf32)
        matmul_0 = paddle._C_ops.matmul(transpose_4, parameter_63, False, False)
        del parameter_63, transpose_4

        # pd_op.add: (-1x1x12x16xf32) <- (-1x1x12x16xf32, 16xf32)
        add_0 = paddle._C_ops.add(matmul_0, parameter_62)
        del matmul_0, parameter_62

        # pd_op.shape64: (4xi64) <- (-1x1x12x16xf32)
        shape64_2 = paddle._C_ops.shape64(add_0)

        # pd_op.slice: (xi64) <- (4xi64, 1xi64, 1xi64)
        slice_14 = paddle._C_ops.slice(
            shape64_2, [0], full_int_array_3, full_int_array_0, [1], [0]
        )
        del shape64_2

        # pd_op.scale: (xi64) <- (xi64, 1xf32)
        scale_1 = paddle._C_ops.scale(slice_14, full_0, float("0"), True)
        del full_0, slice_14

        # pd_op.full: (xi64) <- ()
        full_14 = paddle._C_ops.full(
            [], float("12"), paddle.int64, paddle.core.CPUPlace()
        )

        # builtin.combine: ([xi64, xi64, xi64]) <- (xi64, xi64, xi64)
        combine_13 = [scale_1, full_14, full_2]
        del full_14, scale_1

        # pd_op.stack: (3xi64) <- ([xi64, xi64, xi64])
        stack_13 = paddle._C_ops.stack(combine_13, 0)
        del combine_13

        # pd_op.reshape: (-1x12x16xf32) <- (-1x1x12x16xf32, 3xi64)
        reshape_0 = paddle._C_ops.reshape(add_0, stack_13)
        del add_0, stack_13

        # pd_op.add: (-1x12x16xf32) <- (-1x12x16xf32, 12x16xf32)
        add_1 = paddle._C_ops.add(reshape_0, data_2)
        del data_2, reshape_0

        # pd_op.full: (1xf32) <- ()
        full_15 = paddle._C_ops.full(
            [1], float("0.3"), paddle.float32, paddle.core.CPUPlace()
        )

        # pd_op.dropout: (-1x12x16xf32, -1x12x16xui8) <- (-1x12x16xf32, None, 1xf32)
        dropout_0, dropout_1 = (lambda x, f: f(x))(
            paddle._C_ops.dropout(
                add_1, None, full_15, True, "upscale_in_train", 0, False
            ),
            lambda out: out if isinstance(out, (list, tuple)) else (out, None),
        )
        del add_1

        # pd_op.shape64: (3xi64) <- (-1x12x16xf32)
        shape64_3 = paddle._C_ops.shape64(dropout_0)

        # pd_op.slice: (xi64) <- (3xi64, 1xi64, 1xi64)
        slice_15 = paddle._C_ops.slice(
            shape64_3, [0], full_int_array_3, full_int_array_0, [1], [0]
        )
        del shape64_3

        # pd_op.matmul: (-1x12x16xf32) <- (-1x12x16xf32, 16x16xf32)
        matmul_1 = paddle._C_ops.matmul(dropout_0, parameter_61, False, False)
        del parameter_61

        # pd_op.add: (-1x12x16xf32) <- (-1x12x16xf32, 16xf32)
        add_2 = paddle._C_ops.add(matmul_1, parameter_60)
        del matmul_1, parameter_60

        # pd_op.full: (xi64) <- ()
        full_16 = paddle._C_ops.full(
            [], float("-1"), paddle.int64, paddle.core.CPUPlace()
        )

        # pd_op.full: (xi64) <- ()
        full_17 = paddle._C_ops.full(
            [], float("4"), paddle.int64, paddle.core.CPUPlace()
        )

        # builtin.combine: ([xi64, xi64, xi64, xi64]) <- (xi64, xi64, xi64, xi64)
        combine_14 = [slice_15, full_16, full_17, full_17]

        # pd_op.stack: (4xi64) <- ([xi64, xi64, xi64, xi64])
        stack_14 = paddle._C_ops.stack(combine_14, 0)
        del combine_14

        # pd_op.reshape: (-1x-1x4x4xf32) <- (-1x12x16xf32, 4xi64)
        reshape_1 = paddle._C_ops.reshape(add_2, stack_14)
        del add_2, stack_14

        # pd_op.transpose: (-1x4x-1x4xf32) <- (-1x-1x4x4xf32)
        transpose_5 = paddle._C_ops.transpose(reshape_1, [0, 2, 1, 3])
        del reshape_1

        # pd_op.matmul: (-1x12x16xf32) <- (-1x12x16xf32, 16x16xf32)
        matmul_2 = paddle._C_ops.matmul(dropout_0, parameter_59, False, False)
        del parameter_59

        # pd_op.add: (-1x12x16xf32) <- (-1x12x16xf32, 16xf32)
        add_3 = paddle._C_ops.add(matmul_2, parameter_58)
        del matmul_2, parameter_58

        # builtin.combine: ([xi64, xi64, xi64, xi64]) <- (xi64, xi64, xi64, xi64)
        combine_15 = [slice_15, full_16, full_17, full_17]

        # pd_op.stack: (4xi64) <- ([xi64, xi64, xi64, xi64])
        stack_15 = paddle._C_ops.stack(combine_15, 0)
        del combine_15

        # pd_op.reshape: (-1x-1x4x4xf32) <- (-1x12x16xf32, 4xi64)
        reshape_2 = paddle._C_ops.reshape(add_3, stack_15)
        del add_3, stack_15

        # pd_op.transpose: (-1x4x4x-1xf32) <- (-1x-1x4x4xf32)
        transpose_6 = paddle._C_ops.transpose(reshape_2, [0, 2, 3, 1])
        del reshape_2

        # pd_op.matmul: (-1x12x16xf32) <- (-1x12x16xf32, 16x16xf32)
        matmul_3 = paddle._C_ops.matmul(dropout_0, parameter_57, False, False)
        del parameter_57

        # pd_op.add: (-1x12x16xf32) <- (-1x12x16xf32, 16xf32)
        add_4 = paddle._C_ops.add(matmul_3, parameter_56)
        del matmul_3, parameter_56

        # builtin.combine: ([xi64, xi64, xi64, xi64]) <- (xi64, xi64, xi64, xi64)
        combine_16 = [slice_15, full_16, full_17, full_17]

        # pd_op.stack: (4xi64) <- ([xi64, xi64, xi64, xi64])
        stack_16 = paddle._C_ops.stack(combine_16, 0)
        del combine_16

        # pd_op.reshape: (-1x-1x4x4xf32) <- (-1x12x16xf32, 4xi64)
        reshape_3 = paddle._C_ops.reshape(add_4, stack_16)
        del add_4, stack_16

        # pd_op.transpose: (-1x4x-1x4xf32) <- (-1x-1x4x4xf32)
        transpose_7 = paddle._C_ops.transpose(reshape_3, [0, 2, 1, 3])
        del reshape_3

        # pd_op.matmul: (-1x4x-1x-1xf32) <- (-1x4x-1x4xf32, -1x4x4x-1xf32)
        matmul_4 = paddle._C_ops.matmul(transpose_5, transpose_6, False, False)
        del transpose_5, transpose_6

        # pd_op.multiply: (-1x4x-1x-1xf32) <- (-1x4x-1x-1xf32, xf32)
        multiply_0 = paddle._C_ops.multiply(matmul_4, data_3)
        del data_3, matmul_4

        # pd_op.softmax: (-1x4x-1x-1xf32) <- (-1x4x-1x-1xf32)
        softmax_0 = paddle._C_ops.softmax(multiply_0, -1)

        # pd_op.matmul: (-1x4x-1x4xf32) <- (-1x4x-1x-1xf32, -1x4x-1x4xf32)
        matmul_5 = paddle._C_ops.matmul(softmax_0, transpose_7, False, False)
        del softmax_0, transpose_7

        # pd_op.transpose: (-1x-1x4x4xf32) <- (-1x4x-1x4xf32)
        transpose_8 = paddle._C_ops.transpose(matmul_5, [0, 2, 1, 3])
        del matmul_5

        # builtin.combine: ([xi64, xi64, xi64]) <- (xi64, xi64, xi64)
        combine_17 = [slice_15, full_16, full_2]
        del slice_15

        # pd_op.stack: (3xi64) <- ([xi64, xi64, xi64])
        stack_17 = paddle._C_ops.stack(combine_17, 0)
        del combine_17

        # pd_op.reshape: (-1x-1x16xf32) <- (-1x-1x4x4xf32, 3xi64)
        reshape_4 = paddle._C_ops.reshape(transpose_8, stack_17)
        del stack_17, transpose_8

        # pd_op.matmul: (-1x-1x16xf32) <- (-1x-1x16xf32, 16x16xf32)
        matmul_6 = paddle._C_ops.matmul(reshape_4, parameter_55, False, False)
        del parameter_55, reshape_4

        # pd_op.add: (-1x-1x16xf32) <- (-1x-1x16xf32, 16xf32)
        add_5 = paddle._C_ops.add(matmul_6, parameter_54)
        del matmul_6, parameter_54

        # pd_op.dropout: (-1x-1x16xf32, -1x-1x16xui8) <- (-1x-1x16xf32, None, 1xf32)
        dropout_2, dropout_3 = (lambda x, f: f(x))(
            paddle._C_ops.dropout(
                add_5, None, full_15, True, "upscale_in_train", 0, False
            ),
            lambda out: out if isinstance(out, (list, tuple)) else (out, None),
        )
        del add_5

        # pd_op.dropout: (-1x-1x16xf32, -1x-1x16xui8) <- (-1x-1x16xf32, None, 1xf32)
        dropout_4, dropout_5 = (lambda x, f: f(x))(
            paddle._C_ops.dropout(
                dropout_2, None, full_15, True, "upscale_in_train", 0, False
            ),
            lambda out: out if isinstance(out, (list, tuple)) else (out, None),
        )
        del dropout_2

        # pd_op.add: (-1x12x16xf32) <- (-1x12x16xf32, -1x-1x16xf32)
        add_6 = paddle._C_ops.add(dropout_0, dropout_4)
        del dropout_0, dropout_4

        # pd_op.transpose: (-1x16x12xf32) <- (-1x12x16xf32)
        transpose_9 = paddle._C_ops.transpose(add_6, [0, 2, 1])
        del add_6

        # pd_op.batch_norm_: (-1x16x12xf32, 16xf32, 16xf32, 16xf32, 16xf32, -1xui8) <- (-1x16x12xf32, 16xf32, 16xf32, 16xf32, 16xf32)
        (
            batch_norm__0,
            batch_norm__1,
            batch_norm__2,
            batch_norm__3,
            batch_norm__4,
            batch_norm__5,
        ) = (lambda x, f: f(x))(
            paddle._C_ops.batch_norm(
                transpose_9,
                parameter_53,
                parameter_52,
                parameter_51,
                parameter_50,
                True,
                float("0.9"),
                float("1e-05"),
                "NCHW",
                True,
                False,
            ),
            lambda out: out
            if isinstance(out, (list, tuple))
            else (out, None, None, None, None, None),
        )
        del parameter_50, parameter_51, parameter_52, parameter_53, transpose_9

        # pd_op.transpose: (-1x12x16xf32) <- (-1x16x12xf32)
        transpose_10 = paddle._C_ops.transpose(batch_norm__0, [0, 2, 1])
        del batch_norm__0

        # pd_op.matmul: (-1x12x128xf32) <- (-1x12x16xf32, 16x128xf32)
        matmul_7 = paddle._C_ops.matmul(transpose_10, parameter_49, False, False)
        del parameter_49

        # pd_op.add: (-1x12x128xf32) <- (-1x12x128xf32, 128xf32)
        add_7 = paddle._C_ops.add(matmul_7, parameter_48)
        del matmul_7, parameter_48

        # pd_op.gelu: (-1x12x128xf32) <- (-1x12x128xf32)
        gelu_0 = paddle._C_ops.gelu(add_7, False)
        del add_7

        # pd_op.dropout: (-1x12x128xf32, -1x12x128xui8) <- (-1x12x128xf32, None, 1xf32)
        dropout_6, dropout_7 = (lambda x, f: f(x))(
            paddle._C_ops.dropout(
                gelu_0, None, full_15, True, "upscale_in_train", 0, False
            ),
            lambda out: out if isinstance(out, (list, tuple)) else (out, None),
        )
        del gelu_0

        # pd_op.matmul: (-1x12x16xf32) <- (-1x12x128xf32, 128x16xf32)
        matmul_8 = paddle._C_ops.matmul(dropout_6, parameter_47, False, False)
        del dropout_6, parameter_47

        # pd_op.add: (-1x12x16xf32) <- (-1x12x16xf32, 16xf32)
        add_8 = paddle._C_ops.add(matmul_8, parameter_46)
        del matmul_8, parameter_46

        # pd_op.dropout: (-1x12x16xf32, -1x12x16xui8) <- (-1x12x16xf32, None, 1xf32)
        dropout_8, dropout_9 = (lambda x, f: f(x))(
            paddle._C_ops.dropout(
                add_8, None, full_15, True, "upscale_in_train", 0, False
            ),
            lambda out: out if isinstance(out, (list, tuple)) else (out, None),
        )
        del add_8

        # pd_op.add: (-1x12x16xf32) <- (-1x12x16xf32, -1x12x16xf32)
        add_9 = paddle._C_ops.add(transpose_10, dropout_8)
        del dropout_8, transpose_10

        # pd_op.transpose: (-1x16x12xf32) <- (-1x12x16xf32)
        transpose_11 = paddle._C_ops.transpose(add_9, [0, 2, 1])
        del add_9

        # pd_op.batch_norm_: (-1x16x12xf32, 16xf32, 16xf32, 16xf32, 16xf32, -1xui8) <- (-1x16x12xf32, 16xf32, 16xf32, 16xf32, 16xf32)
        (
            batch_norm__6,
            batch_norm__7,
            batch_norm__8,
            batch_norm__9,
            batch_norm__10,
            batch_norm__11,
        ) = (lambda x, f: f(x))(
            paddle._C_ops.batch_norm(
                transpose_11,
                parameter_45,
                parameter_44,
                parameter_43,
                parameter_42,
                True,
                float("0.9"),
                float("1e-05"),
                "NCHW",
                True,
                False,
            ),
            lambda out: out
            if isinstance(out, (list, tuple))
            else (out, None, None, None, None, None),
        )
        del parameter_42, parameter_43, parameter_44, parameter_45, transpose_11

        # pd_op.transpose: (-1x12x16xf32) <- (-1x16x12xf32)
        transpose_12 = paddle._C_ops.transpose(batch_norm__6, [0, 2, 1])
        del batch_norm__6

        # pd_op.shape64: (3xi64) <- (-1x12x16xf32)
        shape64_4 = paddle._C_ops.shape64(transpose_12)

        # pd_op.slice: (xi64) <- (3xi64, 1xi64, 1xi64)
        slice_16 = paddle._C_ops.slice(
            shape64_4, [0], full_int_array_3, full_int_array_0, [1], [0]
        )
        del shape64_4

        # pd_op.matmul: (-1x12x16xf32) <- (-1x12x16xf32, 16x16xf32)
        matmul_9 = paddle._C_ops.matmul(transpose_12, parameter_41, False, False)
        del parameter_41

        # pd_op.add: (-1x12x16xf32) <- (-1x12x16xf32, 16xf32)
        add_10 = paddle._C_ops.add(matmul_9, parameter_40)
        del matmul_9, parameter_40

        # builtin.combine: ([xi64, xi64, xi64, xi64]) <- (xi64, xi64, xi64, xi64)
        combine_18 = [slice_16, full_16, full_17, full_17]

        # pd_op.stack: (4xi64) <- ([xi64, xi64, xi64, xi64])
        stack_18 = paddle._C_ops.stack(combine_18, 0)
        del combine_18

        # pd_op.reshape: (-1x-1x4x4xf32) <- (-1x12x16xf32, 4xi64)
        reshape_5 = paddle._C_ops.reshape(add_10, stack_18)
        del add_10, stack_18

        # pd_op.transpose: (-1x4x-1x4xf32) <- (-1x-1x4x4xf32)
        transpose_13 = paddle._C_ops.transpose(reshape_5, [0, 2, 1, 3])
        del reshape_5

        # pd_op.matmul: (-1x12x16xf32) <- (-1x12x16xf32, 16x16xf32)
        matmul_10 = paddle._C_ops.matmul(transpose_12, parameter_39, False, False)
        del parameter_39

        # pd_op.add: (-1x12x16xf32) <- (-1x12x16xf32, 16xf32)
        add_11 = paddle._C_ops.add(matmul_10, parameter_38)
        del matmul_10, parameter_38

        # builtin.combine: ([xi64, xi64, xi64, xi64]) <- (xi64, xi64, xi64, xi64)
        combine_19 = [slice_16, full_16, full_17, full_17]

        # pd_op.stack: (4xi64) <- ([xi64, xi64, xi64, xi64])
        stack_19 = paddle._C_ops.stack(combine_19, 0)
        del combine_19

        # pd_op.reshape: (-1x-1x4x4xf32) <- (-1x12x16xf32, 4xi64)
        reshape_6 = paddle._C_ops.reshape(add_11, stack_19)
        del add_11, stack_19

        # pd_op.transpose: (-1x4x4x-1xf32) <- (-1x-1x4x4xf32)
        transpose_14 = paddle._C_ops.transpose(reshape_6, [0, 2, 3, 1])
        del reshape_6

        # pd_op.matmul: (-1x12x16xf32) <- (-1x12x16xf32, 16x16xf32)
        matmul_11 = paddle._C_ops.matmul(transpose_12, parameter_37, False, False)
        del parameter_37

        # pd_op.add: (-1x12x16xf32) <- (-1x12x16xf32, 16xf32)
        add_12 = paddle._C_ops.add(matmul_11, parameter_36)
        del matmul_11, parameter_36

        # builtin.combine: ([xi64, xi64, xi64, xi64]) <- (xi64, xi64, xi64, xi64)
        combine_20 = [slice_16, full_16, full_17, full_17]

        # pd_op.stack: (4xi64) <- ([xi64, xi64, xi64, xi64])
        stack_20 = paddle._C_ops.stack(combine_20, 0)
        del combine_20

        # pd_op.reshape: (-1x-1x4x4xf32) <- (-1x12x16xf32, 4xi64)
        reshape_7 = paddle._C_ops.reshape(add_12, stack_20)
        del add_12, stack_20

        # pd_op.transpose: (-1x4x-1x4xf32) <- (-1x-1x4x4xf32)
        transpose_15 = paddle._C_ops.transpose(reshape_7, [0, 2, 1, 3])
        del reshape_7

        # pd_op.matmul: (-1x4x-1x-1xf32) <- (-1x4x-1x4xf32, -1x4x4x-1xf32)
        matmul_12 = paddle._C_ops.matmul(transpose_13, transpose_14, False, False)
        del transpose_13, transpose_14

        # pd_op.multiply: (-1x4x-1x-1xf32) <- (-1x4x-1x-1xf32, xf32)
        multiply_1 = paddle._C_ops.multiply(matmul_12, data_0)
        del data_0, matmul_12

        # pd_op.add: (-1x4x-1x-1xf32) <- (-1x4x-1x-1xf32, -1x4x-1x-1xf32)
        add_13 = paddle._C_ops.add(multiply_1, multiply_0)
        del multiply_0, multiply_1

        # pd_op.softmax: (-1x4x-1x-1xf32) <- (-1x4x-1x-1xf32)
        softmax_1 = paddle._C_ops.softmax(add_13, -1)

        # pd_op.matmul: (-1x4x-1x4xf32) <- (-1x4x-1x-1xf32, -1x4x-1x4xf32)
        matmul_13 = paddle._C_ops.matmul(softmax_1, transpose_15, False, False)
        del softmax_1, transpose_15

        # pd_op.transpose: (-1x-1x4x4xf32) <- (-1x4x-1x4xf32)
        transpose_16 = paddle._C_ops.transpose(matmul_13, [0, 2, 1, 3])
        del matmul_13

        # builtin.combine: ([xi64, xi64, xi64]) <- (xi64, xi64, xi64)
        combine_21 = [slice_16, full_16, full_2]
        del slice_16

        # pd_op.stack: (3xi64) <- ([xi64, xi64, xi64])
        stack_21 = paddle._C_ops.stack(combine_21, 0)
        del combine_21

        # pd_op.reshape: (-1x-1x16xf32) <- (-1x-1x4x4xf32, 3xi64)
        reshape_8 = paddle._C_ops.reshape(transpose_16, stack_21)
        del stack_21, transpose_16

        # pd_op.matmul: (-1x-1x16xf32) <- (-1x-1x16xf32, 16x16xf32)
        matmul_14 = paddle._C_ops.matmul(reshape_8, parameter_35, False, False)
        del parameter_35, reshape_8

        # pd_op.add: (-1x-1x16xf32) <- (-1x-1x16xf32, 16xf32)
        add_14 = paddle._C_ops.add(matmul_14, parameter_34)
        del matmul_14, parameter_34

        # pd_op.dropout: (-1x-1x16xf32, -1x-1x16xui8) <- (-1x-1x16xf32, None, 1xf32)
        dropout_10, dropout_11 = (lambda x, f: f(x))(
            paddle._C_ops.dropout(
                add_14, None, full_15, True, "upscale_in_train", 0, False
            ),
            lambda out: out if isinstance(out, (list, tuple)) else (out, None),
        )
        del add_14

        # pd_op.dropout: (-1x-1x16xf32, -1x-1x16xui8) <- (-1x-1x16xf32, None, 1xf32)
        dropout_12, dropout_13 = (lambda x, f: f(x))(
            paddle._C_ops.dropout(
                dropout_10, None, full_15, True, "upscale_in_train", 0, False
            ),
            lambda out: out if isinstance(out, (list, tuple)) else (out, None),
        )
        del dropout_10

        # pd_op.add: (-1x12x16xf32) <- (-1x12x16xf32, -1x-1x16xf32)
        add_15 = paddle._C_ops.add(transpose_12, dropout_12)
        del dropout_12, transpose_12

        # pd_op.transpose: (-1x16x12xf32) <- (-1x12x16xf32)
        transpose_17 = paddle._C_ops.transpose(add_15, [0, 2, 1])
        del add_15

        # pd_op.batch_norm_: (-1x16x12xf32, 16xf32, 16xf32, 16xf32, 16xf32, -1xui8) <- (-1x16x12xf32, 16xf32, 16xf32, 16xf32, 16xf32)
        (
            batch_norm__12,
            batch_norm__13,
            batch_norm__14,
            batch_norm__15,
            batch_norm__16,
            batch_norm__17,
        ) = (lambda x, f: f(x))(
            paddle._C_ops.batch_norm(
                transpose_17,
                parameter_33,
                parameter_32,
                parameter_31,
                parameter_30,
                True,
                float("0.9"),
                float("1e-05"),
                "NCHW",
                True,
                False,
            ),
            lambda out: out
            if isinstance(out, (list, tuple))
            else (out, None, None, None, None, None),
        )
        del parameter_30, parameter_31, parameter_32, parameter_33, transpose_17

        # pd_op.transpose: (-1x12x16xf32) <- (-1x16x12xf32)
        transpose_18 = paddle._C_ops.transpose(batch_norm__12, [0, 2, 1])
        del batch_norm__12

        # pd_op.matmul: (-1x12x128xf32) <- (-1x12x16xf32, 16x128xf32)
        matmul_15 = paddle._C_ops.matmul(transpose_18, parameter_29, False, False)
        del parameter_29

        # pd_op.add: (-1x12x128xf32) <- (-1x12x128xf32, 128xf32)
        add_16 = paddle._C_ops.add(matmul_15, parameter_28)
        del matmul_15, parameter_28

        # pd_op.gelu: (-1x12x128xf32) <- (-1x12x128xf32)
        gelu_1 = paddle._C_ops.gelu(add_16, False)
        del add_16

        # pd_op.dropout: (-1x12x128xf32, -1x12x128xui8) <- (-1x12x128xf32, None, 1xf32)
        dropout_14, dropout_15 = (lambda x, f: f(x))(
            paddle._C_ops.dropout(
                gelu_1, None, full_15, True, "upscale_in_train", 0, False
            ),
            lambda out: out if isinstance(out, (list, tuple)) else (out, None),
        )
        del gelu_1

        # pd_op.matmul: (-1x12x16xf32) <- (-1x12x128xf32, 128x16xf32)
        matmul_16 = paddle._C_ops.matmul(dropout_14, parameter_27, False, False)
        del dropout_14, parameter_27

        # pd_op.add: (-1x12x16xf32) <- (-1x12x16xf32, 16xf32)
        add_17 = paddle._C_ops.add(matmul_16, parameter_26)
        del matmul_16, parameter_26

        # pd_op.dropout: (-1x12x16xf32, -1x12x16xui8) <- (-1x12x16xf32, None, 1xf32)
        dropout_16, dropout_17 = (lambda x, f: f(x))(
            paddle._C_ops.dropout(
                add_17, None, full_15, True, "upscale_in_train", 0, False
            ),
            lambda out: out if isinstance(out, (list, tuple)) else (out, None),
        )
        del add_17

        # pd_op.add: (-1x12x16xf32) <- (-1x12x16xf32, -1x12x16xf32)
        add_18 = paddle._C_ops.add(transpose_18, dropout_16)
        del dropout_16, transpose_18

        # pd_op.transpose: (-1x16x12xf32) <- (-1x12x16xf32)
        transpose_19 = paddle._C_ops.transpose(add_18, [0, 2, 1])
        del add_18

        # pd_op.batch_norm_: (-1x16x12xf32, 16xf32, 16xf32, 16xf32, 16xf32, -1xui8) <- (-1x16x12xf32, 16xf32, 16xf32, 16xf32, 16xf32)
        (
            batch_norm__18,
            batch_norm__19,
            batch_norm__20,
            batch_norm__21,
            batch_norm__22,
            batch_norm__23,
        ) = (lambda x, f: f(x))(
            paddle._C_ops.batch_norm(
                transpose_19,
                parameter_25,
                parameter_24,
                parameter_23,
                parameter_22,
                True,
                float("0.9"),
                float("1e-05"),
                "NCHW",
                True,
                False,
            ),
            lambda out: out
            if isinstance(out, (list, tuple))
            else (out, None, None, None, None, None),
        )
        del parameter_22, parameter_23, parameter_24, parameter_25, transpose_19

        # pd_op.transpose: (-1x12x16xf32) <- (-1x16x12xf32)
        transpose_20 = paddle._C_ops.transpose(batch_norm__18, [0, 2, 1])
        del batch_norm__18

        # pd_op.shape64: (3xi64) <- (-1x12x16xf32)
        shape64_5 = paddle._C_ops.shape64(transpose_20)

        # pd_op.slice: (xi64) <- (3xi64, 1xi64, 1xi64)
        slice_17 = paddle._C_ops.slice(
            shape64_5, [0], full_int_array_3, full_int_array_0, [1], [0]
        )
        del shape64_5

        # pd_op.matmul: (-1x12x16xf32) <- (-1x12x16xf32, 16x16xf32)
        matmul_17 = paddle._C_ops.matmul(transpose_20, parameter_21, False, False)
        del parameter_21

        # pd_op.add: (-1x12x16xf32) <- (-1x12x16xf32, 16xf32)
        add_19 = paddle._C_ops.add(matmul_17, parameter_20)
        del matmul_17, parameter_20

        # builtin.combine: ([xi64, xi64, xi64, xi64]) <- (xi64, xi64, xi64, xi64)
        combine_22 = [slice_17, full_16, full_17, full_17]

        # pd_op.stack: (4xi64) <- ([xi64, xi64, xi64, xi64])
        stack_22 = paddle._C_ops.stack(combine_22, 0)
        del combine_22

        # pd_op.reshape: (-1x-1x4x4xf32) <- (-1x12x16xf32, 4xi64)
        reshape_9 = paddle._C_ops.reshape(add_19, stack_22)
        del add_19, stack_22

        # pd_op.transpose: (-1x4x-1x4xf32) <- (-1x-1x4x4xf32)
        transpose_21 = paddle._C_ops.transpose(reshape_9, [0, 2, 1, 3])
        del reshape_9

        # pd_op.matmul: (-1x12x16xf32) <- (-1x12x16xf32, 16x16xf32)
        matmul_18 = paddle._C_ops.matmul(transpose_20, parameter_19, False, False)
        del parameter_19

        # pd_op.add: (-1x12x16xf32) <- (-1x12x16xf32, 16xf32)
        add_20 = paddle._C_ops.add(matmul_18, parameter_18)
        del matmul_18, parameter_18

        # builtin.combine: ([xi64, xi64, xi64, xi64]) <- (xi64, xi64, xi64, xi64)
        combine_23 = [slice_17, full_16, full_17, full_17]

        # pd_op.stack: (4xi64) <- ([xi64, xi64, xi64, xi64])
        stack_23 = paddle._C_ops.stack(combine_23, 0)
        del combine_23

        # pd_op.reshape: (-1x-1x4x4xf32) <- (-1x12x16xf32, 4xi64)
        reshape_10 = paddle._C_ops.reshape(add_20, stack_23)
        del add_20, stack_23

        # pd_op.transpose: (-1x4x4x-1xf32) <- (-1x-1x4x4xf32)
        transpose_22 = paddle._C_ops.transpose(reshape_10, [0, 2, 3, 1])
        del reshape_10

        # pd_op.matmul: (-1x12x16xf32) <- (-1x12x16xf32, 16x16xf32)
        matmul_19 = paddle._C_ops.matmul(transpose_20, parameter_17, False, False)
        del parameter_17

        # pd_op.add: (-1x12x16xf32) <- (-1x12x16xf32, 16xf32)
        add_21 = paddle._C_ops.add(matmul_19, parameter_16)
        del matmul_19, parameter_16

        # builtin.combine: ([xi64, xi64, xi64, xi64]) <- (xi64, xi64, xi64, xi64)
        combine_24 = [slice_17, full_16, full_17, full_17]
        del full_17

        # pd_op.stack: (4xi64) <- ([xi64, xi64, xi64, xi64])
        stack_24 = paddle._C_ops.stack(combine_24, 0)
        del combine_24

        # pd_op.reshape: (-1x-1x4x4xf32) <- (-1x12x16xf32, 4xi64)
        reshape_11 = paddle._C_ops.reshape(add_21, stack_24)
        del add_21, stack_24

        # pd_op.transpose: (-1x4x-1x4xf32) <- (-1x-1x4x4xf32)
        transpose_23 = paddle._C_ops.transpose(reshape_11, [0, 2, 1, 3])
        del reshape_11

        # pd_op.matmul: (-1x4x-1x-1xf32) <- (-1x4x-1x4xf32, -1x4x4x-1xf32)
        matmul_20 = paddle._C_ops.matmul(transpose_21, transpose_22, False, False)
        del transpose_21, transpose_22

        # pd_op.multiply: (-1x4x-1x-1xf32) <- (-1x4x-1x-1xf32, xf32)
        multiply_2 = paddle._C_ops.multiply(matmul_20, data_1)
        del data_1, matmul_20

        # pd_op.add: (-1x4x-1x-1xf32) <- (-1x4x-1x-1xf32, -1x4x-1x-1xf32)
        add_22 = paddle._C_ops.add(multiply_2, add_13)
        del add_13, multiply_2

        # pd_op.softmax: (-1x4x-1x-1xf32) <- (-1x4x-1x-1xf32)
        softmax_2 = paddle._C_ops.softmax(add_22, -1)
        del add_22

        # pd_op.matmul: (-1x4x-1x4xf32) <- (-1x4x-1x-1xf32, -1x4x-1x4xf32)
        matmul_21 = paddle._C_ops.matmul(softmax_2, transpose_23, False, False)
        del softmax_2, transpose_23

        # pd_op.transpose: (-1x-1x4x4xf32) <- (-1x4x-1x4xf32)
        transpose_24 = paddle._C_ops.transpose(matmul_21, [0, 2, 1, 3])
        del matmul_21

        # builtin.combine: ([xi64, xi64, xi64]) <- (xi64, xi64, xi64)
        combine_25 = [slice_17, full_16, full_2]
        del full_16, full_2, slice_17

        # pd_op.stack: (3xi64) <- ([xi64, xi64, xi64])
        stack_25 = paddle._C_ops.stack(combine_25, 0)
        del combine_25

        # pd_op.reshape: (-1x-1x16xf32) <- (-1x-1x4x4xf32, 3xi64)
        reshape_12 = paddle._C_ops.reshape(transpose_24, stack_25)
        del stack_25, transpose_24

        # pd_op.matmul: (-1x-1x16xf32) <- (-1x-1x16xf32, 16x16xf32)
        matmul_22 = paddle._C_ops.matmul(reshape_12, parameter_15, False, False)
        del parameter_15, reshape_12

        # pd_op.add: (-1x-1x16xf32) <- (-1x-1x16xf32, 16xf32)
        add_23 = paddle._C_ops.add(matmul_22, parameter_14)
        del matmul_22, parameter_14

        # pd_op.dropout: (-1x-1x16xf32, -1x-1x16xui8) <- (-1x-1x16xf32, None, 1xf32)
        dropout_18, dropout_19 = (lambda x, f: f(x))(
            paddle._C_ops.dropout(
                add_23, None, full_15, True, "upscale_in_train", 0, False
            ),
            lambda out: out if isinstance(out, (list, tuple)) else (out, None),
        )
        del add_23

        # pd_op.dropout: (-1x-1x16xf32, -1x-1x16xui8) <- (-1x-1x16xf32, None, 1xf32)
        dropout_20, dropout_21 = (lambda x, f: f(x))(
            paddle._C_ops.dropout(
                dropout_18, None, full_15, True, "upscale_in_train", 0, False
            ),
            lambda out: out if isinstance(out, (list, tuple)) else (out, None),
        )
        del dropout_18

        # pd_op.add: (-1x12x16xf32) <- (-1x12x16xf32, -1x-1x16xf32)
        add_24 = paddle._C_ops.add(transpose_20, dropout_20)
        del dropout_20, transpose_20

        # pd_op.transpose: (-1x16x12xf32) <- (-1x12x16xf32)
        transpose_25 = paddle._C_ops.transpose(add_24, [0, 2, 1])
        del add_24

        # pd_op.batch_norm_: (-1x16x12xf32, 16xf32, 16xf32, 16xf32, 16xf32, -1xui8) <- (-1x16x12xf32, 16xf32, 16xf32, 16xf32, 16xf32)
        (
            batch_norm__24,
            batch_norm__25,
            batch_norm__26,
            batch_norm__27,
            batch_norm__28,
            batch_norm__29,
        ) = (lambda x, f: f(x))(
            paddle._C_ops.batch_norm(
                transpose_25,
                parameter_13,
                parameter_12,
                parameter_11,
                parameter_10,
                True,
                float("0.9"),
                float("1e-05"),
                "NCHW",
                True,
                False,
            ),
            lambda out: out
            if isinstance(out, (list, tuple))
            else (out, None, None, None, None, None),
        )
        del parameter_10, parameter_11, parameter_12, parameter_13, transpose_25

        # pd_op.transpose: (-1x12x16xf32) <- (-1x16x12xf32)
        transpose_26 = paddle._C_ops.transpose(batch_norm__24, [0, 2, 1])
        del batch_norm__24

        # pd_op.matmul: (-1x12x128xf32) <- (-1x12x16xf32, 16x128xf32)
        matmul_23 = paddle._C_ops.matmul(transpose_26, parameter_9, False, False)
        del parameter_9

        # pd_op.add: (-1x12x128xf32) <- (-1x12x128xf32, 128xf32)
        add_25 = paddle._C_ops.add(matmul_23, parameter_8)
        del matmul_23, parameter_8

        # pd_op.gelu: (-1x12x128xf32) <- (-1x12x128xf32)
        gelu_2 = paddle._C_ops.gelu(add_25, False)
        del add_25

        # pd_op.dropout: (-1x12x128xf32, -1x12x128xui8) <- (-1x12x128xf32, None, 1xf32)
        dropout_22, dropout_23 = (lambda x, f: f(x))(
            paddle._C_ops.dropout(
                gelu_2, None, full_15, True, "upscale_in_train", 0, False
            ),
            lambda out: out if isinstance(out, (list, tuple)) else (out, None),
        )
        del gelu_2

        # pd_op.matmul: (-1x12x16xf32) <- (-1x12x128xf32, 128x16xf32)
        matmul_24 = paddle._C_ops.matmul(dropout_22, parameter_7, False, False)
        del dropout_22, parameter_7

        # pd_op.add: (-1x12x16xf32) <- (-1x12x16xf32, 16xf32)
        add_26 = paddle._C_ops.add(matmul_24, parameter_6)
        del matmul_24, parameter_6

        # pd_op.dropout: (-1x12x16xf32, -1x12x16xui8) <- (-1x12x16xf32, None, 1xf32)
        dropout_24, dropout_25 = (lambda x, f: f(x))(
            paddle._C_ops.dropout(
                add_26, None, full_15, True, "upscale_in_train", 0, False
            ),
            lambda out: out if isinstance(out, (list, tuple)) else (out, None),
        )
        del add_26, full_15

        # pd_op.add: (-1x12x16xf32) <- (-1x12x16xf32, -1x12x16xf32)
        add_27 = paddle._C_ops.add(transpose_26, dropout_24)
        del dropout_24, transpose_26

        # pd_op.transpose: (-1x16x12xf32) <- (-1x12x16xf32)
        transpose_27 = paddle._C_ops.transpose(add_27, [0, 2, 1])
        del add_27

        # pd_op.batch_norm_: (-1x16x12xf32, 16xf32, 16xf32, 16xf32, 16xf32, -1xui8) <- (-1x16x12xf32, 16xf32, 16xf32, 16xf32, 16xf32)
        (
            batch_norm__30,
            batch_norm__31,
            batch_norm__32,
            batch_norm__33,
            batch_norm__34,
            batch_norm__35,
        ) = (lambda x, f: f(x))(
            paddle._C_ops.batch_norm(
                transpose_27,
                parameter_5,
                parameter_4,
                parameter_3,
                parameter_2,
                True,
                float("0.9"),
                float("1e-05"),
                "NCHW",
                True,
                False,
            ),
            lambda out: out
            if isinstance(out, (list, tuple))
            else (out, None, None, None, None, None),
        )
        del parameter_2, parameter_3, parameter_4, parameter_5, transpose_27

        # pd_op.transpose: (-1x12x16xf32) <- (-1x16x12xf32)
        transpose_28 = paddle._C_ops.transpose(batch_norm__30, [0, 2, 1])
        del batch_norm__30

        # pd_op.shape64: (3xi64) <- (-1x12x16xf32)
        shape64_6 = paddle._C_ops.shape64(transpose_28)

        # pd_op.slice: (xi64) <- (3xi64, 1xi64, 1xi64)
        slice_18 = paddle._C_ops.slice(
            shape64_6, [0], full_int_array_3, full_int_array_0, [1], [0]
        )
        del full_int_array_0, full_int_array_3, shape64_6

        # pd_op.full_int_array: (4xi64) <- ()
        full_int_array_16 = [-1, 1, 12, 16]

        # pd_op.reshape: (-1x1x12x16xf32) <- (-1x12x16xf32, 4xi64)
        reshape_13 = paddle._C_ops.reshape(transpose_28, full_int_array_16)
        del full_int_array_16, transpose_28

        # pd_op.transpose: (-1x1x16x12xf32) <- (-1x1x12x16xf32)
        transpose_29 = paddle._C_ops.transpose(reshape_13, [0, 1, 3, 2])
        del reshape_13

        # pd_op.flatten: (-1x1x192xf32) <- (-1x1x16x12xf32)
        flatten_0 = paddle._C_ops.flatten(transpose_29, 2, 3)
        del transpose_29

        # pd_op.matmul: (-1x1x96xf32) <- (-1x1x192xf32, 192x96xf32)
        matmul_25 = paddle._C_ops.matmul(flatten_0, parameter_1, False, False)
        del flatten_0, parameter_1

        # pd_op.add: (-1x1x96xf32) <- (-1x1x96xf32, 96xf32)
        add_28 = paddle._C_ops.add(matmul_25, parameter_0)
        del matmul_25, parameter_0

        # pd_op.transpose: (-1x96x1xf32) <- (-1x1x96xf32)
        transpose_30 = paddle._C_ops.transpose(add_28, [0, 2, 1])
        del add_28

        # pd_op.multiply: (-1x96x1xf32) <- (-1x96x1xf32, -1x1x1xf32)
        multiply_3 = paddle._C_ops.multiply(transpose_30, share_data__1)
        del transpose_30

        # pd_op.add: (-1x96x1xf32) <- (-1x96x1xf32, -1x1x1xf32)
        add_29 = paddle._C_ops.add(multiply_3, share_data__0)
        del multiply_3

        # pd_op.transpose: (-1x1x96xf32) <- (-1x96x1xf32)
        transpose_31 = paddle._C_ops.transpose(add_29, [0, 2, 1])
        del add_29

        # pd_op.transpose: (-1x96x1xf32) <- (-1x1x96xf32)
        transpose_0 = paddle._C_ops.transpose(transpose_31, [0, 2, 1])
        del share_data__0, share_data__1, transpose_31

        return transpose_0
