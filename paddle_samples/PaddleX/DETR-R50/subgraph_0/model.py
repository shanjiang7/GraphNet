...
class GraphModule(paddle.nn.Layer):
    ...
    def forward(
        ...
        full_like_0 = paddle._C_ops.full_like(
            data_16, full_3, paddle.int64, paddle.framework._current_expected_place()
        )

        # pd_op.full_like: (-1xi64) <- (-1xi64, 1xf32)
        full_like_1 = paddle._C_ops.full_like(
            data_18, full_1, paddle.int64, paddle.framework._current_expected_place()
        )
        ...
        full_like_2 = paddle._C_ops.full_like(
            data_20, full_4, paddle.int64, paddle.framework._current_expected_place()
        )
        ...
        full_like_3 = paddle._C_ops.full_like(
            data_22, full_5, paddle.int64, paddle.framework._current_expected_place()
        )
        del full_5
        ...
        # builtin.combine: ([-1xi64, -1xi64, -1xi64, -1xi64]) <- (-1xi64, -1xi64, -1xi64, -1xi64)
        combine_1 = [data_16, data_18, data_20, data_22]
        ...
        scale_2 = paddle._C_ops.scale(concat_0, full_7, float("0"), True)
        del concat_0, full_7

        # pd_op.add: (-1xi64) <- (-1xi64, -1xi64)
        add_3 = paddle._C_ops.add(concat_1, scale_2)
        del concat_1, scale_2

        # pd_op.gather: (-1x1xi32) <- (-1x1xi32, -1xi64, 1xi32)
        gather_0 = paddle._C_ops.gather(data_29, data_17, full_6)
        del data_29

        # pd_op.gather: (-1x1xi32) <- (-1x1xi32, -1xi64, 1xi32)
        gather_1 = paddle._C_ops.gather(data_30, data_19, full_6)
        del data_30

        # pd_op.gather: (-1x1xi32) <- (-1x1xi32, -1xi64, 1xi32)
        gather_2 = paddle._C_ops.gather(data_31, data_21, full_6)
        del data_31

        # pd_op.gather: (-1x1xi32) <- (-1x1xi32, -1xi64, 1xi32)
        gather_3 = paddle._C_ops.gather(data_32, data_23, full_6)
        del data_32
        ...
        cross_entropy_with_softmax_0, cross_entropy_with_softmax_1 = (
            ...
            paddle._C_ops.cross_entropy_with_softmax(
                data_28, unsqueeze_0, False, True, True, -100, -1
            ),
        ...
        gather_nd_0 = paddle._C_ops.gather_nd(data_35, multiply_0)
        del data_35, multiply_0
        ...
        divide_1 = paddle._C_ops.divide(sum_0, add_4)

        # pd_op.scale: (xi64) <- (xi64, 1xf32)
        scale_3 = paddle._C_ops.scale(data_4, full_1, float("0"), True)
        del data_4

        # pd_op.add: (xi64) <- (xi64, xi64)
        add_5 = paddle._C_ops.add(scale_3, data_5)
        del data_5, scale_3

        # pd_op.add: (xi64) <- (xi64, xi64)
        add_6 = paddle._C_ops.add(add_5, data_6)
        del add_5, data_6

        # pd_op.add: (xi64) <- (xi64, xi64)
        add_7 = paddle._C_ops.add(add_6, data_7)
        del add_6, data_7

        # pd_op.equal: (xb) <- (xi64, xi64)
        equal_2 = paddle._C_ops.equal(add_7, full_2)
        del add_7
        ...
        slice_0 = paddle._C_ops.slice(
            data_34, [0], full_int_array_5, full_int_array_6, [1], [0]
        )

        # pd_op.greater_than: (xb) <- (xi64, xi64)
        greater_than_1 = paddle._C_ops.greater_than(data_8, full_2)
        del data_8
        ...
        gather_4 = paddle._C_ops.gather(slice_0, data_16, full_6)
        del data_16
        ...
        slice_1 = paddle._C_ops.slice(
            data_34, [0], full_int_array_6, full_int_array_7, [1], [0]
        )

        # pd_op.greater_than: (xb) <- (xi64, xi64)
        greater_than_2 = paddle._C_ops.greater_than(data_9, full_2)
        del data_9
        ...
        # pd_op.gather: (-1x4xf32) <- (100x4xf32, -1xi64, 1xi32)
        gather_5 = paddle._C_ops.gather(slice_1, data_18, full_6)
        del data_18
        ...
        slice_2 = paddle._C_ops.slice(
            data_34, [0], full_int_array_7, full_int_array_8, [1], [0]
        )

        # pd_op.greater_than: (xb) <- (xi64, xi64)
        greater_than_3 = paddle._C_ops.greater_than(data_10, full_2)
        del data_10
        ...
        # pd_op.gather: (-1x4xf32) <- (100x4xf32, -1xi64, 1xi32)
        gather_6 = paddle._C_ops.gather(slice_2, data_20, full_6)
        del data_20
        ...
        slice_3 = paddle._C_ops.slice(
            data_34, [0], full_int_array_8, full_int_array_9, [1], [0]
        )
        del data_34

        # pd_op.greater_than: (xb) <- (xi64, xi64)
        greater_than_4 = paddle._C_ops.greater_than(data_11, full_2)
        del data_11
        ...
        # pd_op.gather: (-1x4xf32) <- (100x4xf32, -1xi64, 1xi32)
        gather_7 = paddle._C_ops.gather(slice_3, data_22, full_6)
        del data_22
        ...
        # pd_op.greater_than: (xb) <- (xi64, xi64)
        greater_than_5 = paddle._C_ops.greater_than(data_12, full_2)
        del data_12

        # pd_op.cast: (xi64) <- (xb)
        cast_16 = paddle._C_ops.cast(greater_than_5, paddle.int64)
        del greater_than_5

        # pd_op.not_equal: (xb) <- (xi64, xi64)
        not_equal_7 = paddle._C_ops.not_equal(cast_16, full_2)
        del cast_16

        # pd_op.cast: (xi64) <- (xb)
        cast_17 = paddle._C_ops.cast(not_equal_7, paddle.int64)
        del not_equal_7

        # pd_op.equal: (xb) <- (xi64, xi64)
        equal_8 = paddle._C_ops.equal(cast_17, full_2)
        del cast_17

        # pd_op.gather: (-1x4xf32) <- (-1x4xf32, -1xi64, 1xi32)
        gather_8 = paddle._C_ops.gather(data_24, data_17, full_6)
        del data_17, data_24

        # pd_op.greater_than: (xb) <- (xi64, xi64)
        greater_than_6 = paddle._C_ops.greater_than(data_13, full_2)
        del data_13

        # pd_op.cast: (xi64) <- (xb)
        cast_18 = paddle._C_ops.cast(greater_than_6, paddle.int64)
        del greater_than_6

        # pd_op.not_equal: (xb) <- (xi64, xi64)
        not_equal_8 = paddle._C_ops.not_equal(cast_18, full_2)
        del cast_18

        # pd_op.cast: (xi64) <- (xb)
        cast_19 = paddle._C_ops.cast(not_equal_8, paddle.int64)
        del not_equal_8

        # pd_op.equal: (xb) <- (xi64, xi64)
        equal_9 = paddle._C_ops.equal(cast_19, full_2)
        del cast_19

        # pd_op.gather: (-1x4xf32) <- (-1x4xf32, -1xi64, 1xi32)
        gather_9 = paddle._C_ops.gather(data_25, data_19, full_6)
        del data_19, data_25

        # pd_op.greater_than: (xb) <- (xi64, xi64)
        greater_than_7 = paddle._C_ops.greater_than(data_14, full_2)
        del data_14

        # pd_op.cast: (xi64) <- (xb)
        cast_20 = paddle._C_ops.cast(greater_than_7, paddle.int64)
        del greater_than_7

        # pd_op.not_equal: (xb) <- (xi64, xi64)
        not_equal_9 = paddle._C_ops.not_equal(cast_20, full_2)
        del cast_20

        # pd_op.cast: (xi64) <- (xb)
        cast_21 = paddle._C_ops.cast(not_equal_9, paddle.int64)
        del not_equal_9

        # pd_op.equal: (xb) <- (xi64, xi64)
        equal_10 = paddle._C_ops.equal(cast_21, full_2)
        del cast_21

        # pd_op.gather: (-1x4xf32) <- (-1x4xf32, -1xi64, 1xi32)
        gather_10 = paddle._C_ops.gather(data_26, data_21, full_6)
        del data_21, data_26

        # pd_op.greater_than: (xb) <- (xi64, xi64)
        greater_than_8 = paddle._C_ops.greater_than(data_15, full_2)
        del data_15

        # pd_op.cast: (xi64) <- (xb)
        cast_22 = paddle._C_ops.cast(greater_than_8, paddle.int64)
        del greater_than_8

        # pd_op.not_equal: (xb) <- (xi64, xi64)
        not_equal_10 = paddle._C_ops.not_equal(cast_22, full_2)
        del cast_22

        # pd_op.cast: (xi64) <- (xb)
        cast_23 = paddle._C_ops.cast(not_equal_10, paddle.int64)
        del not_equal_10

        # pd_op.equal: (xb) <- (xi64, xi64)
        equal_11 = paddle._C_ops.equal(cast_23, full_2)
        del cast_23, full_2

        # pd_op.gather: (-1x4xf32) <- (-1x4xf32, -1xi64, 1xi32)
        gather_11 = paddle._C_ops.gather(data_27, data_23, full_6)
        del data_23, data_27
        ...
        scale_4 = paddle._C_ops.scale(sum_2, full_10, float("0"), True)
        del sum_2

        # pd_op.divide: (1xf32) <- (xf32, 1xf32)
        divide_0 = paddle._C_ops.divide(scale_4, data_33)
        ...
        scale_5 = paddle._C_ops.scale(split_1, full_12, float("0"), True)
        del split_1
        ...
        subtract_1 = paddle._C_ops.subtract(split_0, scale_5)

        # pd_op.add: (-1x2xf32) <- (-1x2xf32, -1x2xf32)
        add_8 = paddle._C_ops.add(split_0, scale_5)
        ...
        # pd_op.scale: (-1x2xf32) <- (-1x2xf32, 1xf32)
        scale_6 = paddle._C_ops.scale(split_3, full_12, float("0"), True)
        del split_3

        # pd_op.subtract: (-1x2xf32) <- (-1x2xf32, -1x2xf32)
        subtract_2 = paddle._C_ops.subtract(split_2, scale_6)

        # pd_op.add: (-1x2xf32) <- (-1x2xf32, -1x2xf32)
        add_9 = paddle._C_ops.add(split_2, scale_6)
        del scale_6, split_2

        # builtin.combine: ([-1x2xf32, -1x2xf32]) <- (-1x2xf32, -1x2xf32)
        combine_6 = [subtract_2, add_9]
        del add_9, subtract_2
        ...
        add_10 = paddle._C_ops.add(multiply_4, multiply_5)

        # pd_op.subtract: (-1x1xf32) <- (-1x1xf32, -1x1xf32)
        subtract_9 = paddle._C_ops.subtract(add_10, multiply_3)

        # pd_op.scale: (-1x1xf32) <- (-1x1xf32, 1xf32)
        scale_7 = paddle._C_ops.scale(subtract_9, full_1, float("1e-10"), True)
        del subtract_9

        # pd_op.divide: (-1x1xf32) <- (-1x1xf32, -1x1xf32)
        divide_2 = paddle._C_ops.divide(multiply_3, scale_7)
        ...
        # pd_op.scale: (-1x1xf32) <- (-1x1xf32, 1xf32)
        scale_8 = paddle._C_ops.scale(multiply_6, full_1, float("1e-10"), True)
        del multiply_6

        # pd_op.subtract: (-1x1xf32) <- (-1x1xf32, -1x1xf32)
        subtract_12 = paddle._C_ops.subtract(scale_8, scale_7)

        # pd_op.divide: (-1x1xf32) <- (-1x1xf32, -1x1xf32)
        divide_3 = paddle._C_ops.divide(subtract_12, scale_8)
        ...
        # pd_op.scale: (-1x1xf32) <- (-1x1xf32, 1xf32)
        scale_9 = paddle._C_ops.scale(subtract_13, full_15, float("1"), True)
        del subtract_13

        # pd_op.scale: (-1x1xf32) <- (-1x1xf32, 1xf32)
        scale_10 = paddle._C_ops.scale(scale_9, full_1, float("0"), True)
        del full_1, scale_9

        # pd_op.sum: (xf32) <- (-1x1xf32, 0xi64)
        sum_3 = paddle._C_ops.sum(scale_10, full_int_array_4, None, False)

        # pd_op.divide: (1xf32) <- (xf32, 1xf32)
        divide_4 = paddle._C_ops.divide(sum_3, data_33)
        del data_33
