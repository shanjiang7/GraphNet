...
class GraphModule(paddle.nn.Layer):
    ...
    def forward(self, data_0, data_1, data_2, data_3, data_4):
        # builtin.combine: ([-1x96x32xf32, -1x96x32xf32, -1x96x32xf32]) <- (-1x96x32xf32, -1x96x32xf32, -1x96x32xf32)
        combine_0 = [data_0, data_1, data_2]
        del data_0, data_1, data_2

        # pd_op.stack: (-1x96x32x3xf32) <- ([-1x96x32xf32, -1x96x32xf32, -1x96x32xf32])
        ...
        # pd_op.unsqueeze: (-1x1x3xf32) <- (-1x3xf32, 1xi64)
        unsqueeze_0 = paddle._C_ops.unsqueeze(softmax_0, full_int_array_0)

        # pd_op.unsqueeze: (-1x1x1x3xf32) <- (-1x1x3xf32, 1xi64)
        unsqueeze_1 = paddle._C_ops.unsqueeze(unsqueeze_0, full_int_array_0)

        # pd_op.full_int_array: (4xi64) <- ()
        full_int_array_1 = [1, 96, 32, 1]

        # pd_op.tile: (-1x96x32x3xf32) <- (-1x1x1x3xf32, 4xi64)
        tile_0 = paddle._C_ops.tile(unsqueeze_1, full_int_array_1)

        # pd_op.multiply: (-1x96x32x3xf32) <- (-1x96x32x3xf32, -1x96x32x3xf32)
        multiply_0 = paddle._C_ops.multiply(stack_0, tile_0)

        # pd_op.full_int_array: (1xi64) <- ()
        full_int_array_2 = [-1]

        # pd_op.sum: (-1x96x32xf32) <- (-1x96x32x3xf32, 1xi64)
