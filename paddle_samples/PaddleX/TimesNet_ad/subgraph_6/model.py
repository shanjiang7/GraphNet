...
class GraphModule(paddle.nn.Layer):
    ...
    def forward(self, data_0):
        # pd_op.fft_r2c: (-1x49x32xc64) <- (-1x96x32xf32)
        fft_r2c_0 = paddle._C_ops.fft_r2c(data_0, [1], "backward", True, True)

        # pd_op.abs: (-1x49x32xf32) <- (-1x49x32xc64)
        abs_0 = paddle._C_ops.abs(fft_r2c_0)

        # pd_op.assign: (-1x49x32xf32) <- (-1x49x32xf32)
        assign_0 = abs_0

        # pd_op.full_int_array: (1xi64) <- ()
        full_int_array_0 = [0]

        # pd_op.mean: (49x32xf32) <- (-1x49x32xf32, 1xi64)
        ...
        # pd_op.mean: (49xf32) <- (49x32xf32, 1xi64)
        mean_1 = paddle._C_ops.mean(mean_0, full_int_array_1, False)

        # pd_op.full_int_array: (1xi64) <- ()
        full_int_array_2 = [1]

        # pd_op.set_value_: (49xf32) <- (49xf32, 1xi64, 1xi64, 1xi64)
        ...
        # pd_op.topk: (3xf32, 3xi64) <- (49xf32, 1xi32)
        ...
        # pd_op.share_data_: (3xi64) <- (3xi64)
        share_data__0 = topk_1.detach()
        del topk_1

        # pd_op.cast: (3xi32) <- (3xi64)
        cast_0 = paddle._C_ops.cast(share_data__0, paddle.int32)
        del share_data__0

        # pd_op.shape64: (3xi64) <- (-1x96x32xf32)
        ...
        # pd_op.floor_divide: (3xi32) <- (xi32, 3xi32)
        floor_divide_0 = paddle._C_ops.floor_divide(cast_1, cast_0)
        del cast_1

        # pd_op.mean: (-1x49xf32) <- (-1x49x32xf32, 1xi64)
        mean_2 = paddle._C_ops.mean(abs_0, full_int_array_1, False)

        # pd_op.index_select: (-1x3xf32) <- (-1x49xf32, 3xi32)
