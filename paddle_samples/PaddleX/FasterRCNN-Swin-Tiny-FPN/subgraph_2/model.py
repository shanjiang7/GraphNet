...
class GraphModule(paddle.nn.Layer):
    ...
    def forward(
        ...
        # pd_op.conv2d: (2x96x248x184xf32) <- (2x3x992x736xf32, 96x3x4x4xf32)
        ...
        # pd_op.add: (2x96x248x184xf32) <- (2x96x248x184xf32, 1x96x1x1xf32)
        add_0 = paddle._C_ops.add(conv2d_0, reshape_0)

        # pd_op.flatten: (2x96x45632xf32) <- (2x96x248x184xf32)
        flatten_0 = paddle._C_ops.flatten(add_0, 2, 3)

        # pd_op.transpose: (2x45632x96xf32) <- (2x96x45632xf32)
        transpose_4 = paddle._C_ops.transpose(flatten_0, [0, 2, 1])
        del flatten_0

        # pd_op.layer_norm: (2x45632x96xf32, 2x45632xf32, 2x45632xf32) <- (2x45632x96xf32, 96xf32, 96xf32)
        ...
        # pd_op.transpose: (2x96x45632xf32) <- (2x45632x96xf32)
        transpose_5 = paddle._C_ops.transpose(layer_norm_0, [0, 2, 1])
        del layer_norm_0

        # pd_op.full_int_array: (4xi64) <- ()
        full_int_array_1 = [-1, 96, 248, 184]

        # pd_op.reshape: (2x96x248x184xf32) <- (2x96x45632xf32, 4xi64)
        ...
        # pd_op.set_value_: (1x252x189x1xf32) <- (1x252x189x1xf32, 2xi64, 2xi64, 2xi64)
        ...
        full_int_array_17 = [-1, 36, 7, 27, 7, 1]

        # pd_op.reshape: (1x36x7x27x7x1xf32) <- (1x252x189x1xf32, 6xi64)
        reshape_2 = paddle._C_ops.reshape(set_value__8, full_int_array_17)
        del full_int_array_17

        # pd_op.transpose: (1x36x27x7x7x1xf32) <- (1x36x7x27x7x1xf32)
        ...
        # pd_op.reshape: (972x7x7x1xf32) <- (1x36x27x7x7x1xf32, 4xi64)
        ...
        # pd_op.reshape: (972x49xf32) <- (972x7x7x1xf32, 2xi64)
        ...
        # pd_op.unsqueeze: (972x1x49xf32) <- (972x49xf32, 1xi64)
        ...
        # pd_op.unsqueeze: (972x49x1xf32) <- (972x49xf32, 1xi64)
        unsqueeze_1 = paddle._C_ops.unsqueeze(reshape_4, full_int_array_21)
        del reshape_4

        # pd_op.subtract: (972x49x49xf32) <- (972x1x49xf32, 972x49x1xf32)
        ...
        # pd_op.full_like: (972x49x49xf32) <- (972x49x49xf32, 1xf32)
        ...
        # pd_op.scale: (972x49x49xf32) <- (972x49x49xf32, 1xf32)
        ...
        # pd_op.not_equal: (972x49x49xb) <- (972x49x49xf32, xf32)
        not_equal_0 = paddle._C_ops.not_equal(subtract_0, full_3)
        del subtract_0

        # pd_op.cast: (972x49x49xf32) <- (972x49x49xb)
        cast_0 = paddle._C_ops.cast(not_equal_0, paddle.float32)
        del not_equal_0

        # pd_op.multiply: (972x49x49xf32) <- (972x49x49xf32, 972x49x49xf32)
        ...
        # pd_op.full_int_array: (4xi64) <- ()
        full_int_array_22 = [-1, 248, 184, 96]

        # pd_op.reshape: (2x248x184x96xf32) <- (2x45632x96xf32, 4xi64)
        ...
        # pd_op.pad: (2x252x189x96xf32) <- (2x248x184x96xf32, 1xf32)
        pad_0 = paddle._C_ops.pad(reshape_5, [0, 0, 0, 4, 0, 5, 0, 0], full_4)

        # pd_op.full_int_array: (6xi64) <- ()
        full_int_array_23 = [-1, 36, 7, 27, 7, 96]

        # pd_op.reshape: (2x36x7x27x7x96xf32) <- (2x252x189x96xf32, 6xi64)
        reshape_6 = paddle._C_ops.reshape(pad_0, full_int_array_23)

        # pd_op.transpose: (2x36x27x7x7x96xf32) <- (2x36x7x27x7x96xf32)
        transpose_8 = paddle._C_ops.transpose(reshape_6, [0, 1, 3, 2, 4, 5])
        del reshape_6

        # pd_op.full_int_array: (4xi64) <- ()
        full_int_array_24 = [-1, 7, 7, 96]

        # pd_op.reshape: (1944x7x7x96xf32) <- (2x36x27x7x7x96xf32, 4xi64)
        reshape_7 = paddle._C_ops.reshape(transpose_8, full_int_array_24)

        # pd_op.full_int_array: (3xi64) <- ()
        full_int_array_25 = [1944, 49, 96]

        # pd_op.reshape: (1944x49x96xf32) <- (1944x7x7x96xf32, 3xi64)
        reshape_8 = paddle._C_ops.reshape(reshape_7, full_int_array_25)

        # pd_op.matmul: (1944x49x288xf32) <- (1944x49x96xf32, 96x288xf32)
        matmul_0 = paddle._C_ops.matmul(reshape_8, parameter_158, False, False)
        del parameter_158

        # pd_op.add: (1944x49x288xf32) <- (1944x49x288xf32, 288xf32)
        ...
        # pd_op.reshape: (1944x49x3x3x32xf32) <- (1944x49x288xf32, 5xi64)
        reshape_9 = paddle._C_ops.reshape(add_1, full_int_array_26)

        # pd_op.transpose: (3x1944x3x49x32xf32) <- (1944x49x3x3x32xf32)
        transpose_9 = paddle._C_ops.transpose(reshape_9, [2, 0, 3, 1, 4])
        del reshape_9
        ...
        # pd_op.slice: (1944x3x49x32xf32) <- (3x1944x3x49x32xf32, 1xi64, 1xi64)
        slice_0 = paddle._C_ops.slice(
            transpose_9, [0], full_int_array_27, full_int_array_20, [1], [0]
        )

        # pd_op.slice: (1944x3x49x32xf32) <- (3x1944x3x49x32xf32, 1xi64, 1xi64)
        slice_1 = paddle._C_ops.slice(
            transpose_9, [0], full_int_array_20, full_int_array_21, [1], [0]
        )
        ...
        slice_2 = paddle._C_ops.slice(
            transpose_9, [0], full_int_array_21, full_int_array_28, [1], [0]
        )
        ...
        # pd_op.scale: (1944x3x49x32xf32) <- (1944x3x49x32xf32, 1xf32)
        scale_1 = paddle._C_ops.scale(slice_0, full_5, float("0"), True)
        del slice_0

        # pd_op.transpose: (1944x3x32x49xf32) <- (1944x3x49x32xf32)
        transpose_10 = paddle._C_ops.transpose(slice_1, [0, 1, 3, 2])
        del slice_1

        # pd_op.matmul: (1944x3x49x49xf32) <- (1944x3x49x32xf32, 1944x3x32x49xf32)
        matmul_1 = paddle._C_ops.matmul(scale_1, transpose_10, False, False)
        ...
        transpose_11 = paddle._C_ops.transpose(reshape_10, [2, 0, 1])
        del reshape_10

        # pd_op.unsqueeze: (1x3x49x49xf32) <- (3x49x49xf32, 1xi64)
        unsqueeze_2 = paddle._C_ops.unsqueeze(transpose_11, full_int_array_27)

        # pd_op.add: (1944x3x49x49xf32) <- (1944x3x49x49xf32, 1x3x49x49xf32)
        add_2 = paddle._C_ops.add(matmul_1, unsqueeze_2)

        # pd_op.softmax: (1944x3x49x49xf32) <- (1944x3x49x49xf32)
        softmax_0 = paddle._C_ops.softmax(add_2, -1)
        del add_2

        # pd_op.matmul: (1944x3x49x32xf32) <- (1944x3x49x49xf32, 1944x3x49x32xf32)
        matmul_2 = paddle._C_ops.matmul(softmax_0, slice_2, False, False)

        # pd_op.transpose: (1944x49x3x32xf32) <- (1944x3x49x32xf32)
        transpose_12 = paddle._C_ops.transpose(matmul_2, [0, 2, 1, 3])
        del matmul_2

        # pd_op.full_int_array: (3xi64) <- ()
        full_int_array_30 = [-1, 49, 96]

        # pd_op.reshape: (1944x49x96xf32) <- (1944x49x3x32xf32, 3xi64)
        reshape_11 = paddle._C_ops.reshape(transpose_12, full_int_array_30)

        # pd_op.matmul: (1944x49x96xf32) <- (1944x49x96xf32, 96x96xf32)
        matmul_3 = paddle._C_ops.matmul(reshape_11, parameter_156, False, False)
        del parameter_156

        # pd_op.add: (1944x49x96xf32) <- (1944x49x96xf32, 96xf32)
        add_3 = paddle._C_ops.add(matmul_3, parameter_155)
        del parameter_155

        # pd_op.full_int_array: (4xi64) <- ()
        full_int_array_31 = [1944, 7, 7, 96]

        # pd_op.reshape: (1944x7x7x96xf32) <- (1944x49x96xf32, 4xi64)
        reshape_12 = paddle._C_ops.reshape(add_3, full_int_array_31)

        # pd_op.full_int_array: (6xi64) <- ()
        full_int_array_32 = [-1, 36, 27, 7, 7, 96]

        # pd_op.reshape: (2x36x27x7x7x96xf32) <- (1944x7x7x96xf32, 6xi64)
        reshape_13 = paddle._C_ops.reshape(reshape_12, full_int_array_32)

        # pd_op.transpose: (2x36x7x27x7x96xf32) <- (2x36x27x7x7x96xf32)
        transpose_13 = paddle._C_ops.transpose(reshape_13, [0, 1, 3, 2, 4, 5])
        del reshape_13

        # pd_op.full_int_array: (4xi64) <- ()
        full_int_array_33 = [-1, 252, 189, 96]

        # pd_op.reshape: (2x252x189x96xf32) <- (2x36x7x27x7x96xf32, 4xi64)
        reshape_14 = paddle._C_ops.reshape(transpose_13, full_int_array_33)

        # pd_op.full_int_array: (2xi64) <- ()
        full_int_array_34 = [248, 184]

        # pd_op.assign: (2xi64) <- (2xi64)
        assign_140 = full_int_array_34

        # pd_op.slice: (2x248x184x96xf32) <- (2x252x189x96xf32, 2xi64, 2xi64)
        slice_3 = paddle._C_ops.slice(
            reshape_14, [1, 2], full_int_array_2, full_int_array_34, [1, 1], []
        )

        # pd_op.full_int_array: (3xi64) <- ()
        full_int_array_35 = [-1, 45632, 96]

        # pd_op.reshape: (2x45632x96xf32) <- (2x248x184x96xf32, 3xi64)
        reshape_15 = paddle._C_ops.reshape(slice_3, full_int_array_35)

        # pd_op.add: (2x45632x96xf32) <- (2x45632x96xf32, 2x45632x96xf32)
        add_4 = paddle._C_ops.add(transpose_6, reshape_15)
        ...
        # pd_op.matmul: (2x45632x384xf32) <- (2x45632x96xf32, 96x384xf32)
        matmul_4 = paddle._C_ops.matmul(layer_norm_6, parameter_152, False, False)
        del parameter_152

        # pd_op.add: (2x45632x384xf32) <- (2x45632x384xf32, 384xf32)
        add_5 = paddle._C_ops.add(matmul_4, parameter_151)
        del parameter_151

        # pd_op.gelu: (2x45632x384xf32) <- (2x45632x384xf32)
        gelu_0 = paddle._C_ops.gelu(add_5, False)

        # pd_op.matmul: (2x45632x96xf32) <- (2x45632x384xf32, 384x96xf32)
        matmul_5 = paddle._C_ops.matmul(gelu_0, parameter_150, False, False)
        del parameter_150

        # pd_op.add: (2x45632x96xf32) <- (2x45632x96xf32, 96xf32)
        ...
        # pd_op.pad: (2x252x189x96xf32) <- (2x248x184x96xf32, 1xf32)
        pad_1 = paddle._C_ops.pad(reshape_16, [0, 0, 0, 4, 0, 5, 0, 0], full_4)

        # pd_op.roll: (2x252x189x96xf32) <- (2x252x189x96xf32, 2xi64)
        ...
        # pd_op.transpose: (2x36x27x7x7x96xf32) <- (2x36x7x27x7x96xf32)
        transpose_14 = paddle._C_ops.transpose(reshape_17, [0, 1, 3, 2, 4, 5])
        del reshape_17

        # pd_op.reshape: (1944x7x7x96xf32) <- (2x36x27x7x7x96xf32, 4xi64)
        reshape_18 = paddle._C_ops.reshape(transpose_14, full_int_array_24)
        ...
        # pd_op.matmul: (1944x49x288xf32) <- (1944x49x96xf32, 96x288xf32)
        matmul_6 = paddle._C_ops.matmul(reshape_19, parameter_146, False, False)
        del parameter_146
        ...
        # pd_op.reshape: (1944x49x3x3x32xf32) <- (1944x49x288xf32, 5xi64)
        reshape_20 = paddle._C_ops.reshape(add_8, full_int_array_26)
        del full_int_array_26

        # pd_op.transpose: (3x1944x3x49x32xf32) <- (1944x49x3x3x32xf32)
        transpose_15 = paddle._C_ops.transpose(reshape_20, [2, 0, 3, 1, 4])
        del reshape_20

        # pd_op.slice: (1944x3x49x32xf32) <- (3x1944x3x49x32xf32, 1xi64, 1xi64)
        slice_4 = paddle._C_ops.slice(
            transpose_15, [0], full_int_array_27, full_int_array_20, [1], [0]
        )
        ...
        slice_6 = paddle._C_ops.slice(
            transpose_15, [0], full_int_array_21, full_int_array_28, [1], [0]
        )
        ...
        # pd_op.transpose: (1944x3x32x49xf32) <- (1944x3x49x32xf32)
        transpose_16 = paddle._C_ops.transpose(slice_5, [0, 1, 3, 2])
        del slice_5
        ...
        # pd_op.transpose: (3x49x49xf32) <- (49x49x3xf32)
        transpose_17 = paddle._C_ops.transpose(reshape_21, [2, 0, 1])
        del reshape_21

        # pd_op.unsqueeze: (1x3x49x49xf32) <- (3x49x49xf32, 1xi64)
        unsqueeze_3 = paddle._C_ops.unsqueeze(transpose_17, full_int_array_27)

        # pd_op.add: (1944x3x49x49xf32) <- (1944x3x49x49xf32, 1x3x49x49xf32)
        add_9 = paddle._C_ops.add(matmul_7, unsqueeze_3)

        # pd_op.full_int_array: (5xi64) <- ()
        full_int_array_36 = [-1, 972, 3, 49, 49]

        # pd_op.reshape: (2x972x3x49x49xf32) <- (1944x3x49x49xf32, 5xi64)
        reshape_22 = paddle._C_ops.reshape(add_9, full_int_array_36)
        del full_int_array_36

        # pd_op.unsqueeze: (972x1x49x49xf32) <- (972x49x49xf32, 1xi64)
        unsqueeze_4 = paddle._C_ops.unsqueeze(multiply_0, full_int_array_20)
        del multiply_0

        # pd_op.unsqueeze: (1x972x1x49x49xf32) <- (972x1x49x49xf32, 1xi64)
        unsqueeze_5 = paddle._C_ops.unsqueeze(unsqueeze_4, full_int_array_27)
        del unsqueeze_4

        # pd_op.add: (2x972x3x49x49xf32) <- (2x972x3x49x49xf32, 1x972x1x49x49xf32)
        add_10 = paddle._C_ops.add(reshape_22, unsqueeze_5)

        # pd_op.full_int_array: (4xi64) <- ()
        full_int_array_37 = [-1, 3, 49, 49]

        # pd_op.reshape: (1944x3x49x49xf32) <- (2x972x3x49x49xf32, 4xi64)
        reshape_23 = paddle._C_ops.reshape(add_10, full_int_array_37)
        ...
        # pd_op.reshape: (1944x49x96xf32) <- (1944x49x3x32xf32, 3xi64)
        reshape_24 = paddle._C_ops.reshape(transpose_18, full_int_array_30)
        del full_int_array_30

        # pd_op.matmul: (1944x49x96xf32) <- (1944x49x96xf32, 96x96xf32)
        matmul_9 = paddle._C_ops.matmul(reshape_24, parameter_144, False, False)
        del parameter_144
        ...
        # pd_op.reshape: (1944x7x7x96xf32) <- (1944x49x96xf32, 4xi64)
        reshape_25 = paddle._C_ops.reshape(add_11, full_int_array_31)
        ...
        # pd_op.transpose: (2x36x7x27x7x96xf32) <- (2x36x27x7x7x96xf32)
        transpose_19 = paddle._C_ops.transpose(reshape_26, [0, 1, 3, 2, 4, 5])
        del reshape_26

        # pd_op.reshape: (2x252x189x96xf32) <- (2x36x7x27x7x96xf32, 4xi64)
        reshape_27 = paddle._C_ops.reshape(transpose_19, full_int_array_33)
        ...
        add_12 = paddle._C_ops.add(cast_1, uniform_0)
        del uniform_0
        ...
        # pd_op.divide: (2x45632x96xf32) <- (2x45632x96xf32, xf32)
        divide_0 = paddle._C_ops.divide(reshape_28, cast_1)

        # pd_op.multiply: (2x45632x96xf32) <- (2x45632x96xf32, 2x1x1xf32)
        ...
        layer_norm_12, layer_norm_13, layer_norm_14 = (lambda x, f: f(x))(
            paddle._C_ops.layer_norm(
                add_13, parameter_142, parameter_141, float("1e-05"), 2
            ),
            lambda out: out if isinstance(out, (list, tuple)) else (out, None, None),
        )
        ...
        # pd_op.add: (2x45632x384xf32) <- (2x45632x384xf32, 384xf32)
        add_14 = paddle._C_ops.add(matmul_10, parameter_139)
        del parameter_139
        ...
        # pd_op.add: (2x45632x96xf32) <- (2x45632x96xf32, 96xf32)
        add_15 = paddle._C_ops.add(matmul_11, parameter_137)
        del parameter_137
        ...
        # pd_op.add: (2x1x1xf32) <- (xf32, 2x1x1xf32)
        add_16 = paddle._C_ops.add(cast_2, uniform_1)
        del uniform_1
        ...
        # pd_op.divide: (2x45632x96xf32) <- (2x45632x96xf32, xf32)
        divide_1 = paddle._C_ops.divide(add_15, cast_2)
        ...
        # pd_op.reshape: (2x248x184x96xf32) <- (2x45632x96xf32, 4xi64)
        reshape_29 = paddle._C_ops.reshape(add_17, full_int_array_22)

        # pd_op.full_int_array: (2xi64) <- ()
        full_int_array_40 = [2, 2]

        # pd_op.assign: (2xi64) <- (2xi64)
        assign_146 = full_int_array_40
        ...
        # pd_op.strided_slice: (2x124x92x96xf32) <- (2x248x184x96xf32, 2xi64, 2xi64, 2xi64)
        ...
        # builtin.combine: ([2x124x92x96xf32, 2x124x92x96xf32, 2x124x92x96xf32, 2x124x92x96xf32]) <- (2x124x92x96xf32, 2x124x92x96xf32, 2x124x92x96xf32, 2x124x92x96xf32)
        combine_0 = [strided_slice_0, strided_slice_1, strided_slice_2, strided_slice_3]

        # pd_op.concat: (2x124x92x384xf32) <- ([2x124x92x96xf32, 2x124x92x96xf32, 2x124x92x96xf32, 2x124x92x96xf32], 1xi32)
        concat_0 = paddle._C_ops.concat(combine_0, full_8)
        del combine_0

        # pd_op.full_int_array: (3xi64) <- ()
        full_int_array_43 = [-1, 11408, 384]

        # pd_op.reshape: (2x11408x384xf32) <- (2x124x92x384xf32, 3xi64)
        reshape_30 = paddle._C_ops.reshape(concat_0, full_int_array_43)
        del full_int_array_43

        # pd_op.layer_norm: (2x11408x384xf32, 2x11408xf32, 2x11408xf32) <- (2x11408x384xf32, 384xf32, 384xf32)
        layer_norm_15, layer_norm_16, layer_norm_17 = (lambda x, f: f(x))(
            paddle._C_ops.layer_norm(
                reshape_30, parameter_136, parameter_135, float("1e-05"), 2
            ),
            lambda out: out if isinstance(out, (list, tuple)) else (out, None, None),
        )
        del parameter_135, parameter_136

        # pd_op.matmul: (2x11408x192xf32) <- (2x11408x384xf32, 384x192xf32)
        ...
        layer_norm_18, layer_norm_19, layer_norm_20 = (lambda x, f: f(x))(
            paddle._C_ops.layer_norm(
                add_17, parameter_133, parameter_132, float("1e-05"), 2
            ),
            lambda out: out if isinstance(out, (list, tuple)) else (out, None, None),
        )
        ...
        # pd_op.transpose: (2x96x248x184xf32) <- (2x248x184x96xf32)
        transpose_0 = paddle._C_ops.transpose(reshape_31, [0, 3, 1, 2])
        del reshape_31

        # pd_op.full: (1x126x98x1xf32) <- ()
        ...
        # pd_op.set_value_: (1x126x98x1xf32) <- (1x126x98x1xf32, 2xi64, 2xi64, 2xi64)
        ...
        # pd_op.full_int_array: (6xi64) <- ()
        full_int_array_44 = [-1, 18, 7, 14, 7, 1]

        # pd_op.reshape: (1x18x7x14x7x1xf32) <- (1x126x98x1xf32, 6xi64)
        reshape_32 = paddle._C_ops.reshape(set_value__17, full_int_array_44)
        del full_int_array_44

        # pd_op.transpose: (1x18x14x7x7x1xf32) <- (1x18x7x14x7x1xf32)
        transpose_20 = paddle._C_ops.transpose(reshape_32, [0, 1, 3, 2, 4, 5])
        del reshape_32

        # pd_op.reshape: (252x7x7x1xf32) <- (1x18x14x7x7x1xf32, 4xi64)
        reshape_33 = paddle._C_ops.reshape(transpose_20, full_int_array_18)
        del transpose_20

        # pd_op.reshape: (252x49xf32) <- (252x7x7x1xf32, 2xi64)
        reshape_34 = paddle._C_ops.reshape(reshape_33, full_int_array_19)
        del reshape_33

        # pd_op.unsqueeze: (252x1x49xf32) <- (252x49xf32, 1xi64)
        unsqueeze_6 = paddle._C_ops.unsqueeze(reshape_34, full_int_array_20)

        # pd_op.unsqueeze: (252x49x1xf32) <- (252x49xf32, 1xi64)
        unsqueeze_7 = paddle._C_ops.unsqueeze(reshape_34, full_int_array_21)
        del reshape_34

        # pd_op.subtract: (252x49x49xf32) <- (252x1x49xf32, 252x49x1xf32)
        subtract_1 = paddle._C_ops.subtract(unsqueeze_6, unsqueeze_7)
        del unsqueeze_6, unsqueeze_7

        # pd_op.full_like: (252x49x49xf32) <- (252x49x49xf32, 1xf32)
        ...
        # pd_op.scale: (252x49x49xf32) <- (252x49x49xf32, 1xf32)
        scale_3 = paddle._C_ops.scale(full_like_1, full_2, float("0"), True)
        del full_like_1

        # pd_op.not_equal: (252x49x49xb) <- (252x49x49xf32, xf32)
        not_equal_1 = paddle._C_ops.not_equal(subtract_1, full_3)
        del subtract_1

        # pd_op.cast: (252x49x49xf32) <- (252x49x49xb)
        cast_3 = paddle._C_ops.cast(not_equal_1, paddle.float32)
        del not_equal_1

        # pd_op.multiply: (252x49x49xf32) <- (252x49x49xf32, 252x49x49xf32)
        multiply_3 = paddle._C_ops.multiply(scale_3, cast_3)
        del cast_3, scale_3

        # pd_op.layer_norm: (2x11408x192xf32, 2x11408xf32, 2x11408xf32) <- (2x11408x192xf32, 192xf32, 192xf32)
        ...
        # pd_op.full_int_array: (4xi64) <- ()
        full_int_array_45 = [-1, 124, 92, 192]

        # pd_op.reshape: (2x124x92x192xf32) <- (2x11408x192xf32, 4xi64)
        reshape_35 = paddle._C_ops.reshape(layer_norm_21, full_int_array_45)

        # pd_op.pad: (2x126x98x192xf32) <- (2x124x92x192xf32, 1xf32)
        pad_2 = paddle._C_ops.pad(reshape_35, [0, 0, 0, 2, 0, 6, 0, 0], full_4)

        # pd_op.full_int_array: (6xi64) <- ()
        full_int_array_46 = [-1, 18, 7, 14, 7, 192]

        # pd_op.reshape: (2x18x7x14x7x192xf32) <- (2x126x98x192xf32, 6xi64)
        reshape_36 = paddle._C_ops.reshape(pad_2, full_int_array_46)

        # pd_op.transpose: (2x18x14x7x7x192xf32) <- (2x18x7x14x7x192xf32)
        transpose_21 = paddle._C_ops.transpose(reshape_36, [0, 1, 3, 2, 4, 5])
        del reshape_36

        # pd_op.full_int_array: (4xi64) <- ()
        full_int_array_47 = [-1, 7, 7, 192]

        # pd_op.reshape: (504x7x7x192xf32) <- (2x18x14x7x7x192xf32, 4xi64)
        reshape_37 = paddle._C_ops.reshape(transpose_21, full_int_array_47)

        # pd_op.full_int_array: (3xi64) <- ()
        full_int_array_48 = [504, 49, 192]

        # pd_op.reshape: (504x49x192xf32) <- (504x7x7x192xf32, 3xi64)
        reshape_38 = paddle._C_ops.reshape(reshape_37, full_int_array_48)

        # pd_op.matmul: (504x49x576xf32) <- (504x49x192xf32, 192x576xf32)
        matmul_13 = paddle._C_ops.matmul(reshape_38, parameter_129, False, False)
        del parameter_129

        # pd_op.add: (504x49x576xf32) <- (504x49x576xf32, 576xf32)
        add_18 = paddle._C_ops.add(matmul_13, parameter_128)
        del parameter_128

        # pd_op.full_int_array: (5xi64) <- ()
        full_int_array_49 = [-1, 49, 3, 6, 32]

        # pd_op.reshape: (504x49x3x6x32xf32) <- (504x49x576xf32, 5xi64)
        reshape_39 = paddle._C_ops.reshape(add_18, full_int_array_49)

        # pd_op.transpose: (3x504x6x49x32xf32) <- (504x49x3x6x32xf32)
        transpose_22 = paddle._C_ops.transpose(reshape_39, [2, 0, 3, 1, 4])
        del reshape_39

        # pd_op.slice: (504x6x49x32xf32) <- (3x504x6x49x32xf32, 1xi64, 1xi64)
        slice_8 = paddle._C_ops.slice(
            transpose_22, [0], full_int_array_27, full_int_array_20, [1], [0]
        )

        # pd_op.slice: (504x6x49x32xf32) <- (3x504x6x49x32xf32, 1xi64, 1xi64)
        slice_9 = paddle._C_ops.slice(
            transpose_22, [0], full_int_array_20, full_int_array_21, [1], [0]
        )

        # pd_op.slice: (504x6x49x32xf32) <- (3x504x6x49x32xf32, 1xi64, 1xi64)
        slice_10 = paddle._C_ops.slice(
            transpose_22, [0], full_int_array_21, full_int_array_28, [1], [0]
        )

        # pd_op.scale: (504x6x49x32xf32) <- (504x6x49x32xf32, 1xf32)
        scale_4 = paddle._C_ops.scale(slice_8, full_5, float("0"), True)
        del slice_8

        # pd_op.transpose: (504x6x32x49xf32) <- (504x6x49x32xf32)
        transpose_23 = paddle._C_ops.transpose(slice_9, [0, 1, 3, 2])
        del slice_9

        # pd_op.matmul: (504x6x49x49xf32) <- (504x6x49x32xf32, 504x6x32x49xf32)
        matmul_14 = paddle._C_ops.matmul(scale_4, transpose_23, False, False)
        ...
        transpose_24 = paddle._C_ops.transpose(reshape_40, [2, 0, 1])
        del reshape_40

        # pd_op.unsqueeze: (1x6x49x49xf32) <- (6x49x49xf32, 1xi64)
        unsqueeze_8 = paddle._C_ops.unsqueeze(transpose_24, full_int_array_27)

        # pd_op.add: (504x6x49x49xf32) <- (504x6x49x49xf32, 1x6x49x49xf32)
        add_19 = paddle._C_ops.add(matmul_14, unsqueeze_8)

        # pd_op.softmax: (504x6x49x49xf32) <- (504x6x49x49xf32)
        softmax_2 = paddle._C_ops.softmax(add_19, -1)
        del add_19

        # pd_op.matmul: (504x6x49x32xf32) <- (504x6x49x49xf32, 504x6x49x32xf32)
        matmul_15 = paddle._C_ops.matmul(softmax_2, slice_10, False, False)

        # pd_op.transpose: (504x49x6x32xf32) <- (504x6x49x32xf32)
        transpose_25 = paddle._C_ops.transpose(matmul_15, [0, 2, 1, 3])
        del matmul_15

        # pd_op.full_int_array: (3xi64) <- ()
        full_int_array_50 = [-1, 49, 192]

        # pd_op.reshape: (504x49x192xf32) <- (504x49x6x32xf32, 3xi64)
        reshape_41 = paddle._C_ops.reshape(transpose_25, full_int_array_50)

        # pd_op.matmul: (504x49x192xf32) <- (504x49x192xf32, 192x192xf32)
        matmul_16 = paddle._C_ops.matmul(reshape_41, parameter_127, False, False)
        del parameter_127

        # pd_op.add: (504x49x192xf32) <- (504x49x192xf32, 192xf32)
        add_20 = paddle._C_ops.add(matmul_16, parameter_126)
        del parameter_126

        # pd_op.full_int_array: (4xi64) <- ()
        full_int_array_51 = [504, 7, 7, 192]

        # pd_op.reshape: (504x7x7x192xf32) <- (504x49x192xf32, 4xi64)
        reshape_42 = paddle._C_ops.reshape(add_20, full_int_array_51)

        # pd_op.full_int_array: (6xi64) <- ()
        full_int_array_52 = [-1, 18, 14, 7, 7, 192]

        # pd_op.reshape: (2x18x14x7x7x192xf32) <- (504x7x7x192xf32, 6xi64)
        reshape_43 = paddle._C_ops.reshape(reshape_42, full_int_array_52)

        # pd_op.transpose: (2x18x7x14x7x192xf32) <- (2x18x14x7x7x192xf32)
        transpose_26 = paddle._C_ops.transpose(reshape_43, [0, 1, 3, 2, 4, 5])
        del reshape_43

        # pd_op.full_int_array: (4xi64) <- ()
        full_int_array_53 = [-1, 126, 98, 192]

        # pd_op.reshape: (2x126x98x192xf32) <- (2x18x7x14x7x192xf32, 4xi64)
        reshape_44 = paddle._C_ops.reshape(transpose_26, full_int_array_53)

        # pd_op.full_int_array: (2xi64) <- ()
        full_int_array_54 = [124, 92]

        # pd_op.assign: (2xi64) <- (2xi64)
        assign_163 = full_int_array_54

        # pd_op.slice: (2x124x92x192xf32) <- (2x126x98x192xf32, 2xi64, 2xi64)
        slice_11 = paddle._C_ops.slice(
            reshape_44, [1, 2], full_int_array_2, full_int_array_54, [1, 1], []
        )

        # pd_op.full_int_array: (3xi64) <- ()
        full_int_array_55 = [-1, 11408, 192]

        # pd_op.reshape: (2x11408x192xf32) <- (2x124x92x192xf32, 3xi64)
        ...
        # pd_op.add: (2x1x1xf32) <- (xf32, 2x1x1xf32)
        add_21 = paddle._C_ops.add(cast_4, uniform_2)
        del uniform_2
        ...
        # pd_op.divide: (2x11408x192xf32) <- (2x11408x192xf32, xf32)
        divide_2 = paddle._C_ops.divide(reshape_45, cast_4)

        # pd_op.multiply: (2x11408x192xf32) <- (2x11408x192xf32, 2x1x1xf32)
        multiply_4 = paddle._C_ops.multiply(divide_2, floor_2)

        # pd_op.add: (2x11408x192xf32) <- (2x11408x192xf32, 2x11408x192xf32)
        add_22 = paddle._C_ops.add(matmul_12, multiply_4)

        # pd_op.layer_norm: (2x11408x192xf32, 2x11408xf32, 2x11408xf32) <- (2x11408x192xf32, 192xf32, 192xf32)
        layer_norm_24, layer_norm_25, layer_norm_26 = (lambda x, f: f(x))(
            paddle._C_ops.layer_norm(
                add_22, parameter_125, parameter_124, float("1e-05"), 2
            ),
            lambda out: out if isinstance(out, (list, tuple)) else (out, None, None),
        )
        del parameter_124, parameter_125

        # pd_op.matmul: (2x11408x768xf32) <- (2x11408x192xf32, 192x768xf32)
        matmul_17 = paddle._C_ops.matmul(layer_norm_24, parameter_123, False, False)
        del parameter_123

        # pd_op.add: (2x11408x768xf32) <- (2x11408x768xf32, 768xf32)
        add_23 = paddle._C_ops.add(matmul_17, parameter_122)
        del parameter_122

        # pd_op.gelu: (2x11408x768xf32) <- (2x11408x768xf32)
        gelu_2 = paddle._C_ops.gelu(add_23, False)

        # pd_op.matmul: (2x11408x192xf32) <- (2x11408x768xf32, 768x192xf32)
        matmul_18 = paddle._C_ops.matmul(gelu_2, parameter_121, False, False)
        del parameter_121

        # pd_op.add: (2x11408x192xf32) <- (2x11408x192xf32, 192xf32)
        add_24 = paddle._C_ops.add(matmul_18, parameter_120)
        del parameter_120
        ...
        # pd_op.add: (2x1x1xf32) <- (xf32, 2x1x1xf32)
        add_25 = paddle._C_ops.add(cast_5, uniform_3)
        del uniform_3
        ...
        # pd_op.divide: (2x11408x192xf32) <- (2x11408x192xf32, xf32)
        divide_3 = paddle._C_ops.divide(add_24, cast_5)
        ...
        layer_norm_27, layer_norm_28, layer_norm_29 = (lambda x, f: f(x))(
            paddle._C_ops.layer_norm(
                add_26, parameter_119, parameter_118, float("1e-05"), 2
            ),
            lambda out: out if isinstance(out, (list, tuple)) else (out, None, None),
        )
        ...
        # pd_op.pad: (2x126x98x192xf32) <- (2x124x92x192xf32, 1xf32)
        pad_3 = paddle._C_ops.pad(reshape_46, [0, 0, 0, 2, 0, 6, 0, 0], full_4)

        # pd_op.roll: (2x126x98x192xf32) <- (2x126x98x192xf32, 2xi64)
        ...
        # pd_op.transpose: (2x18x14x7x7x192xf32) <- (2x18x7x14x7x192xf32)
        transpose_27 = paddle._C_ops.transpose(reshape_47, [0, 1, 3, 2, 4, 5])
        del reshape_47

        # pd_op.reshape: (504x7x7x192xf32) <- (2x18x14x7x7x192xf32, 4xi64)
        reshape_48 = paddle._C_ops.reshape(transpose_27, full_int_array_47)
        ...
        # pd_op.matmul: (504x49x576xf32) <- (504x49x192xf32, 192x576xf32)
        matmul_19 = paddle._C_ops.matmul(reshape_49, parameter_117, False, False)
        del parameter_117

        # pd_op.add: (504x49x576xf32) <- (504x49x576xf32, 576xf32)
        add_27 = paddle._C_ops.add(matmul_19, parameter_116)
        del parameter_116

        # pd_op.reshape: (504x49x3x6x32xf32) <- (504x49x576xf32, 5xi64)
        reshape_50 = paddle._C_ops.reshape(add_27, full_int_array_49)
        del full_int_array_49

        # pd_op.transpose: (3x504x6x49x32xf32) <- (504x49x3x6x32xf32)
        transpose_28 = paddle._C_ops.transpose(reshape_50, [2, 0, 3, 1, 4])
        del reshape_50

        # pd_op.slice: (504x6x49x32xf32) <- (3x504x6x49x32xf32, 1xi64, 1xi64)
        slice_12 = paddle._C_ops.slice(
            transpose_28, [0], full_int_array_27, full_int_array_20, [1], [0]
        )
        ...
        slice_14 = paddle._C_ops.slice(
            transpose_28, [0], full_int_array_21, full_int_array_28, [1], [0]
        )
        ...
        # pd_op.matmul: (504x6x49x49xf32) <- (504x6x49x32xf32, 504x6x32x49xf32)
        matmul_20 = paddle._C_ops.matmul(scale_5, transpose_29, False, False)
        ...
        # pd_op.transpose: (6x49x49xf32) <- (49x49x6xf32)
        transpose_30 = paddle._C_ops.transpose(reshape_51, [2, 0, 1])
        del reshape_51

        # pd_op.unsqueeze: (1x6x49x49xf32) <- (6x49x49xf32, 1xi64)
        unsqueeze_9 = paddle._C_ops.unsqueeze(transpose_30, full_int_array_27)

        # pd_op.add: (504x6x49x49xf32) <- (504x6x49x49xf32, 1x6x49x49xf32)
        add_28 = paddle._C_ops.add(matmul_20, unsqueeze_9)

        # pd_op.full_int_array: (5xi64) <- ()
        full_int_array_56 = [-1, 252, 6, 49, 49]

        # pd_op.reshape: (2x252x6x49x49xf32) <- (504x6x49x49xf32, 5xi64)
        reshape_52 = paddle._C_ops.reshape(add_28, full_int_array_56)
        del full_int_array_56

        # pd_op.unsqueeze: (252x1x49x49xf32) <- (252x49x49xf32, 1xi64)
        unsqueeze_10 = paddle._C_ops.unsqueeze(multiply_3, full_int_array_20)
        del multiply_3

        # pd_op.unsqueeze: (1x252x1x49x49xf32) <- (252x1x49x49xf32, 1xi64)
        unsqueeze_11 = paddle._C_ops.unsqueeze(unsqueeze_10, full_int_array_27)
        del unsqueeze_10

        # pd_op.add: (2x252x6x49x49xf32) <- (2x252x6x49x49xf32, 1x252x1x49x49xf32)
        add_29 = paddle._C_ops.add(reshape_52, unsqueeze_11)

        # pd_op.full_int_array: (4xi64) <- ()
        full_int_array_57 = [-1, 6, 49, 49]

        # pd_op.reshape: (504x6x49x49xf32) <- (2x252x6x49x49xf32, 4xi64)
        reshape_53 = paddle._C_ops.reshape(add_29, full_int_array_57)
        ...
        # pd_op.transpose: (504x49x6x32xf32) <- (504x6x49x32xf32)
        transpose_31 = paddle._C_ops.transpose(matmul_21, [0, 2, 1, 3])
        del matmul_21

        # pd_op.reshape: (504x49x192xf32) <- (504x49x6x32xf32, 3xi64)
        reshape_54 = paddle._C_ops.reshape(transpose_31, full_int_array_50)
        del full_int_array_50

        # pd_op.matmul: (504x49x192xf32) <- (504x49x192xf32, 192x192xf32)
        matmul_22 = paddle._C_ops.matmul(reshape_54, parameter_115, False, False)
        del parameter_115

        # pd_op.add: (504x49x192xf32) <- (504x49x192xf32, 192xf32)
        add_30 = paddle._C_ops.add(matmul_22, parameter_114)
        del parameter_114

        # pd_op.reshape: (504x7x7x192xf32) <- (504x49x192xf32, 4xi64)
        reshape_55 = paddle._C_ops.reshape(add_30, full_int_array_51)
        ...
        # pd_op.transpose: (2x18x7x14x7x192xf32) <- (2x18x14x7x7x192xf32)
        transpose_32 = paddle._C_ops.transpose(reshape_56, [0, 1, 3, 2, 4, 5])
        del reshape_56

        # pd_op.reshape: (2x126x98x192xf32) <- (2x18x7x14x7x192xf32, 4xi64)
        reshape_57 = paddle._C_ops.reshape(transpose_32, full_int_array_53)
        ...
        # pd_op.add: (2x1x1xf32) <- (xf32, 2x1x1xf32)
        add_31 = paddle._C_ops.add(cast_6, uniform_4)
        del uniform_4
        ...
        layer_norm_30, layer_norm_31, layer_norm_32 = (lambda x, f: f(x))(
            paddle._C_ops.layer_norm(
                add_32, parameter_113, parameter_112, float("1e-05"), 2
            ),
            lambda out: out if isinstance(out, (list, tuple)) else (out, None, None),
        )
        ...
        # pd_op.add: (2x11408x768xf32) <- (2x11408x768xf32, 768xf32)
        add_33 = paddle._C_ops.add(matmul_23, parameter_110)
        del parameter_110
        ...
        # pd_op.add: (2x11408x192xf32) <- (2x11408x192xf32, 192xf32)
        add_34 = paddle._C_ops.add(matmul_24, parameter_108)
        del parameter_108
        ...
        # pd_op.add: (2x1x1xf32) <- (xf32, 2x1x1xf32)
        add_35 = paddle._C_ops.add(cast_7, uniform_5)
        del uniform_5
        ...
        # pd_op.divide: (2x11408x192xf32) <- (2x11408x192xf32, xf32)
        divide_5 = paddle._C_ops.divide(add_34, cast_7)
        ...
        # pd_op.reshape: (2x124x92x192xf32) <- (2x11408x192xf32, 4xi64)
        reshape_59 = paddle._C_ops.reshape(add_36, full_int_array_45)

        # pd_op.strided_slice: (2x62x46x192xf32) <- (2x124x92x192xf32, 2xi64, 2xi64, 2xi64)
        ...
        # builtin.combine: ([2x62x46x192xf32, 2x62x46x192xf32, 2x62x46x192xf32, 2x62x46x192xf32]) <- (2x62x46x192xf32, 2x62x46x192xf32, 2x62x46x192xf32, 2x62x46x192xf32)
        combine_1 = [strided_slice_4, strided_slice_5, strided_slice_6, strided_slice_7]

        # pd_op.concat: (2x62x46x768xf32) <- ([2x62x46x192xf32, 2x62x46x192xf32, 2x62x46x192xf32, 2x62x46x192xf32], 1xi32)
        concat_1 = paddle._C_ops.concat(combine_1, full_8)
        del combine_1

        # pd_op.full_int_array: (3xi64) <- ()
        full_int_array_58 = [-1, 2852, 768]

        # pd_op.reshape: (2x2852x768xf32) <- (2x62x46x768xf32, 3xi64)
        reshape_60 = paddle._C_ops.reshape(concat_1, full_int_array_58)
        del full_int_array_58

        # pd_op.layer_norm: (2x2852x768xf32, 2x2852xf32, 2x2852xf32) <- (2x2852x768xf32, 768xf32, 768xf32)
        layer_norm_33, layer_norm_34, layer_norm_35 = (lambda x, f: f(x))(
            paddle._C_ops.layer_norm(
                reshape_60, parameter_107, parameter_106, float("1e-05"), 2
            ),
            lambda out: out if isinstance(out, (list, tuple)) else (out, None, None),
        )
        del parameter_106, parameter_107

        # pd_op.matmul: (2x2852x384xf32) <- (2x2852x768xf32, 768x384xf32)
        ...
        layer_norm_36, layer_norm_37, layer_norm_38 = (lambda x, f: f(x))(
            paddle._C_ops.layer_norm(
                add_36, parameter_104, parameter_103, float("1e-05"), 2
            ),
            lambda out: out if isinstance(out, (list, tuple)) else (out, None, None),
        )
        ...
        # pd_op.transpose: (2x192x124x92xf32) <- (2x124x92x192xf32)
        transpose_1 = paddle._C_ops.transpose(reshape_61, [0, 3, 1, 2])
        del reshape_61
        ...
        transpose_33 = paddle._C_ops.transpose(reshape_62, [0, 1, 3, 2, 4, 5])
        del reshape_62

        # pd_op.reshape: (63x7x7x1xf32) <- (1x9x7x7x7x1xf32, 4xi64)
        reshape_63 = paddle._C_ops.reshape(transpose_33, full_int_array_18)
        del transpose_33
        ...
        # pd_op.layer_norm: (2x2852x384xf32, 2x2852xf32, 2x2852xf32) <- (2x2852x384xf32, 384xf32, 384xf32)
        ...
        # pd_op.full_int_array: (4xi64) <- ()
        full_int_array_60 = [-1, 62, 46, 384]

        # pd_op.reshape: (2x62x46x384xf32) <- (2x2852x384xf32, 4xi64)
        reshape_65 = paddle._C_ops.reshape(layer_norm_39, full_int_array_60)

        # pd_op.pad: (2x63x49x384xf32) <- (2x62x46x384xf32, 1xf32)
        pad_4 = paddle._C_ops.pad(reshape_65, [0, 0, 0, 1, 0, 3, 0, 0], full_4)
        ...
        transpose_34 = paddle._C_ops.transpose(reshape_66, [0, 1, 3, 2, 4, 5])
        del reshape_66
        ...
        reshape_67 = paddle._C_ops.reshape(transpose_34, full_int_array_62)
        ...
        add_37 = paddle._C_ops.add(matmul_26, parameter_99)
        del parameter_99
        ...
        reshape_69 = paddle._C_ops.reshape(add_37, full_int_array_64)

        # pd_op.transpose: (3x126x12x49x32xf32) <- (126x49x3x12x32xf32)
        transpose_35 = paddle._C_ops.transpose(reshape_69, [2, 0, 3, 1, 4])
        del reshape_69

        # pd_op.slice: (126x12x49x32xf32) <- (3x126x12x49x32xf32, 1xi64, 1xi64)
        slice_16 = paddle._C_ops.slice(
            transpose_35, [0], full_int_array_27, full_int_array_20, [1], [0]
        )

        # pd_op.slice: (126x12x49x32xf32) <- (3x126x12x49x32xf32, 1xi64, 1xi64)
        slice_17 = paddle._C_ops.slice(
            transpose_35, [0], full_int_array_20, full_int_array_21, [1], [0]
        )

        # pd_op.slice: (126x12x49x32xf32) <- (3x126x12x49x32xf32, 1xi64, 1xi64)
        slice_18 = paddle._C_ops.slice(
            transpose_35, [0], full_int_array_21, full_int_array_28, [1], [0]
        )
        ...
        transpose_36 = paddle._C_ops.transpose(slice_17, [0, 1, 3, 2])
        del slice_17

        # pd_op.matmul: (126x12x49x49xf32) <- (126x12x49x32xf32, 126x12x32x49xf32)
        matmul_27 = paddle._C_ops.matmul(scale_7, transpose_36, False, False)
        ...
        transpose_37 = paddle._C_ops.transpose(reshape_70, [2, 0, 1])
        del reshape_70

        # pd_op.unsqueeze: (1x12x49x49xf32) <- (12x49x49xf32, 1xi64)
        unsqueeze_14 = paddle._C_ops.unsqueeze(transpose_37, full_int_array_27)

        # pd_op.add: (126x12x49x49xf32) <- (126x12x49x49xf32, 1x12x49x49xf32)
        add_38 = paddle._C_ops.add(matmul_27, unsqueeze_14)
        ...
        reshape_71 = paddle._C_ops.reshape(transpose_38, full_int_array_65)

        # pd_op.matmul: (126x49x384xf32) <- (126x49x384xf32, 384x384xf32)
        matmul_29 = paddle._C_ops.matmul(reshape_71, parameter_98, False, False)
        del parameter_98

        # pd_op.add: (126x49x384xf32) <- (126x49x384xf32, 384xf32)
        add_39 = paddle._C_ops.add(matmul_29, parameter_97)
        del parameter_97
        ...
        reshape_72 = paddle._C_ops.reshape(add_39, full_int_array_66)

        # pd_op.reshape: (2x9x7x7x7x384xf32) <- (126x7x7x384xf32, 6xi64)
        reshape_73 = paddle._C_ops.reshape(reshape_72, full_int_array_61)

        # pd_op.transpose: (2x9x7x7x7x384xf32) <- (2x9x7x7x7x384xf32)
        transpose_39 = paddle._C_ops.transpose(reshape_73, [0, 1, 3, 2, 4, 5])
        del reshape_73
        ...
        reshape_74 = paddle._C_ops.reshape(transpose_39, full_int_array_67)

        # pd_op.full_int_array: (2xi64) <- ()
        full_int_array_68 = [62, 46]

        # pd_op.assign: (2xi64) <- (2xi64)
        assign_164 = full_int_array_68
        ...
        # pd_op.slice: (2x62x46x384xf32) <- (2x63x49x384xf32, 2xi64, 2xi64)
        slice_19 = paddle._C_ops.slice(
            reshape_74, [1, 2], full_int_array_2, full_int_array_68, [1, 1], []
        )

        # pd_op.full_int_array: (3xi64) <- ()
        full_int_array_69 = [-1, 2852, 384]

        # pd_op.reshape: (2x2852x384xf32) <- (2x62x46x384xf32, 3xi64)
        ...
        # pd_op.add: (2x1x1xf32) <- (xf32, 2x1x1xf32)
        add_40 = paddle._C_ops.add(cast_9, uniform_6)
        del uniform_6
        ...
        # pd_op.divide: (2x2852x384xf32) <- (2x2852x384xf32, xf32)
        divide_6 = paddle._C_ops.divide(reshape_75, cast_9)

        # pd_op.multiply: (2x2852x384xf32) <- (2x2852x384xf32, 2x1x1xf32)
        multiply_9 = paddle._C_ops.multiply(divide_6, floor_6)

        # pd_op.add: (2x2852x384xf32) <- (2x2852x384xf32, 2x2852x384xf32)
        add_41 = paddle._C_ops.add(matmul_25, multiply_9)

        # pd_op.layer_norm: (2x2852x384xf32, 2x2852xf32, 2x2852xf32) <- (2x2852x384xf32, 384xf32, 384xf32)
        layer_norm_42, layer_norm_43, layer_norm_44 = (lambda x, f: f(x))(
            paddle._C_ops.layer_norm(
                add_41, parameter_96, parameter_95, float("1e-05"), 2
            ),
            lambda out: out if isinstance(out, (list, tuple)) else (out, None, None),
        )
        del parameter_95, parameter_96

        # pd_op.matmul: (2x2852x1536xf32) <- (2x2852x384xf32, 384x1536xf32)
        matmul_30 = paddle._C_ops.matmul(layer_norm_42, parameter_94, False, False)
        del parameter_94

        # pd_op.add: (2x2852x1536xf32) <- (2x2852x1536xf32, 1536xf32)
        add_42 = paddle._C_ops.add(matmul_30, parameter_93)
        del parameter_93

        # pd_op.gelu: (2x2852x1536xf32) <- (2x2852x1536xf32)
        gelu_4 = paddle._C_ops.gelu(add_42, False)

        # pd_op.matmul: (2x2852x384xf32) <- (2x2852x1536xf32, 1536x384xf32)
        matmul_31 = paddle._C_ops.matmul(gelu_4, parameter_92, False, False)
        del parameter_92

        # pd_op.add: (2x2852x384xf32) <- (2x2852x384xf32, 384xf32)
        add_43 = paddle._C_ops.add(matmul_31, parameter_91)
        del parameter_91
        ...
        # pd_op.add: (2x1x1xf32) <- (xf32, 2x1x1xf32)
        add_44 = paddle._C_ops.add(cast_10, uniform_7)
        del uniform_7
        ...
        # pd_op.divide: (2x2852x384xf32) <- (2x2852x384xf32, xf32)
        divide_7 = paddle._C_ops.divide(add_43, cast_10)
        ...
        layer_norm_45, layer_norm_46, layer_norm_47 = (lambda x, f: f(x))(
            paddle._C_ops.layer_norm(
                add_45, parameter_90, parameter_89, float("1e-05"), 2
            ),
            lambda out: out if isinstance(out, (list, tuple)) else (out, None, None),
        )
        ...
        # pd_op.pad: (2x63x49x384xf32) <- (2x62x46x384xf32, 1xf32)
        pad_5 = paddle._C_ops.pad(reshape_76, [0, 0, 0, 1, 0, 3, 0, 0], full_4)
        ...
        # pd_op.transpose: (2x9x7x7x7x384xf32) <- (2x9x7x7x7x384xf32)
        transpose_40 = paddle._C_ops.transpose(reshape_77, [0, 1, 3, 2, 4, 5])
        del reshape_77

        # pd_op.reshape: (126x7x7x384xf32) <- (2x9x7x7x7x384xf32, 4xi64)
        reshape_78 = paddle._C_ops.reshape(transpose_40, full_int_array_62)

        # pd_op.reshape: (126x49x384xf32) <- (126x7x7x384xf32, 3xi64)
        reshape_79 = paddle._C_ops.reshape(reshape_78, full_int_array_63)

        # pd_op.matmul: (126x49x1152xf32) <- (126x49x384xf32, 384x1152xf32)
        matmul_32 = paddle._C_ops.matmul(reshape_79, parameter_88, False, False)
        del parameter_88

        # pd_op.add: (126x49x1152xf32) <- (126x49x1152xf32, 1152xf32)
        add_46 = paddle._C_ops.add(matmul_32, parameter_87)
        del parameter_87

        # pd_op.reshape: (126x49x3x12x32xf32) <- (126x49x1152xf32, 5xi64)
        reshape_80 = paddle._C_ops.reshape(add_46, full_int_array_64)

        # pd_op.transpose: (3x126x12x49x32xf32) <- (126x49x3x12x32xf32)
        transpose_41 = paddle._C_ops.transpose(reshape_80, [2, 0, 3, 1, 4])
        del reshape_80

        # pd_op.slice: (126x12x49x32xf32) <- (3x126x12x49x32xf32, 1xi64, 1xi64)
        slice_20 = paddle._C_ops.slice(
            transpose_41, [0], full_int_array_27, full_int_array_20, [1], [0]
        )

        # pd_op.slice: (126x12x49x32xf32) <- (3x126x12x49x32xf32, 1xi64, 1xi64)
        slice_21 = paddle._C_ops.slice(
            transpose_41, [0], full_int_array_20, full_int_array_21, [1], [0]
        )

        # pd_op.slice: (126x12x49x32xf32) <- (3x126x12x49x32xf32, 1xi64, 1xi64)
        slice_22 = paddle._C_ops.slice(
            transpose_41, [0], full_int_array_21, full_int_array_28, [1], [0]
        )
        ...
        # pd_op.transpose: (126x12x32x49xf32) <- (126x12x49x32xf32)
        transpose_42 = paddle._C_ops.transpose(slice_21, [0, 1, 3, 2])
        del slice_21

        # pd_op.matmul: (126x12x49x49xf32) <- (126x12x49x32xf32, 126x12x32x49xf32)
        matmul_33 = paddle._C_ops.matmul(scale_8, transpose_42, False, False)
        ...
        # pd_op.transpose: (12x49x49xf32) <- (49x49x12xf32)
        transpose_43 = paddle._C_ops.transpose(reshape_81, [2, 0, 1])
        del reshape_81

        # pd_op.unsqueeze: (1x12x49x49xf32) <- (12x49x49xf32, 1xi64)
        unsqueeze_15 = paddle._C_ops.unsqueeze(transpose_43, full_int_array_27)

        # pd_op.add: (126x12x49x49xf32) <- (126x12x49x49xf32, 1x12x49x49xf32)
        add_47 = paddle._C_ops.add(matmul_33, unsqueeze_15)

        # pd_op.full_int_array: (5xi64) <- ()
        full_int_array_70 = [-1, 63, 12, 49, 49]

        # pd_op.reshape: (2x63x12x49x49xf32) <- (126x12x49x49xf32, 5xi64)
        reshape_82 = paddle._C_ops.reshape(add_47, full_int_array_70)
        ...
        add_48 = paddle._C_ops.add(reshape_82, unsqueeze_17)

        # pd_op.full_int_array: (4xi64) <- ()
        full_int_array_71 = [-1, 12, 49, 49]

        # pd_op.reshape: (126x12x49x49xf32) <- (2x63x12x49x49xf32, 4xi64)
        reshape_83 = paddle._C_ops.reshape(add_48, full_int_array_71)
        ...
        # pd_op.reshape: (126x49x384xf32) <- (126x49x12x32xf32, 3xi64)
        reshape_84 = paddle._C_ops.reshape(transpose_44, full_int_array_65)

        # pd_op.matmul: (126x49x384xf32) <- (126x49x384xf32, 384x384xf32)
        matmul_35 = paddle._C_ops.matmul(reshape_84, parameter_86, False, False)
        del parameter_86

        # pd_op.add: (126x49x384xf32) <- (126x49x384xf32, 384xf32)
        add_49 = paddle._C_ops.add(matmul_35, parameter_85)
        del parameter_85

        # pd_op.reshape: (126x7x7x384xf32) <- (126x49x384xf32, 4xi64)
        reshape_85 = paddle._C_ops.reshape(add_49, full_int_array_66)

        # pd_op.reshape: (2x9x7x7x7x384xf32) <- (126x7x7x384xf32, 6xi64)
        reshape_86 = paddle._C_ops.reshape(reshape_85, full_int_array_61)

        # pd_op.transpose: (2x9x7x7x7x384xf32) <- (2x9x7x7x7x384xf32)
        transpose_45 = paddle._C_ops.transpose(reshape_86, [0, 1, 3, 2, 4, 5])
        del reshape_86

        # pd_op.reshape: (2x63x49x384xf32) <- (2x9x7x7x7x384xf32, 4xi64)
        reshape_87 = paddle._C_ops.reshape(transpose_45, full_int_array_67)
        ...
        # pd_op.add: (2x1x1xf32) <- (xf32, 2x1x1xf32)
        add_50 = paddle._C_ops.add(cast_11, uniform_8)
        del uniform_8
        ...
        layer_norm_48, layer_norm_49, layer_norm_50 = (lambda x, f: f(x))(
            paddle._C_ops.layer_norm(
                add_51, parameter_84, parameter_83, float("1e-05"), 2
            ),
            lambda out: out if isinstance(out, (list, tuple)) else (out, None, None),
        )
        ...
        # pd_op.add: (2x2852x1536xf32) <- (2x2852x1536xf32, 1536xf32)
        add_52 = paddle._C_ops.add(matmul_36, parameter_81)
        del parameter_81
        ...
        # pd_op.add: (2x2852x384xf32) <- (2x2852x384xf32, 384xf32)
        add_53 = paddle._C_ops.add(matmul_37, parameter_79)
        del parameter_79
        ...
        # pd_op.add: (2x1x1xf32) <- (xf32, 2x1x1xf32)
        add_54 = paddle._C_ops.add(cast_12, uniform_9)
        del uniform_9
        ...
        # pd_op.divide: (2x2852x384xf32) <- (2x2852x384xf32, xf32)
        divide_9 = paddle._C_ops.divide(add_53, cast_12)
        ...
        layer_norm_51, layer_norm_52, layer_norm_53 = (lambda x, f: f(x))(
            paddle._C_ops.layer_norm(
                add_55, parameter_78, parameter_77, float("1e-05"), 2
            ),
            lambda out: out if isinstance(out, (list, tuple)) else (out, None, None),
        )
        ...
        # pd_op.pad: (2x63x49x384xf32) <- (2x62x46x384xf32, 1xf32)
        pad_6 = paddle._C_ops.pad(reshape_89, [0, 0, 0, 1, 0, 3, 0, 0], full_4)

        # pd_op.reshape: (2x9x7x7x7x384xf32) <- (2x63x49x384xf32, 6xi64)
        reshape_90 = paddle._C_ops.reshape(pad_6, full_int_array_61)

        # pd_op.transpose: (2x9x7x7x7x384xf32) <- (2x9x7x7x7x384xf32)
        transpose_46 = paddle._C_ops.transpose(reshape_90, [0, 1, 3, 2, 4, 5])
        del reshape_90

        # pd_op.reshape: (126x7x7x384xf32) <- (2x9x7x7x7x384xf32, 4xi64)
        reshape_91 = paddle._C_ops.reshape(transpose_46, full_int_array_62)

        # pd_op.reshape: (126x49x384xf32) <- (126x7x7x384xf32, 3xi64)
        reshape_92 = paddle._C_ops.reshape(reshape_91, full_int_array_63)

        # pd_op.matmul: (126x49x1152xf32) <- (126x49x384xf32, 384x1152xf32)
        matmul_38 = paddle._C_ops.matmul(reshape_92, parameter_76, False, False)
        del parameter_76

        # pd_op.add: (126x49x1152xf32) <- (126x49x1152xf32, 1152xf32)
        add_56 = paddle._C_ops.add(matmul_38, parameter_75)
        del parameter_75

        # pd_op.reshape: (126x49x3x12x32xf32) <- (126x49x1152xf32, 5xi64)
        reshape_93 = paddle._C_ops.reshape(add_56, full_int_array_64)

        # pd_op.transpose: (3x126x12x49x32xf32) <- (126x49x3x12x32xf32)
        transpose_47 = paddle._C_ops.transpose(reshape_93, [2, 0, 3, 1, 4])
        del reshape_93

        # pd_op.slice: (126x12x49x32xf32) <- (3x126x12x49x32xf32, 1xi64, 1xi64)
        slice_24 = paddle._C_ops.slice(
            transpose_47, [0], full_int_array_27, full_int_array_20, [1], [0]
        )

        # pd_op.slice: (126x12x49x32xf32) <- (3x126x12x49x32xf32, 1xi64, 1xi64)
        slice_25 = paddle._C_ops.slice(
            transpose_47, [0], full_int_array_20, full_int_array_21, [1], [0]
        )

        # pd_op.slice: (126x12x49x32xf32) <- (3x126x12x49x32xf32, 1xi64, 1xi64)
        slice_26 = paddle._C_ops.slice(
            transpose_47, [0], full_int_array_21, full_int_array_28, [1], [0]
        )
        ...
        # pd_op.transpose: (126x12x32x49xf32) <- (126x12x49x32xf32)
        transpose_48 = paddle._C_ops.transpose(slice_25, [0, 1, 3, 2])
        del slice_25

        # pd_op.matmul: (126x12x49x49xf32) <- (126x12x49x32xf32, 126x12x32x49xf32)
        matmul_39 = paddle._C_ops.matmul(scale_9, transpose_48, False, False)
        ...
        # pd_op.transpose: (12x49x49xf32) <- (49x49x12xf32)
        transpose_49 = paddle._C_ops.transpose(reshape_94, [2, 0, 1])
        del reshape_94

        # pd_op.unsqueeze: (1x12x49x49xf32) <- (12x49x49xf32, 1xi64)
        unsqueeze_18 = paddle._C_ops.unsqueeze(transpose_49, full_int_array_27)

        # pd_op.add: (126x12x49x49xf32) <- (126x12x49x49xf32, 1x12x49x49xf32)
        add_57 = paddle._C_ops.add(matmul_39, unsqueeze_18)
        ...
        # pd_op.transpose: (126x49x12x32xf32) <- (126x12x49x32xf32)
        transpose_50 = paddle._C_ops.transpose(matmul_40, [0, 2, 1, 3])
        del matmul_40

        # pd_op.reshape: (126x49x384xf32) <- (126x49x12x32xf32, 3xi64)
        reshape_95 = paddle._C_ops.reshape(transpose_50, full_int_array_65)

        # pd_op.matmul: (126x49x384xf32) <- (126x49x384xf32, 384x384xf32)
        matmul_41 = paddle._C_ops.matmul(reshape_95, parameter_74, False, False)
        del parameter_74

        # pd_op.add: (126x49x384xf32) <- (126x49x384xf32, 384xf32)
        add_58 = paddle._C_ops.add(matmul_41, parameter_73)
        del parameter_73

        # pd_op.reshape: (126x7x7x384xf32) <- (126x49x384xf32, 4xi64)
        reshape_96 = paddle._C_ops.reshape(add_58, full_int_array_66)

        # pd_op.reshape: (2x9x7x7x7x384xf32) <- (126x7x7x384xf32, 6xi64)
        reshape_97 = paddle._C_ops.reshape(reshape_96, full_int_array_61)

        # pd_op.transpose: (2x9x7x7x7x384xf32) <- (2x9x7x7x7x384xf32)
        transpose_51 = paddle._C_ops.transpose(reshape_97, [0, 1, 3, 2, 4, 5])
        del reshape_97

        # pd_op.reshape: (2x63x49x384xf32) <- (2x9x7x7x7x384xf32, 4xi64)
        reshape_98 = paddle._C_ops.reshape(transpose_51, full_int_array_67)
        ...
        # pd_op.add: (2x1x1xf32) <- (xf32, 2x1x1xf32)
        add_59 = paddle._C_ops.add(cast_13, uniform_10)
        del uniform_10
        ...
        layer_norm_54, layer_norm_55, layer_norm_56 = (lambda x, f: f(x))(
            paddle._C_ops.layer_norm(
                add_60, parameter_72, parameter_71, float("1e-05"), 2
            ),
            lambda out: out if isinstance(out, (list, tuple)) else (out, None, None),
        )
        ...
        # pd_op.add: (2x2852x1536xf32) <- (2x2852x1536xf32, 1536xf32)
        add_61 = paddle._C_ops.add(matmul_42, parameter_69)
        del parameter_69
        ...
        # pd_op.add: (2x2852x384xf32) <- (2x2852x384xf32, 384xf32)
        add_62 = paddle._C_ops.add(matmul_43, parameter_67)
        del parameter_67
        ...
        # pd_op.add: (2x1x1xf32) <- (xf32, 2x1x1xf32)
        add_63 = paddle._C_ops.add(cast_14, uniform_11)
        del uniform_11
        ...
        # pd_op.divide: (2x2852x384xf32) <- (2x2852x384xf32, xf32)
        divide_11 = paddle._C_ops.divide(add_62, cast_14)
        ...
        layer_norm_57, layer_norm_58, layer_norm_59 = (lambda x, f: f(x))(
            paddle._C_ops.layer_norm(
                add_64, parameter_66, parameter_65, float("1e-05"), 2
            ),
            lambda out: out if isinstance(out, (list, tuple)) else (out, None, None),
        )
        ...
        # pd_op.pad: (2x63x49x384xf32) <- (2x62x46x384xf32, 1xf32)
        pad_7 = paddle._C_ops.pad(reshape_100, [0, 0, 0, 1, 0, 3, 0, 0], full_4)
        ...
        # pd_op.transpose: (2x9x7x7x7x384xf32) <- (2x9x7x7x7x384xf32)
        transpose_52 = paddle._C_ops.transpose(reshape_101, [0, 1, 3, 2, 4, 5])
        del reshape_101

        # pd_op.reshape: (126x7x7x384xf32) <- (2x9x7x7x7x384xf32, 4xi64)
        reshape_102 = paddle._C_ops.reshape(transpose_52, full_int_array_62)

        # pd_op.reshape: (126x49x384xf32) <- (126x7x7x384xf32, 3xi64)
        reshape_103 = paddle._C_ops.reshape(reshape_102, full_int_array_63)

        # pd_op.matmul: (126x49x1152xf32) <- (126x49x384xf32, 384x1152xf32)
        matmul_44 = paddle._C_ops.matmul(reshape_103, parameter_64, False, False)
        del parameter_64

        # pd_op.add: (126x49x1152xf32) <- (126x49x1152xf32, 1152xf32)
        add_65 = paddle._C_ops.add(matmul_44, parameter_63)
        del parameter_63

        # pd_op.reshape: (126x49x3x12x32xf32) <- (126x49x1152xf32, 5xi64)
        reshape_104 = paddle._C_ops.reshape(add_65, full_int_array_64)

        # pd_op.transpose: (3x126x12x49x32xf32) <- (126x49x3x12x32xf32)
        transpose_53 = paddle._C_ops.transpose(reshape_104, [2, 0, 3, 1, 4])
        del reshape_104

        # pd_op.slice: (126x12x49x32xf32) <- (3x126x12x49x32xf32, 1xi64, 1xi64)
        slice_28 = paddle._C_ops.slice(
            transpose_53, [0], full_int_array_27, full_int_array_20, [1], [0]
        )
        ...
        slice_30 = paddle._C_ops.slice(
            transpose_53, [0], full_int_array_21, full_int_array_28, [1], [0]
        )
        ...
        # pd_op.transpose: (126x12x32x49xf32) <- (126x12x49x32xf32)
        transpose_54 = paddle._C_ops.transpose(slice_29, [0, 1, 3, 2])
        del slice_29

        # pd_op.matmul: (126x12x49x49xf32) <- (126x12x49x32xf32, 126x12x32x49xf32)
        matmul_45 = paddle._C_ops.matmul(scale_10, transpose_54, False, False)
        ...
        # pd_op.transpose: (12x49x49xf32) <- (49x49x12xf32)
        transpose_55 = paddle._C_ops.transpose(reshape_105, [2, 0, 1])
        del reshape_105

        # pd_op.unsqueeze: (1x12x49x49xf32) <- (12x49x49xf32, 1xi64)
        unsqueeze_19 = paddle._C_ops.unsqueeze(transpose_55, full_int_array_27)

        # pd_op.add: (126x12x49x49xf32) <- (126x12x49x49xf32, 1x12x49x49xf32)
        add_66 = paddle._C_ops.add(matmul_45, unsqueeze_19)

        # pd_op.reshape: (2x63x12x49x49xf32) <- (126x12x49x49xf32, 5xi64)
        reshape_106 = paddle._C_ops.reshape(add_66, full_int_array_70)
        ...
        # pd_op.add: (2x63x12x49x49xf32) <- (2x63x12x49x49xf32, 1x63x1x49x49xf32)
        add_67 = paddle._C_ops.add(reshape_106, unsqueeze_21)

        # pd_op.reshape: (126x12x49x49xf32) <- (2x63x12x49x49xf32, 4xi64)
        reshape_107 = paddle._C_ops.reshape(add_67, full_int_array_71)
        ...
        # pd_op.reshape: (126x49x384xf32) <- (126x49x12x32xf32, 3xi64)
        reshape_108 = paddle._C_ops.reshape(transpose_56, full_int_array_65)

        # pd_op.matmul: (126x49x384xf32) <- (126x49x384xf32, 384x384xf32)
        matmul_47 = paddle._C_ops.matmul(reshape_108, parameter_62, False, False)
        del parameter_62

        # pd_op.add: (126x49x384xf32) <- (126x49x384xf32, 384xf32)
        add_68 = paddle._C_ops.add(matmul_47, parameter_61)
        del parameter_61

        # pd_op.reshape: (126x7x7x384xf32) <- (126x49x384xf32, 4xi64)
        reshape_109 = paddle._C_ops.reshape(add_68, full_int_array_66)

        # pd_op.reshape: (2x9x7x7x7x384xf32) <- (126x7x7x384xf32, 6xi64)
        reshape_110 = paddle._C_ops.reshape(reshape_109, full_int_array_61)

        # pd_op.transpose: (2x9x7x7x7x384xf32) <- (2x9x7x7x7x384xf32)
        transpose_57 = paddle._C_ops.transpose(reshape_110, [0, 1, 3, 2, 4, 5])
        del reshape_110

        # pd_op.reshape: (2x63x49x384xf32) <- (2x9x7x7x7x384xf32, 4xi64)
        reshape_111 = paddle._C_ops.reshape(transpose_57, full_int_array_67)
        ...
        # pd_op.add: (2x1x1xf32) <- (xf32, 2x1x1xf32)
        add_69 = paddle._C_ops.add(cast_15, uniform_12)
        del uniform_12
        ...
        layer_norm_60, layer_norm_61, layer_norm_62 = (lambda x, f: f(x))(
            paddle._C_ops.layer_norm(
                add_70, parameter_60, parameter_59, float("1e-05"), 2
            ),
            lambda out: out if isinstance(out, (list, tuple)) else (out, None, None),
        )
        ...
        # pd_op.add: (2x2852x1536xf32) <- (2x2852x1536xf32, 1536xf32)
        add_71 = paddle._C_ops.add(matmul_48, parameter_57)
        del parameter_57
        ...
        # pd_op.add: (2x2852x384xf32) <- (2x2852x384xf32, 384xf32)
        add_72 = paddle._C_ops.add(matmul_49, parameter_55)
        del parameter_55
        ...
        # pd_op.add: (2x1x1xf32) <- (xf32, 2x1x1xf32)
        add_73 = paddle._C_ops.add(cast_16, uniform_13)
        del uniform_13
        ...
        # pd_op.divide: (2x2852x384xf32) <- (2x2852x384xf32, xf32)
        divide_13 = paddle._C_ops.divide(add_72, cast_16)
        ...
        layer_norm_63, layer_norm_64, layer_norm_65 = (lambda x, f: f(x))(
            paddle._C_ops.layer_norm(
                add_74, parameter_54, parameter_53, float("1e-05"), 2
            ),
            lambda out: out if isinstance(out, (list, tuple)) else (out, None, None),
        )
        ...
        # pd_op.pad: (2x63x49x384xf32) <- (2x62x46x384xf32, 1xf32)
        pad_8 = paddle._C_ops.pad(reshape_113, [0, 0, 0, 1, 0, 3, 0, 0], full_4)

        # pd_op.reshape: (2x9x7x7x7x384xf32) <- (2x63x49x384xf32, 6xi64)
        reshape_114 = paddle._C_ops.reshape(pad_8, full_int_array_61)

        # pd_op.transpose: (2x9x7x7x7x384xf32) <- (2x9x7x7x7x384xf32)
        transpose_58 = paddle._C_ops.transpose(reshape_114, [0, 1, 3, 2, 4, 5])
        del reshape_114

        # pd_op.reshape: (126x7x7x384xf32) <- (2x9x7x7x7x384xf32, 4xi64)
        reshape_115 = paddle._C_ops.reshape(transpose_58, full_int_array_62)

        # pd_op.reshape: (126x49x384xf32) <- (126x7x7x384xf32, 3xi64)
        reshape_116 = paddle._C_ops.reshape(reshape_115, full_int_array_63)

        # pd_op.matmul: (126x49x1152xf32) <- (126x49x384xf32, 384x1152xf32)
        matmul_50 = paddle._C_ops.matmul(reshape_116, parameter_52, False, False)
        del parameter_52

        # pd_op.add: (126x49x1152xf32) <- (126x49x1152xf32, 1152xf32)
        add_75 = paddle._C_ops.add(matmul_50, parameter_51)
        del parameter_51

        # pd_op.reshape: (126x49x3x12x32xf32) <- (126x49x1152xf32, 5xi64)
        reshape_117 = paddle._C_ops.reshape(add_75, full_int_array_64)

        # pd_op.transpose: (3x126x12x49x32xf32) <- (126x49x3x12x32xf32)
        transpose_59 = paddle._C_ops.transpose(reshape_117, [2, 0, 3, 1, 4])
        del reshape_117

        # pd_op.slice: (126x12x49x32xf32) <- (3x126x12x49x32xf32, 1xi64, 1xi64)
        slice_32 = paddle._C_ops.slice(
            transpose_59, [0], full_int_array_27, full_int_array_20, [1], [0]
        )
        ...
        slice_34 = paddle._C_ops.slice(
            transpose_59, [0], full_int_array_21, full_int_array_28, [1], [0]
        )
        ...
        # pd_op.transpose: (126x12x32x49xf32) <- (126x12x49x32xf32)
        transpose_60 = paddle._C_ops.transpose(slice_33, [0, 1, 3, 2])
        del slice_33

        # pd_op.matmul: (126x12x49x49xf32) <- (126x12x49x32xf32, 126x12x32x49xf32)
        matmul_51 = paddle._C_ops.matmul(scale_11, transpose_60, False, False)
        ...
        # pd_op.transpose: (12x49x49xf32) <- (49x49x12xf32)
        transpose_61 = paddle._C_ops.transpose(reshape_118, [2, 0, 1])
        del reshape_118

        # pd_op.unsqueeze: (1x12x49x49xf32) <- (12x49x49xf32, 1xi64)
        unsqueeze_22 = paddle._C_ops.unsqueeze(transpose_61, full_int_array_27)

        # pd_op.add: (126x12x49x49xf32) <- (126x12x49x49xf32, 1x12x49x49xf32)
        add_76 = paddle._C_ops.add(matmul_51, unsqueeze_22)
        ...
        # pd_op.transpose: (126x49x12x32xf32) <- (126x12x49x32xf32)
        transpose_62 = paddle._C_ops.transpose(matmul_52, [0, 2, 1, 3])
        del matmul_52

        # pd_op.reshape: (126x49x384xf32) <- (126x49x12x32xf32, 3xi64)
        reshape_119 = paddle._C_ops.reshape(transpose_62, full_int_array_65)

        # pd_op.matmul: (126x49x384xf32) <- (126x49x384xf32, 384x384xf32)
        matmul_53 = paddle._C_ops.matmul(reshape_119, parameter_50, False, False)
        del parameter_50

        # pd_op.add: (126x49x384xf32) <- (126x49x384xf32, 384xf32)
        add_77 = paddle._C_ops.add(matmul_53, parameter_49)
        del parameter_49

        # pd_op.reshape: (126x7x7x384xf32) <- (126x49x384xf32, 4xi64)
        reshape_120 = paddle._C_ops.reshape(add_77, full_int_array_66)

        # pd_op.reshape: (2x9x7x7x7x384xf32) <- (126x7x7x384xf32, 6xi64)
        reshape_121 = paddle._C_ops.reshape(reshape_120, full_int_array_61)

        # pd_op.transpose: (2x9x7x7x7x384xf32) <- (2x9x7x7x7x384xf32)
        transpose_63 = paddle._C_ops.transpose(reshape_121, [0, 1, 3, 2, 4, 5])
        del reshape_121

        # pd_op.reshape: (2x63x49x384xf32) <- (2x9x7x7x7x384xf32, 4xi64)
        reshape_122 = paddle._C_ops.reshape(transpose_63, full_int_array_67)
        ...
        # pd_op.add: (2x1x1xf32) <- (xf32, 2x1x1xf32)
        add_78 = paddle._C_ops.add(cast_17, uniform_14)
        del uniform_14
        ...
        layer_norm_66, layer_norm_67, layer_norm_68 = (lambda x, f: f(x))(
            paddle._C_ops.layer_norm(
                add_79, parameter_48, parameter_47, float("1e-05"), 2
            ),
            lambda out: out if isinstance(out, (list, tuple)) else (out, None, None),
        )
        ...
        # pd_op.add: (2x2852x1536xf32) <- (2x2852x1536xf32, 1536xf32)
        add_80 = paddle._C_ops.add(matmul_54, parameter_45)
        del parameter_45
        ...
        # pd_op.add: (2x2852x384xf32) <- (2x2852x384xf32, 384xf32)
        add_81 = paddle._C_ops.add(matmul_55, parameter_43)
        del parameter_43
        ...
        # pd_op.add: (2x1x1xf32) <- (xf32, 2x1x1xf32)
        add_82 = paddle._C_ops.add(cast_18, uniform_15)
        del uniform_15
        ...
        # pd_op.divide: (2x2852x384xf32) <- (2x2852x384xf32, xf32)
        divide_15 = paddle._C_ops.divide(add_81, cast_18)
        ...
        layer_norm_69, layer_norm_70, layer_norm_71 = (lambda x, f: f(x))(
            paddle._C_ops.layer_norm(
                add_83, parameter_42, parameter_41, float("1e-05"), 2
            ),
            lambda out: out if isinstance(out, (list, tuple)) else (out, None, None),
        )
        ...
        # pd_op.pad: (2x63x49x384xf32) <- (2x62x46x384xf32, 1xf32)
        pad_9 = paddle._C_ops.pad(reshape_124, [0, 0, 0, 1, 0, 3, 0, 0], full_4)
        ...
        # pd_op.transpose: (2x9x7x7x7x384xf32) <- (2x9x7x7x7x384xf32)
        transpose_64 = paddle._C_ops.transpose(reshape_125, [0, 1, 3, 2, 4, 5])
        del reshape_125

        # pd_op.reshape: (126x7x7x384xf32) <- (2x9x7x7x7x384xf32, 4xi64)
        reshape_126 = paddle._C_ops.reshape(transpose_64, full_int_array_62)
        ...
        # pd_op.matmul: (126x49x1152xf32) <- (126x49x384xf32, 384x1152xf32)
        matmul_56 = paddle._C_ops.matmul(reshape_127, parameter_40, False, False)
        del parameter_40

        # pd_op.add: (126x49x1152xf32) <- (126x49x1152xf32, 1152xf32)
        add_84 = paddle._C_ops.add(matmul_56, parameter_39)
        del parameter_39

        # pd_op.reshape: (126x49x3x12x32xf32) <- (126x49x1152xf32, 5xi64)
        reshape_128 = paddle._C_ops.reshape(add_84, full_int_array_64)
        del full_int_array_64

        # pd_op.transpose: (3x126x12x49x32xf32) <- (126x49x3x12x32xf32)
        transpose_65 = paddle._C_ops.transpose(reshape_128, [2, 0, 3, 1, 4])
        del reshape_128

        # pd_op.slice: (126x12x49x32xf32) <- (3x126x12x49x32xf32, 1xi64, 1xi64)
        slice_36 = paddle._C_ops.slice(
            transpose_65, [0], full_int_array_27, full_int_array_20, [1], [0]
        )

        # pd_op.slice: (126x12x49x32xf32) <- (3x126x12x49x32xf32, 1xi64, 1xi64)
        slice_37 = paddle._C_ops.slice(
            transpose_65, [0], full_int_array_20, full_int_array_21, [1], [0]
        )

        # pd_op.slice: (126x12x49x32xf32) <- (3x126x12x49x32xf32, 1xi64, 1xi64)
        slice_38 = paddle._C_ops.slice(
            transpose_65, [0], full_int_array_21, full_int_array_28, [1], [0]
        )
        ...
        # pd_op.transpose: (126x12x32x49xf32) <- (126x12x49x32xf32)
        transpose_66 = paddle._C_ops.transpose(slice_37, [0, 1, 3, 2])
        del slice_37

        # pd_op.matmul: (126x12x49x49xf32) <- (126x12x49x32xf32, 126x12x32x49xf32)
        matmul_57 = paddle._C_ops.matmul(scale_12, transpose_66, False, False)
        ...
        # pd_op.transpose: (12x49x49xf32) <- (49x49x12xf32)
        transpose_67 = paddle._C_ops.transpose(reshape_129, [2, 0, 1])
        del reshape_129

        # pd_op.unsqueeze: (1x12x49x49xf32) <- (12x49x49xf32, 1xi64)
        unsqueeze_23 = paddle._C_ops.unsqueeze(transpose_67, full_int_array_27)

        # pd_op.add: (126x12x49x49xf32) <- (126x12x49x49xf32, 1x12x49x49xf32)
        add_85 = paddle._C_ops.add(matmul_57, unsqueeze_23)

        # pd_op.reshape: (2x63x12x49x49xf32) <- (126x12x49x49xf32, 5xi64)
        reshape_130 = paddle._C_ops.reshape(add_85, full_int_array_70)
        ...
        # pd_op.add: (2x63x12x49x49xf32) <- (2x63x12x49x49xf32, 1x63x1x49x49xf32)
        add_86 = paddle._C_ops.add(reshape_130, unsqueeze_25)

        # pd_op.reshape: (126x12x49x49xf32) <- (2x63x12x49x49xf32, 4xi64)
        reshape_131 = paddle._C_ops.reshape(add_86, full_int_array_71)
        ...
        # pd_op.transpose: (126x49x12x32xf32) <- (126x12x49x32xf32)
        transpose_68 = paddle._C_ops.transpose(matmul_58, [0, 2, 1, 3])
        del matmul_58

        # pd_op.reshape: (126x49x384xf32) <- (126x49x12x32xf32, 3xi64)
        reshape_132 = paddle._C_ops.reshape(transpose_68, full_int_array_65)
        del full_int_array_65

        # pd_op.matmul: (126x49x384xf32) <- (126x49x384xf32, 384x384xf32)
        matmul_59 = paddle._C_ops.matmul(reshape_132, parameter_38, False, False)
        del parameter_38

        # pd_op.add: (126x49x384xf32) <- (126x49x384xf32, 384xf32)
        add_87 = paddle._C_ops.add(matmul_59, parameter_37)
        del parameter_37

        # pd_op.reshape: (126x7x7x384xf32) <- (126x49x384xf32, 4xi64)
        reshape_133 = paddle._C_ops.reshape(add_87, full_int_array_66)
        ...
        # pd_op.transpose: (2x9x7x7x7x384xf32) <- (2x9x7x7x7x384xf32)
        transpose_69 = paddle._C_ops.transpose(reshape_134, [0, 1, 3, 2, 4, 5])
        del reshape_134

        # pd_op.reshape: (2x63x49x384xf32) <- (2x9x7x7x7x384xf32, 4xi64)
        reshape_135 = paddle._C_ops.reshape(transpose_69, full_int_array_67)
        ...
        # pd_op.add: (2x1x1xf32) <- (xf32, 2x1x1xf32)
        add_88 = paddle._C_ops.add(cast_19, uniform_16)
        del uniform_16
        ...
        layer_norm_72, layer_norm_73, layer_norm_74 = (lambda x, f: f(x))(
            paddle._C_ops.layer_norm(
                add_89, parameter_36, parameter_35, float("1e-05"), 2
            ),
            lambda out: out if isinstance(out, (list, tuple)) else (out, None, None),
        )
        ...
        # pd_op.add: (2x2852x1536xf32) <- (2x2852x1536xf32, 1536xf32)
        add_90 = paddle._C_ops.add(matmul_60, parameter_33)
        del parameter_33
        ...
        # pd_op.add: (2x2852x384xf32) <- (2x2852x384xf32, 384xf32)
        add_91 = paddle._C_ops.add(matmul_61, parameter_31)
        del parameter_31
        ...
        # pd_op.add: (2x1x1xf32) <- (xf32, 2x1x1xf32)
        add_92 = paddle._C_ops.add(cast_20, uniform_17)
        del uniform_17
        ...
        # pd_op.divide: (2x2852x384xf32) <- (2x2852x384xf32, xf32)
        divide_17 = paddle._C_ops.divide(add_91, cast_20)
        ...
        # pd_op.reshape: (2x62x46x384xf32) <- (2x2852x384xf32, 4xi64)
        reshape_137 = paddle._C_ops.reshape(add_93, full_int_array_60)

        # pd_op.strided_slice: (2x31x23x384xf32) <- (2x62x46x384xf32, 2xi64, 2xi64, 2xi64)
        ...
        # builtin.combine: ([2x31x23x384xf32, 2x31x23x384xf32, 2x31x23x384xf32, 2x31x23x384xf32]) <- (2x31x23x384xf32, 2x31x23x384xf32, 2x31x23x384xf32, 2x31x23x384xf32)
        combine_2 = [
            strided_slice_8,
            strided_slice_9,
            strided_slice_10,
        ...
        # pd_op.concat: (2x31x23x1536xf32) <- ([2x31x23x384xf32, 2x31x23x384xf32, 2x31x23x384xf32, 2x31x23x384xf32], 1xi32)
        concat_2 = paddle._C_ops.concat(combine_2, full_8)
        del combine_2

        # pd_op.full_int_array: (3xi64) <- ()
        full_int_array_72 = [-1, 713, 1536]

        # pd_op.reshape: (2x713x1536xf32) <- (2x31x23x1536xf32, 3xi64)
        reshape_138 = paddle._C_ops.reshape(concat_2, full_int_array_72)
        del full_int_array_72

        # pd_op.layer_norm: (2x713x1536xf32, 2x713xf32, 2x713xf32) <- (2x713x1536xf32, 1536xf32, 1536xf32)
        layer_norm_75, layer_norm_76, layer_norm_77 = (lambda x, f: f(x))(
            paddle._C_ops.layer_norm(
                reshape_138, parameter_30, parameter_29, float("1e-05"), 2
            ),
            lambda out: out if isinstance(out, (list, tuple)) else (out, None, None),
        )
        del parameter_29, parameter_30

        # pd_op.matmul: (2x713x768xf32) <- (2x713x1536xf32, 1536x768xf32)
        ...
        layer_norm_78, layer_norm_79, layer_norm_80 = (lambda x, f: f(x))(
            paddle._C_ops.layer_norm(
                add_93, parameter_27, parameter_26, float("1e-05"), 2
            ),
            lambda out: out if isinstance(out, (list, tuple)) else (out, None, None),
        )
        ...
        # pd_op.transpose: (2x384x62x46xf32) <- (2x62x46x384xf32)
        transpose_2 = paddle._C_ops.transpose(reshape_139, [0, 3, 1, 2])
        del reshape_139
        ...
        transpose_70 = paddle._C_ops.transpose(reshape_140, [0, 1, 3, 2, 4, 5])
        del reshape_140

        # pd_op.reshape: (20x7x7x1xf32) <- (1x5x4x7x7x1xf32, 4xi64)
        reshape_141 = paddle._C_ops.reshape(transpose_70, full_int_array_18)
        del full_int_array_18, transpose_70
        ...
        # pd_op.layer_norm: (2x713x768xf32, 2x713xf32, 2x713xf32) <- (2x713x768xf32, 768xf32, 768xf32)
        ...
        # pd_op.full_int_array: (4xi64) <- ()
        full_int_array_74 = [-1, 31, 23, 768]

        # pd_op.reshape: (2x31x23x768xf32) <- (2x713x768xf32, 4xi64)
        reshape_143 = paddle._C_ops.reshape(layer_norm_81, full_int_array_74)

        # pd_op.pad: (2x35x28x768xf32) <- (2x31x23x768xf32, 1xf32)
        pad_10 = paddle._C_ops.pad(reshape_143, [0, 0, 0, 4, 0, 5, 0, 0], full_4)
        ...
        transpose_71 = paddle._C_ops.transpose(reshape_144, [0, 1, 3, 2, 4, 5])
        del reshape_144
        ...
        reshape_145 = paddle._C_ops.reshape(transpose_71, full_int_array_76)
        ...
        matmul_63 = paddle._C_ops.matmul(reshape_146, parameter_23, False, False)
        del parameter_23

        # pd_op.add: (40x49x2304xf32) <- (40x49x2304xf32, 2304xf32)
        add_94 = paddle._C_ops.add(matmul_63, parameter_22)
        del parameter_22
        ...
        reshape_147 = paddle._C_ops.reshape(add_94, full_int_array_78)

        # pd_op.transpose: (3x40x24x49x32xf32) <- (40x49x3x24x32xf32)
        transpose_72 = paddle._C_ops.transpose(reshape_147, [2, 0, 3, 1, 4])
        del reshape_147

        # pd_op.slice: (40x24x49x32xf32) <- (3x40x24x49x32xf32, 1xi64, 1xi64)
        slice_40 = paddle._C_ops.slice(
            transpose_72, [0], full_int_array_27, full_int_array_20, [1], [0]
        )

        # pd_op.slice: (40x24x49x32xf32) <- (3x40x24x49x32xf32, 1xi64, 1xi64)
        slice_41 = paddle._C_ops.slice(
            transpose_72, [0], full_int_array_20, full_int_array_21, [1], [0]
        )

        # pd_op.slice: (40x24x49x32xf32) <- (3x40x24x49x32xf32, 1xi64, 1xi64)
        slice_42 = paddle._C_ops.slice(
            transpose_72, [0], full_int_array_21, full_int_array_28, [1], [0]
        )
        ...
        transpose_73 = paddle._C_ops.transpose(slice_41, [0, 1, 3, 2])
        del slice_41

        # pd_op.matmul: (40x24x49x49xf32) <- (40x24x49x32xf32, 40x24x32x49xf32)
        matmul_64 = paddle._C_ops.matmul(scale_14, transpose_73, False, False)
        ...
        transpose_74 = paddle._C_ops.transpose(reshape_148, [2, 0, 1])
        del reshape_148

        # pd_op.unsqueeze: (1x24x49x49xf32) <- (24x49x49xf32, 1xi64)
        unsqueeze_28 = paddle._C_ops.unsqueeze(transpose_74, full_int_array_27)

        # pd_op.add: (40x24x49x49xf32) <- (40x24x49x49xf32, 1x24x49x49xf32)
        add_95 = paddle._C_ops.add(matmul_64, unsqueeze_28)
        ...
        transpose_75 = paddle._C_ops.transpose(matmul_65, [0, 2, 1, 3])
        del matmul_65
        ...
        reshape_149 = paddle._C_ops.reshape(transpose_75, full_int_array_79)

        # pd_op.matmul: (40x49x768xf32) <- (40x49x768xf32, 768x768xf32)
        matmul_66 = paddle._C_ops.matmul(reshape_149, parameter_21, False, False)
        del parameter_21

        # pd_op.add: (40x49x768xf32) <- (40x49x768xf32, 768xf32)
        add_96 = paddle._C_ops.add(matmul_66, parameter_20)
        del parameter_20
        ...
        reshape_150 = paddle._C_ops.reshape(add_96, full_int_array_80)
        ...
        transpose_76 = paddle._C_ops.transpose(reshape_151, [0, 1, 3, 2, 4, 5])
        del reshape_151
        ...
        reshape_152 = paddle._C_ops.reshape(transpose_76, full_int_array_82)

        # pd_op.full_int_array: (2xi64) <- ()
        full_int_array_83 = [31, 23]

        # pd_op.assign: (2xi64) <- (2xi64)
        assign_169 = full_int_array_83

        # pd_op.slice: (2x31x23x768xf32) <- (2x35x28x768xf32, 2xi64, 2xi64)
        slice_43 = paddle._C_ops.slice(
            reshape_152, [1, 2], full_int_array_2, full_int_array_83, [1, 1], []
        )

        # pd_op.full_int_array: (3xi64) <- ()
        full_int_array_84 = [-1, 713, 768]

        # pd_op.reshape: (2x713x768xf32) <- (2x31x23x768xf32, 3xi64)
        ...
        # pd_op.add: (2x1x1xf32) <- (xf32, 2x1x1xf32)
        add_97 = paddle._C_ops.add(cast_22, uniform_18)
        del uniform_18
        ...
        # pd_op.divide: (2x713x768xf32) <- (2x713x768xf32, xf32)
        divide_18 = paddle._C_ops.divide(reshape_153, cast_22)

        # pd_op.multiply: (2x713x768xf32) <- (2x713x768xf32, 2x1x1xf32)
        multiply_22 = paddle._C_ops.multiply(divide_18, floor_18)

        # pd_op.add: (2x713x768xf32) <- (2x713x768xf32, 2x713x768xf32)
        add_98 = paddle._C_ops.add(matmul_62, multiply_22)

        # pd_op.layer_norm: (2x713x768xf32, 2x713xf32, 2x713xf32) <- (2x713x768xf32, 768xf32, 768xf32)
        layer_norm_84, layer_norm_85, layer_norm_86 = (lambda x, f: f(x))(
            paddle._C_ops.layer_norm(
                add_98, parameter_19, parameter_18, float("1e-05"), 2
            ),
            lambda out: out if isinstance(out, (list, tuple)) else (out, None, None),
        )
        del parameter_18, parameter_19

        # pd_op.matmul: (2x713x3072xf32) <- (2x713x768xf32, 768x3072xf32)
        matmul_67 = paddle._C_ops.matmul(layer_norm_84, parameter_17, False, False)
        del parameter_17

        # pd_op.add: (2x713x3072xf32) <- (2x713x3072xf32, 3072xf32)
        add_99 = paddle._C_ops.add(matmul_67, parameter_16)
        del parameter_16

        # pd_op.gelu: (2x713x3072xf32) <- (2x713x3072xf32)
        gelu_10 = paddle._C_ops.gelu(add_99, False)

        # pd_op.matmul: (2x713x768xf32) <- (2x713x3072xf32, 3072x768xf32)
        matmul_68 = paddle._C_ops.matmul(gelu_10, parameter_15, False, False)
        del parameter_15

        # pd_op.add: (2x713x768xf32) <- (2x713x768xf32, 768xf32)
        add_100 = paddle._C_ops.add(matmul_68, parameter_14)
        del parameter_14
        ...
        # pd_op.add: (2x1x1xf32) <- (xf32, 2x1x1xf32)
        add_101 = paddle._C_ops.add(cast_23, uniform_19)
        del uniform_19
        ...
        # pd_op.divide: (2x713x768xf32) <- (2x713x768xf32, xf32)
        divide_19 = paddle._C_ops.divide(add_100, cast_23)
        ...
        layer_norm_87, layer_norm_88, layer_norm_89 = (lambda x, f: f(x))(
            paddle._C_ops.layer_norm(
                add_102, parameter_13, parameter_12, float("1e-05"), 2
            ),
            lambda out: out if isinstance(out, (list, tuple)) else (out, None, None),
        )
        ...
        # pd_op.pad: (2x35x28x768xf32) <- (2x31x23x768xf32, 1xf32)
        pad_11 = paddle._C_ops.pad(reshape_154, [0, 0, 0, 4, 0, 5, 0, 0], full_4)
        ...
        # pd_op.transpose: (2x5x4x7x7x768xf32) <- (2x5x7x4x7x768xf32)
        transpose_77 = paddle._C_ops.transpose(reshape_155, [0, 1, 3, 2, 4, 5])
        del reshape_155

        # pd_op.reshape: (40x7x7x768xf32) <- (2x5x4x7x7x768xf32, 4xi64)
        reshape_156 = paddle._C_ops.reshape(transpose_77, full_int_array_76)
        ...
        # pd_op.matmul: (40x49x2304xf32) <- (40x49x768xf32, 768x2304xf32)
        matmul_69 = paddle._C_ops.matmul(reshape_157, parameter_11, False, False)
        del parameter_11

        # pd_op.add: (40x49x2304xf32) <- (40x49x2304xf32, 2304xf32)
        add_103 = paddle._C_ops.add(matmul_69, parameter_10)
        del parameter_10

        # pd_op.reshape: (40x49x3x24x32xf32) <- (40x49x2304xf32, 5xi64)
        reshape_158 = paddle._C_ops.reshape(add_103, full_int_array_78)
        del full_int_array_78

        # pd_op.transpose: (3x40x24x49x32xf32) <- (40x49x3x24x32xf32)
        transpose_78 = paddle._C_ops.transpose(reshape_158, [2, 0, 3, 1, 4])
        del reshape_158

        # pd_op.slice: (40x24x49x32xf32) <- (3x40x24x49x32xf32, 1xi64, 1xi64)
        slice_44 = paddle._C_ops.slice(
            transpose_78, [0], full_int_array_27, full_int_array_20, [1], [0]
        )

        # pd_op.slice: (40x24x49x32xf32) <- (3x40x24x49x32xf32, 1xi64, 1xi64)
        slice_45 = paddle._C_ops.slice(
            transpose_78, [0], full_int_array_20, full_int_array_21, [1], [0]
        )

        # pd_op.slice: (40x24x49x32xf32) <- (3x40x24x49x32xf32, 1xi64, 1xi64)
        slice_46 = paddle._C_ops.slice(
            transpose_78, [0], full_int_array_21, full_int_array_28, [1], [0]
        )
        ...
        # pd_op.transpose: (40x24x32x49xf32) <- (40x24x49x32xf32)
        transpose_79 = paddle._C_ops.transpose(slice_45, [0, 1, 3, 2])
        del slice_45

        # pd_op.matmul: (40x24x49x49xf32) <- (40x24x49x32xf32, 40x24x32x49xf32)
        matmul_70 = paddle._C_ops.matmul(scale_15, transpose_79, False, False)
        ...
        # pd_op.transpose: (24x49x49xf32) <- (49x49x24xf32)
        transpose_80 = paddle._C_ops.transpose(reshape_159, [2, 0, 1])
        del reshape_159

        # pd_op.unsqueeze: (1x24x49x49xf32) <- (24x49x49xf32, 1xi64)
        unsqueeze_29 = paddle._C_ops.unsqueeze(transpose_80, full_int_array_27)

        # pd_op.add: (40x24x49x49xf32) <- (40x24x49x49xf32, 1x24x49x49xf32)
        add_104 = paddle._C_ops.add(matmul_70, unsqueeze_29)

        # pd_op.full_int_array: (5xi64) <- ()
        full_int_array_85 = [-1, 20, 24, 49, 49]

        # pd_op.reshape: (2x20x24x49x49xf32) <- (40x24x49x49xf32, 5xi64)
        reshape_160 = paddle._C_ops.reshape(add_104, full_int_array_85)
        ...
        add_105 = paddle._C_ops.add(reshape_160, unsqueeze_31)

        # pd_op.full_int_array: (4xi64) <- ()
        full_int_array_86 = [-1, 24, 49, 49]

        # pd_op.reshape: (40x24x49x49xf32) <- (2x20x24x49x49xf32, 4xi64)
        reshape_161 = paddle._C_ops.reshape(add_105, full_int_array_86)
        ...
        # pd_op.transpose: (40x49x24x32xf32) <- (40x24x49x32xf32)
        transpose_81 = paddle._C_ops.transpose(matmul_71, [0, 2, 1, 3])
        del matmul_71

        # pd_op.reshape: (40x49x768xf32) <- (40x49x24x32xf32, 3xi64)
        reshape_162 = paddle._C_ops.reshape(transpose_81, full_int_array_79)
        del full_int_array_79

        # pd_op.matmul: (40x49x768xf32) <- (40x49x768xf32, 768x768xf32)
        matmul_72 = paddle._C_ops.matmul(reshape_162, parameter_9, False, False)
        del parameter_9

        # pd_op.add: (40x49x768xf32) <- (40x49x768xf32, 768xf32)
        add_106 = paddle._C_ops.add(matmul_72, parameter_8)
        del parameter_8

        # pd_op.reshape: (40x7x7x768xf32) <- (40x49x768xf32, 4xi64)
        reshape_163 = paddle._C_ops.reshape(add_106, full_int_array_80)
        ...
        # pd_op.transpose: (2x5x7x4x7x768xf32) <- (2x5x4x7x7x768xf32)
        transpose_82 = paddle._C_ops.transpose(reshape_164, [0, 1, 3, 2, 4, 5])
        del reshape_164

        # pd_op.reshape: (2x35x28x768xf32) <- (2x5x7x4x7x768xf32, 4xi64)
        reshape_165 = paddle._C_ops.reshape(transpose_82, full_int_array_82)
        ...
        # pd_op.add: (2x1x1xf32) <- (xf32, 2x1x1xf32)
        add_107 = paddle._C_ops.add(cast_24, uniform_20)
        del uniform_20
        ...
        layer_norm_90, layer_norm_91, layer_norm_92 = (lambda x, f: f(x))(
            paddle._C_ops.layer_norm(
                add_108, parameter_7, parameter_6, float("1e-05"), 2
            ),
            lambda out: out if isinstance(out, (list, tuple)) else (out, None, None),
        )
        ...
        # pd_op.add: (2x713x3072xf32) <- (2x713x3072xf32, 3072xf32)
        add_109 = paddle._C_ops.add(matmul_73, parameter_4)
        del parameter_4
        ...
        # pd_op.add: (2x713x768xf32) <- (2x713x768xf32, 768xf32)
        add_110 = paddle._C_ops.add(matmul_74, parameter_2)
        del parameter_2
        ...
        # pd_op.add: (2x1x1xf32) <- (xf32, 2x1x1xf32)
        add_111 = paddle._C_ops.add(cast_25, uniform_21)
        del uniform_21
        ...
        # pd_op.divide: (2x713x768xf32) <- (2x713x768xf32, xf32)
        divide_21 = paddle._C_ops.divide(add_110, cast_25)
        ...
        layer_norm_93, layer_norm_94, layer_norm_95 = (lambda x, f: f(x))(
            paddle._C_ops.layer_norm(
                add_112, parameter_1, parameter_0, float("1e-05"), 2
            ),
            lambda out: out if isinstance(out, (list, tuple)) else (out, None, None),
        )
        ...
        # pd_op.transpose: (2x768x31x23xf32) <- (2x31x23x768xf32)
        transpose_3 = paddle._C_ops.transpose(reshape_167, [0, 3, 1, 2])
        del (
            add_0,
            add_1,
            add_10,
