...
class GraphModule(paddle.nn.Layer):
    ...
    def forward(
        ...
        # pd_op.full_int_array: (6xi64) <- ()
        full_int_array_12 = [-1, 12, 14, 7, 7, 192]

        # pd_op.reshape: (2x12x14x7x7x192xf32) <- (336x7x7x192xf32, 6xi64)
        reshape_8 = paddle._C_ops.reshape(reshape_7, full_int_array_12)
        ...
        # pd_op.full_int_array: (4xi64) <- ()
        full_int_array_13 = [-1, 84, 98, 192]

        # pd_op.reshape: (2x84x98x192xf32) <- (2x12x7x14x7x192xf32, 4xi64)
        reshape_9 = paddle._C_ops.reshape(transpose_5, full_int_array_13)
        ...
        # pd_op.full_int_array: (2xi64) <- ()
        full_int_array_15 = [80, 96]

        # pd_op.slice: (2x80x96x192xf32) <- (2x84x98x192xf32, 2xi64, 2xi64)
        slice_3 = paddle._C_ops.slice(
            reshape_9, [1, 2], full_int_array_14, full_int_array_15, [1, 1], []
        )

        # pd_op.full_int_array: (3xi64) <- ()
        full_int_array_16 = [-1, 7680, 192]

        # pd_op.reshape: (2x7680x192xf32) <- (2x80x96x192xf32, 3xi64)
        reshape_10 = paddle._C_ops.reshape(slice_3, full_int_array_16)
