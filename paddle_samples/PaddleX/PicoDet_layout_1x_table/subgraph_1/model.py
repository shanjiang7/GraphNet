...
class GraphModule(paddle.nn.Layer):
    ...
    def forward(self, data_0, data_1, data_2, data_3, data_4):
        # pd_op.cast: (43376x11xf32) <- (43376x11xf64)
        ...
        # pd_op.greater_than: (43376x11xb) <- (43376x11xf32, xf32)
        greater_than_0 = paddle._C_ops.greater_than(cast_0, full_0)

        # pd_op.cast: (43376x11xf32) <- (43376x11xb)
        cast_1 = paddle._C_ops.cast(greater_than_0, paddle.float32)
        del greater_than_0

        # pd_op.multiply: (43376x11xf32) <- (43376x11xf32, 43376x11xf32)
        multiply_0 = paddle._C_ops.multiply(cast_0, cast_1)
        del cast_1

        # pd_op.subtract: (43376x11xf32) <- (43376x11xf32, 43376x11xf32)
        subtract_0 = paddle._C_ops.subtract(data_1, cast_0)

        # pd_op.abs: (43376x11xf32) <- (43376x11xf32)
        abs_0 = paddle._C_ops.abs(subtract_0)
        del subtract_0

        # pd_op.pow: (43376x11xf32) <- (43376x11xf32)
        ...
        # pd_op.scale: (43376x11xf32) <- (43376x11xf32, 1xf32)
        scale_0 = paddle._C_ops.scale(pow_0, full_1, float("0"), True)
        del full_1, pow_0

        # pd_op.less_equal: (43376x11xb) <- (43376x11xf32, xf32)
        ...
        # pd_op.add: (43376x11xf32) <- (43376x11xf32, 43376x11xf32)
        add_1 = paddle._C_ops.add(multiply_0, multiply_1)
        del multiply_0, multiply_1

        # pd_op.bce_loss: (43376x11xf32) <- (43376x11xf32, 43376x11xf32)
        ...
        # pd_op.sum: (43376xf32) <- (43376x11xf32, 1xi64)
        sum_0 = paddle._C_ops.sum(multiply_2, full_int_array_0, None, False)
        del multiply_2

        # pd_op.full_int_array: (2xi64) <- ()
        full_int_array_1 = [5422, 8]

        # pd_op.reshape: (5422x8xf32) <- (43376xf32, 2xi64)
        ...
        # pd_op.slice: (5422xf32) <- (5422x4xf32, 1xi64, 1xi64)
        ...
        # pd_op.subtract: (5422xf32) <- (5422xf32, 5422xf32)
        ...
        # pd_op.multiply: (5422xf32) <- (5422xf32, 5422xf32)
        ...
        # pd_op.subtract: (8xf32) <- (8xf32, 8xf32)
        ...
        # pd_op.minimum: (5422x8x2xf32) <- (5422x1x2xf32, 8x2xf32)
        minimum_0 = paddle._C_ops.minimum(reshape_2, slice_11)

        # pd_op.subtract: (5422x8x2xf32) <- (5422x8x2xf32, 5422x8x2xf32)
        ...
        # pd_op.clip: (5422x8x2xf32) <- (5422x8x2xf32, 1xf32, 1xf32)
        clip_0 = paddle._C_ops.clip(subtract_5, full_2, full_3)
        del subtract_5

        # pd_op.slice: (5422x8xf32) <- (5422x8x2xf32, 1xi64, 1xi64)
        ...
        # pd_op.maximum: (5422x8xf32) <- (5422x8xf32, 1xf32)
        maximum_2 = paddle._C_ops.maximum(subtract_6, assign_value__0)
        del subtract_6

        # pd_op.divide: (5422x8xf32) <- (5422x8xf32, 5422x8xf32)
        ...
        # pd_op.scale: (5422x8xf32) <- (5422x8xf32, 1xf32)
        ...
        # pd_op.add: (5422x8xf32) <- (5422x8xf32, 5422x8xf32)
        add_3 = paddle._C_ops.add(scale_2, scale_3)
        del scale_2, scale_3

        # pd_op.logical_not: (5422x8xb) <- (5422x8xb)
        logical_not_0 = paddle._C_ops.logical_not(data_4)
        del data_4

        # pd_op.cast: (5422x8xf32) <- (5422x8xb)
