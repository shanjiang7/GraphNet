...
class GraphModule(paddle.nn.Layer):
    ...
    def forward(
        ...
        # pd_op.pool2d: (8x576x1x1xf32) <- (8x576x22x22xf32, 2xi64)
        ...
        # pd_op.multiply: (8x576x22x22xf32) <- (8x576x22x22xf32, 8x576x1x1xf32)
        multiply_0 = paddle._C_ops.multiply(data_0, sigmoid_0)

        # pd_op.conv2d: (8x576x22x22xf32) <- (8x576x22x22xf32, 576x576x1x1xf32)
        conv2d_1 = paddle._C_ops.conv2d(
            multiply_0, parameter_51, [1, 1], [0, 0], "EXPLICIT", [1, 1], 1, "NCHW"
        )
        del parameter_51

        # pd_op.batch_norm_: (8x576x22x22xf32, 576xf32, 576xf32, 576xf32, 576xf32, -1xui8) <- (8x576x22x22xf32, 576xf32, 576xf32, 576xf32, 576xf32)
        (
            batch_norm__0,
            batch_norm__1,
            batch_norm__2,
        ...
        # pd_op.swish: (8x576x22x22xf32) <- (8x576x22x22xf32)
        swish_0 = paddle._C_ops.swish(batch_norm__0)

        # pd_op.add: (8x576x22x22xf32) <- (8x576x22x22xf32, 8x576x22x22xf32)
        add_1 = paddle._C_ops.add(swish_0, data_0)

        # pd_op.conv2d: (8x4x22x22xf32) <- (8x576x22x22xf32, 4x576x3x3xf32)
        ...
        # pd_op.add: (8x4x22x22xf32) <- (8x4x22x22xf32, 1x4x1x1xf32)
        ...
        # pd_op.conv2d: (8x68x22x22xf32) <- (8x576x22x22xf32, 68x576x3x3xf32)
        ...
        # pd_op.add: (8x68x22x22xf32) <- (8x68x22x22xf32, 1x68x1x1xf32)
        add_4 = paddle._C_ops.add(conv2d_5, reshape_9)

        # pd_op.sigmoid: (8x4x22x22xf32) <- (8x4x22x22xf32)
        sigmoid_2 = paddle._C_ops.sigmoid(add_2)
        del add_2

        # pd_op.flatten: (8x4x484xf32) <- (8x4x22x22xf32)
        flatten_0 = paddle._C_ops.flatten(sigmoid_2, 2, 3)

        # pd_op.transpose: (8x484x4xf32) <- (8x4x484xf32)
        transpose_0 = paddle._C_ops.transpose(flatten_0, [0, 2, 1])
        del flatten_0

        # pd_op.flatten: (8x68x484xf32) <- (8x68x22x22xf32)
        flatten_1 = paddle._C_ops.flatten(add_4, 2, 3)

        # pd_op.transpose: (8x484x68xf32) <- (8x68x484xf32)
        transpose_1 = paddle._C_ops.transpose(flatten_1, [0, 2, 1])
        del flatten_1

        # pd_op.pool2d: (8x288x1x1xf32) <- (8x288x44x44xf32, 2xi64)
        ...
        # pd_op.multiply: (8x288x44x44xf32) <- (8x288x44x44xf32, 8x288x1x1xf32)
        multiply_2 = paddle._C_ops.multiply(data_1, sigmoid_3)

        # pd_op.conv2d: (8x288x44x44xf32) <- (8x288x44x44xf32, 288x288x1x1xf32)
        conv2d_7 = paddle._C_ops.conv2d(
            multiply_2, parameter_33, [1, 1], [0, 0], "EXPLICIT", [1, 1], 1, "NCHW"
        )
        del parameter_33

        # pd_op.batch_norm_: (8x288x44x44xf32, 288xf32, 288xf32, 288xf32, 288xf32, -1xui8) <- (8x288x44x44xf32, 288xf32, 288xf32, 288xf32, 288xf32)
        (
            batch_norm__12,
            batch_norm__13,
            batch_norm__14,
        ...
        # pd_op.swish: (8x288x44x44xf32) <- (8x288x44x44xf32)
        swish_2 = paddle._C_ops.swish(batch_norm__12)

        # pd_op.add: (8x288x44x44xf32) <- (8x288x44x44xf32, 8x288x44x44xf32)
        add_6 = paddle._C_ops.add(swish_2, data_1)

        # pd_op.conv2d: (8x4x44x44xf32) <- (8x288x44x44xf32, 4x288x3x3xf32)
        ...
        # pd_op.add: (8x4x44x44xf32) <- (8x4x44x44xf32, 1x4x1x1xf32)
        ...
        # pd_op.conv2d: (8x68x44x44xf32) <- (8x288x44x44xf32, 68x288x3x3xf32)
        ...
        # pd_op.add: (8x68x44x44xf32) <- (8x68x44x44xf32, 1x68x1x1xf32)
        add_9 = paddle._C_ops.add(conv2d_11, reshape_13)

        # pd_op.sigmoid: (8x4x44x44xf32) <- (8x4x44x44xf32)
        sigmoid_5 = paddle._C_ops.sigmoid(add_7)
        del add_7

        # pd_op.flatten: (8x4x1936xf32) <- (8x4x44x44xf32)
        flatten_2 = paddle._C_ops.flatten(sigmoid_5, 2, 3)

        # pd_op.transpose: (8x1936x4xf32) <- (8x4x1936xf32)
        transpose_2 = paddle._C_ops.transpose(flatten_2, [0, 2, 1])
        del flatten_2

        # pd_op.flatten: (8x68x1936xf32) <- (8x68x44x44xf32)
        flatten_3 = paddle._C_ops.flatten(add_9, 2, 3)

        # pd_op.transpose: (8x1936x68xf32) <- (8x68x1936xf32)
        transpose_3 = paddle._C_ops.transpose(flatten_3, [0, 2, 1])
        del flatten_3

        # pd_op.pool2d: (8x144x1x1xf32) <- (8x144x88x88xf32, 2xi64)
        ...
        # pd_op.multiply: (8x144x88x88xf32) <- (8x144x88x88xf32, 8x144x1x1xf32)
        multiply_4 = paddle._C_ops.multiply(data_2, sigmoid_6)

        # pd_op.conv2d: (8x144x88x88xf32) <- (8x144x88x88xf32, 144x144x1x1xf32)
        conv2d_13 = paddle._C_ops.conv2d(
            multiply_4, parameter_15, [1, 1], [0, 0], "EXPLICIT", [1, 1], 1, "NCHW"
        )
        del parameter_15

        # pd_op.batch_norm_: (8x144x88x88xf32, 144xf32, 144xf32, 144xf32, 144xf32, -1xui8) <- (8x144x88x88xf32, 144xf32, 144xf32, 144xf32, 144xf32)
        (
            batch_norm__24,
            batch_norm__25,
            batch_norm__26,
        ...
        # pd_op.swish: (8x144x88x88xf32) <- (8x144x88x88xf32)
        swish_4 = paddle._C_ops.swish(batch_norm__24)

        # pd_op.add: (8x144x88x88xf32) <- (8x144x88x88xf32, 8x144x88x88xf32)
        add_11 = paddle._C_ops.add(swish_4, data_2)

        # pd_op.conv2d: (8x4x88x88xf32) <- (8x144x88x88xf32, 4x144x3x3xf32)
        ...
        # pd_op.add: (8x4x88x88xf32) <- (8x4x88x88xf32, 1x4x1x1xf32)
        ...
        # pd_op.conv2d: (8x68x88x88xf32) <- (8x144x88x88xf32, 68x144x3x3xf32)
        ...
        # pd_op.add: (8x68x88x88xf32) <- (8x68x88x88xf32, 1x68x1x1xf32)
        add_14 = paddle._C_ops.add(conv2d_17, reshape_17)

        # pd_op.sigmoid: (8x4x88x88xf32) <- (8x4x88x88xf32)
        sigmoid_8 = paddle._C_ops.sigmoid(add_12)
        del add_12

        # pd_op.flatten: (8x4x7744xf32) <- (8x4x88x88xf32)
        flatten_4 = paddle._C_ops.flatten(sigmoid_8, 2, 3)

        # pd_op.transpose: (8x7744x4xf32) <- (8x4x7744xf32)
        transpose_4 = paddle._C_ops.transpose(flatten_4, [0, 2, 1])
        del flatten_4

        # pd_op.flatten: (8x68x7744xf32) <- (8x68x88x88xf32)
        flatten_5 = paddle._C_ops.flatten(add_14, 2, 3)

        # pd_op.transpose: (8x7744x68xf32) <- (8x68x7744xf32)
