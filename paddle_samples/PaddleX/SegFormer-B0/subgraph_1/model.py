...
class GraphModule(paddle.nn.Layer):
    ...
    def forward(
        ...
        # pd_op.conv2d: (-1x32x-1x-1xf32) <- (-1x3x-1x-1xf32, 32x3x7x7xf32)
        ...
        # pd_op.shape64: (4xi64) <- (-1x32x-1x-1xf32)
        ...
        # pd_op.shape64: (3xi64) <- (-1x-1x32xf32)
        ...
        # pd_op.matmul: (-1x-1x32xf32) <- (-1x-1x32xf32, 32x32xf32)
        matmul_0 = paddle._C_ops.matmul(layer_norm_3, parameter_184, False, False)
        del parameter_184

        # pd_op.add: (-1x-1x32xf32) <- (-1x-1x32xf32, 32xf32)
        ...
        # pd_op.reshape: (-1x-1x1x32xf32) <- (-1x-1x32xf32, 4xi64)
        reshape_1 = paddle._C_ops.reshape(add_1, stack_0)
        del add_1, stack_0

        # pd_op.transpose: (-1x1x-1x32xf32) <- (-1x-1x1x32xf32)
        transpose_1 = paddle._C_ops.transpose(reshape_1, [0, 2, 1, 3])
        del reshape_1

        # pd_op.transpose: (-1x32x-1xf32) <- (-1x-1x32xf32)
        ...
        # pd_op.reshape: (-1x32x-1x-1xf32) <- (-1x32x-1xf32, 4xi64)
        ...
        # pd_op.reshape: (-1x32x-1xf32) <- (-1x32x-1x-1xf32, 3xi64)
        ...
        # pd_op.reshape: (-1x-1x2x1x32xf32) <- (-1x-1x64xf32, 5xi64)
        reshape_5 = paddle._C_ops.reshape(add_3, stack_3)
        del add_3, stack_3

        # pd_op.transpose: (2x-1x1x-1x32xf32) <- (-1x-1x2x1x32xf32)
        transpose_4 = paddle._C_ops.transpose(reshape_5, [2, 0, 3, 1, 4])
        del reshape_5

        # pd_op.slice: (-1x1x-1x32xf32) <- (2x-1x1x-1x32xf32, 1xi64, 1xi64)
        ...
        # pd_op.matmul: (-1x1x-1x-1xf32) <- (-1x1x-1x32xf32, -1x1x32x-1xf32)
        ...
        # pd_op.matmul: (-1x1x-1x32xf32) <- (-1x1x-1x-1xf32, -1x1x-1x32xf32)
        matmul_3 = paddle._C_ops.matmul(softmax_0, slice_6, False, False)
        del slice_6, softmax_0

        # pd_op.transpose: (-1x-1x1x32xf32) <- (-1x1x-1x32xf32)
        ...
        # pd_op.reshape: (-1x-1x32xf32) <- (-1x-1x1x32xf32, 3xi64)
        ...
        # pd_op.add: (-1x-1x32xf32) <- (-1x-1x32xf32, -1x-1x32xf32)
        ...
        # pd_op.matmul: (-1x-1x128xf32) <- (-1x-1x32xf32, 32x128xf32)
        ...
        # pd_op.gelu: (-1x-1x128xf32) <- (-1x-1x128xf32)
        gelu_0 = paddle._C_ops.gelu(transpose_8, False)
        del transpose_8

        # pd_op.matmul: (-1x-1x32xf32) <- (-1x-1x128xf32, 128x32xf32)
        ...
        # pd_op.reshape: (-1x-1x-1x32xf32) <- (-1x-1x32xf32, 4xi64)
        reshape_17 = paddle._C_ops.reshape(layer_norm_21, stack_12)
        del layer_norm_21, stack_12

        # pd_op.transpose: (-1x32x-1x-1xf32) <- (-1x-1x-1x32xf32)
        ...
        # pd_op.reshape: (-1x-1x2x32xf32) <- (-1x-1x64xf32, 4xi64)
        reshape_19 = paddle._C_ops.reshape(add_20, stack_13)
        del add_20, stack_13

        # pd_op.transpose: (-1x2x-1x32xf32) <- (-1x-1x2x32xf32)
        ...
        # pd_op.conv2d: (-1x64x-1x-1xf32) <- (-1x64x-1x-1xf32, 64x64x4x4xf32)
        ...
        # pd_op.reshape: (-1x-1x2x2x32xf32) <- (-1x-1x128xf32, 5xi64)
        reshape_23 = paddle._C_ops.reshape(add_22, stack_16)
        del add_22, stack_16

        # pd_op.transpose: (2x-1x2x-1x32xf32) <- (-1x-1x2x2x32xf32)
        transpose_22 = paddle._C_ops.transpose(reshape_23, [2, 0, 3, 1, 4])
        del reshape_23

        # pd_op.slice: (-1x2x-1x32xf32) <- (2x-1x2x-1x32xf32, 1xi64, 1xi64)
        ...
        # pd_op.transpose: (-1x2x32x-1xf32) <- (-1x2x-1x32xf32)
        transpose_23 = paddle._C_ops.transpose(slice_20, [0, 1, 3, 2])
        del slice_20

        # pd_op.matmul: (-1x2x-1x-1xf32) <- (-1x2x-1x32xf32, -1x2x32x-1xf32)
        ...
        # pd_op.matmul: (-1x2x-1x32xf32) <- (-1x2x-1x-1xf32, -1x2x-1x32xf32)
        matmul_17 = paddle._C_ops.matmul(softmax_2, slice_21, False, False)
        del slice_21, softmax_2

        # pd_op.transpose: (-1x-1x2x32xf32) <- (-1x2x-1x32xf32)
        ...
        # pd_op.reshape: (-1x-1x64xf32) <- (-1x-1x2x32xf32, 3xi64)
        ...
        # pd_op.flatten: (-1x160x-1xf32) <- (-1x160x-1x-1xf32)
        ...
        # pd_op.shape64: (3xi64) <- (-1x-1x160xf32)
        ...
        # pd_op.matmul: (-1x-1x160xf32) <- (-1x-1x160xf32, 160x160xf32)
        ...
        # pd_op.reshape: (-1x-1x5x32xf32) <- (-1x-1x160xf32, 4xi64)
        reshape_37 = paddle._C_ops.reshape(add_39, stack_26)
        del add_39, stack_26

        # pd_op.transpose: (-1x5x-1x32xf32) <- (-1x-1x5x32xf32)
        transpose_37 = paddle._C_ops.transpose(reshape_37, [0, 2, 1, 3])
        del reshape_37

        # pd_op.transpose: (-1x160x-1xf32) <- (-1x-1x160xf32)
        transpose_38 = paddle._C_ops.transpose(layer_norm_51, [0, 2, 1])
        del layer_norm_51

        # pd_op.full: (xi64) <- ()
        full_9 = paddle._C_ops.full(
            [], float("160"), paddle.int64, paddle.core.CPUPlace()
        )
        ...
        # pd_op.reshape: (-1x160x-1x-1xf32) <- (-1x160x-1xf32, 4xi64)
        reshape_38 = paddle._C_ops.reshape(transpose_38, stack_27)
        del stack_27, transpose_38

        # pd_op.conv2d: (-1x160x-1x-1xf32) <- (-1x160x-1x-1xf32, 160x160x2x2xf32)
        ...
        # pd_op.reshape: (-1x160x-1xf32) <- (-1x160x-1x-1xf32, 3xi64)
        ...
        # pd_op.matmul: (-1x-1x320xf32) <- (-1x-1x160xf32, 160x320xf32)
        ...
        # pd_op.reshape: (-1x-1x2x5x32xf32) <- (-1x-1x320xf32, 5xi64)
        reshape_41 = paddle._C_ops.reshape(add_41, stack_29)
        del add_41, stack_29

        # pd_op.transpose: (2x-1x5x-1x32xf32) <- (-1x-1x2x5x32xf32)
        transpose_40 = paddle._C_ops.transpose(reshape_41, [2, 0, 3, 1, 4])
        del reshape_41

        # pd_op.slice: (-1x5x-1x32xf32) <- (2x-1x5x-1x32xf32, 1xi64, 1xi64)
        ...
        # pd_op.transpose: (-1x5x32x-1xf32) <- (-1x5x-1x32xf32)
        transpose_41 = paddle._C_ops.transpose(slice_35, [0, 1, 3, 2])
        del slice_35

        # pd_op.matmul: (-1x5x-1x-1xf32) <- (-1x5x-1x32xf32, -1x5x32x-1xf32)
        ...
        # pd_op.matmul: (-1x5x-1x32xf32) <- (-1x5x-1x-1xf32, -1x5x-1x32xf32)
        matmul_31 = paddle._C_ops.matmul(softmax_4, slice_36, False, False)
        del slice_36, softmax_4

        # pd_op.transpose: (-1x-1x5x32xf32) <- (-1x5x-1x32xf32)
        ...
        # pd_op.reshape: (-1x-1x160xf32) <- (-1x-1x5x32xf32, 3xi64)
        ...
        # pd_op.add: (-1x-1x160xf32) <- (-1x-1x160xf32, -1x-1x160xf32)
        ...
        # pd_op.matmul: (-1x-1x640xf32) <- (-1x-1x160xf32, 160x640xf32)
        ...
        # pd_op.shape64: (3xi64) <- (-1x-1x640xf32)
        ...
        # pd_op.transpose: (-1x640x-1xf32) <- (-1x-1x640xf32)
        transpose_43 = paddle._C_ops.transpose(add_44, [0, 2, 1])
        del add_44

        # pd_op.full: (xi64) <- ()
        full_10 = paddle._C_ops.full(
            [], float("640"), paddle.int64, paddle.core.CPUPlace()
        )
        ...
        # pd_op.reshape: (-1x640x-1x-1xf32) <- (-1x640x-1xf32, 4xi64)
        ...
        # pd_op.reshape: (-1x-1x-1x160xf32) <- (-1x-1x160xf32, 4xi64)
        reshape_53 = paddle._C_ops.reshape(layer_norm_69, stack_38)
        del layer_norm_69, stack_38

        # pd_op.transpose: (-1x160x-1x-1xf32) <- (-1x-1x-1x160xf32)
        transpose_53 = paddle._C_ops.transpose(reshape_53, [0, 3, 1, 2])
        del reshape_53

        # pd_op.conv2d: (-1x256x-1x-1xf32) <- (-1x160x-1x-1xf32, 256x160x3x3xf32)
        ...
        # pd_op.reshape: (-1x-1x2x8x32xf32) <- (-1x-1x512xf32, 5xi64)
        ...
        # pd_op.flatten: (-1x1024x-1xf32) <- (-1x1024x-1x-1xf32)
        flatten_10 = paddle._C_ops.flatten(add_63, 2, 3)
        del add_63

        # pd_op.transpose: (-1x-1x1024xf32) <- (-1x1024x-1xf32)
        ...
        # pd_op.reshape: (-1x-1x-1x256xf32) <- (-1x-1x256xf32, 4xi64)
        reshape_65 = paddle._C_ops.reshape(layer_norm_87, stack_47)
        del layer_norm_87, stack_47

        # pd_op.transpose: (-1x256x-1x-1xf32) <- (-1x-1x-1x256xf32)
        ...
        # pd_op.matmul: (-1x-1x256xf32) <- (-1x-1x160xf32, 160x256xf32)
        ...
        # pd_op.matmul: (-1x-1x256xf32) <- (-1x-1x32xf32, 32x256xf32)
