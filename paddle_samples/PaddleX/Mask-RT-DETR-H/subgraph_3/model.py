...
class GraphModule(paddle.nn.Layer):
    ...
    def forward(
        ...
        # pd_op.add: (1x441x512xf32) <- (1x441x512xf32, 1x441x512xf32)
        ...
        # pd_op.matmul: (1x441x512xf32) <- (1x441x512xf32, 512x512xf32)
        matmul_0 = paddle._C_ops.matmul(add_0, slice_0, False, False)

        # pd_op.add: (1x441x512xf32) <- (1x441x512xf32, 512xf32)
        add_1 = paddle._C_ops.add(matmul_0, slice_1)

        # pd_op.full_int_array: (4xi64) <- ()
        full_int_array_2 = [0, 0, 8, 64]

        # pd_op.reshape: (1x441x8x64xf32) <- (1x441x512xf32, 4xi64)
        reshape_0 = paddle._C_ops.reshape(add_1, full_int_array_2)

        # pd_op.transpose: (1x8x441x64xf32) <- (1x441x8x64xf32)
        ...
        # pd_op.matmul: (1x8x441x441xf32) <- (1x8x441x64xf32, 1x8x441x64xf32)
        ...
        # pd_op.scale: (1x8x441x441xf32) <- (1x8x441x441xf32, 1xf32)
        scale_0 = paddle._C_ops.scale(matmul_3, full_0, float("0"), True)
        del matmul_3

        # pd_op.softmax: (1x8x441x441xf32) <- (1x8x441x441xf32)
        softmax_0 = paddle._C_ops.softmax(scale_0, -1)
        del scale_0

        # pd_op.matmul: (1x8x441x64xf32) <- (1x8x441x441xf32, 1x8x441x64xf32)
        matmul_4 = paddle._C_ops.matmul(softmax_0, transpose_2, False, False)

        # pd_op.transpose: (1x441x8x64xf32) <- (1x8x441x64xf32)
        transpose_3 = paddle._C_ops.transpose(matmul_4, [0, 2, 1, 3])
        del matmul_4

        # pd_op.full_int_array: (3xi64) <- ()
        full_int_array_5 = [0, 0, 512]

        # pd_op.reshape: (1x441x512xf32) <- (1x441x8x64xf32, 3xi64)
        ...
        # pd_op.layer_norm: (1x441x512xf32, 1x441xf32, 1x441xf32) <- (1x441x512xf32, 512xf32, 512xf32)
        ...
        # pd_op.matmul: (1x441x2048xf32) <- (1x441x512xf32, 512x2048xf32)
        matmul_6 = paddle._C_ops.matmul(layer_norm_1, parameter_15, False, False)
        del parameter_15

        # pd_op.add: (1x441x2048xf32) <- (1x441x2048xf32, 2048xf32)
        add_6 = paddle._C_ops.add(matmul_6, parameter_14)
        del parameter_14

        # pd_op.gelu: (1x441x2048xf32) <- (1x441x2048xf32)
        gelu_0 = paddle._C_ops.gelu(add_6, False)

        # pd_op.matmul: (1x441x512xf32) <- (1x441x2048xf32, 2048x512xf32)
        ...
        # pd_op.matmul: (1x441x512xf32) <- (1x441x512xf32, 512x512xf32)
        matmul_8 = paddle._C_ops.matmul(add_9, slice_6, False, False)

        # pd_op.add: (1x441x512xf32) <- (1x441x512xf32, 512xf32)
        add_10 = paddle._C_ops.add(matmul_8, slice_7)
        ...
        # pd_op.matmul: (1x441x512xf32) <- (1x441x512xf32, 512x512xf32)
        matmul_9 = paddle._C_ops.matmul(add_9, slice_8, False, False)

        # pd_op.add: (1x441x512xf32) <- (1x441x512xf32, 512xf32)
        add_11 = paddle._C_ops.add(matmul_9, slice_9)
        ...
        # pd_op.matmul: (1x441x512xf32) <- (1x441x512xf32, 512x512xf32)
        matmul_10 = paddle._C_ops.matmul(layer_norm_4, slice_10, False, False)

        # pd_op.add: (1x441x512xf32) <- (1x441x512xf32, 512xf32)
        add_12 = paddle._C_ops.add(matmul_10, slice_11)
