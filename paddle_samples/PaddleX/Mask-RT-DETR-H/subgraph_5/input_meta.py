class Program_weight_tensor_data_0:
    name = "data_0"
    original_name = "param_20"
    shape = [2, 256]
    dtype = "float32"
    min_val = float("-3.40036")
    max_val = float("3.31069")
    mean = float("0.0689429")
    std = float("1.04544")
    data = None


class Program_weight_tensor_data_1:
    name = "data_1"
    original_name = "param_21"
    shape = [256, 768]
    dtype = "float32"
    min_val = float("-0.629611")
    max_val = float("0.740622")
    mean = float("0.000378865")
    std = float("0.11709")
    data = None


class Program_weight_tensor_data_2:
    name = "data_2"
    original_name = "param_22"
    shape = [768]
    dtype = "float32"
    min_val = float("-0.745722")
    max_val = float("0.705338")
    mean = float("-2.9425e-05")
    std = float("0.155185")
    data = None


class Program_weight_tensor_data_3:
    name = "data_3"
    original_name = "param_23"
    shape = [256, 768]
    dtype = "float32"
    min_val = float("-1.12133")
    max_val = float("1.54205")
    mean = float("4.84274e-05")
    std = float("0.104564")
    data = None


class Program_weight_tensor_data_4:
    name = "data_4"
    original_name = "param_24"
    shape = [768]
    dtype = "float32"
    min_val = float("-0.300444")
    max_val = float("0.371003")
    mean = float("0.00208138")
    std = float("0.0752105")
    data = None


class Program_weight_tensor_data_5:
    name = "data_5"
    original_name = "param_25"
    shape = [256, 768]
    dtype = "float32"
    min_val = float("-1.52103")
    max_val = float("1.2401")
    mean = float("3.17146e-05")
    std = float("0.11153")
    data = None


class Program_weight_tensor_data_6:
    name = "data_6"
    original_name = "param_26"
    shape = [768]
    dtype = "float32"
    min_val = float("-0.355645")
    max_val = float("0.357498")
    mean = float("-0.00218777")
    std = float("0.0712676")
    data = None


class Program_weight_tensor_data_7:
    name = "data_7"
    original_name = "param_27"
    shape = [256, 768]
    dtype = "float32"
    min_val = float("-0.935949")
    max_val = float("0.751876")
    mean = float("0.000282368")
    std = float("0.109812")
    data = None


class Program_weight_tensor_data_8:
    name = "data_8"
    original_name = "param_28"
    shape = [768]
    dtype = "float32"
    min_val = float("-0.318404")
    max_val = float("0.356069")
    mean = float("0.0042011")
    std = float("0.0676694")
    data = None


class Program_weight_tensor_data_9:
    name = "data_9"
    original_name = "param_29"
    shape = [256, 768]
    dtype = "float32"
    min_val = float("-0.702972")
    max_val = float("0.666464")
    mean = float("0.00011593")
    std = float("0.105487")
    data = None


class Program_weight_tensor_data_10:
    name = "data_10"
    original_name = "param_30"
    shape = [768]
    dtype = "float32"
    min_val = float("-0.198399")
    max_val = float("0.286041")
    mean = float("0.000419409")
    std = float("0.0477778")
    data = None


class Program_weight_tensor_data_11:
    name = "data_11"
    original_name = "param_31"
    shape = [256, 768]
    dtype = "float32"
    min_val = float("-0.51271")
    max_val = float("0.529931")
    mean = float("-6.07625e-06")
    std = float("0.0945711")
    data = None


class Program_weight_tensor_data_12:
    name = "data_12"
    original_name = "param_32"
    shape = [768]
    dtype = "float32"
    min_val = float("-0.149108")
    max_val = float("0.164866")
    mean = float("-4.50971e-05")
    std = float("0.0334905")
    data = None


class Program_weight_tensor_data_13:
    name = "data_13"
    original_name = "var_2723"
    shape = [1, 512, 84, 84]
    dtype = "float32"
    min_val = float("-0.556929")
    max_val = float("61.584")
    mean = float("0.381581")
    std = float("1.02096")
    data = None


class Program_weight_tensor_data_14:
    name = "data_14"
    original_name = "var_2724"
    shape = [1, 512, 42, 42]
    dtype = "float32"
    min_val = float("-0.556929")
    max_val = float("97.1385")
    mean = float("0.421437")
    std = float("1.25604")
    data = None


class Program_weight_tensor_data_15:
    name = "data_15"
    original_name = "var_2725"
    shape = [1, 512, 21, 21]
    dtype = "float32"
    min_val = float("-0.556929")
    max_val = float("88.8719")
    mean = float("0.375246")
    std = float("1.47401")
    data = None


class Program_weight_tensor_data_16:
    name = "data_16"
    original_name = "var_2726"
    shape = [1, 128, 168, 168]
    dtype = "float32"
    min_val = float("-21.5963")
    max_val = float("19.5051")
    mean = float("-0.0307551")
    std = float("1.15197")
    data = None


class Program_weight_tensor_data_17:
    name = "data_17"
    original_name = "var_2729"
    shape = [1, 1]
    dtype = "int32"
    data = [0]


class Program_weight_tensor_data_18:
    name = "data_18"
    original_name = "var_2730"
    shape = [1, 4]
    dtype = "float32"
    data = [0.376923, 0.5, 0.728846, 1.0]
