...
class GraphModule(paddle.nn.Layer):
    ...
    def forward(
        ...
        # pd_op.scale: (136x136xf32) <- (136x136xf32, 1xf32)
        ...
        # builtin.combine: ([136x136xf32, 136x136xf32, 136x136xf32, 136x136xf32]) <- (136x136xf32, 136x136xf32, 136x136xf32, 136x136xf32)
        combine_7 = [scale_14, scale_15, scale_16, scale_17]
        del scale_14, scale_15, scale_16, scale_17

        # pd_op.stack: (136x136x4xf32) <- ([136x136xf32, 136x136xf32, 136x136xf32, 136x136xf32])
        ...
        # pd_op.reshape: (18496x4xf32) <- (136x136x4xf32, 2xi64)
        ...
        # builtin.combine: ([1156x4xf32, 4624x4xf32, 18496x4xf32]) <- (1156x4xf32, 4624x4xf32, 18496x4xf32)
        combine_9 = [reshape_0, reshape_2, reshape_4]

        # pd_op.concat: (24276x4xf32) <- ([1156x4xf32, 4624x4xf32, 18496x4xf32], 1xi32)
        concat_2 = paddle._C_ops.concat(combine_9, full_12)
        del combine_9

        # builtin.combine: ([1156x2xf32, 4624x2xf32, 18496x2xf32]) <- (1156x2xf32, 4624x2xf32, 18496x2xf32)
        combine_10 = [reshape_1, reshape_3, reshape_5]
        del reshape_1, reshape_3, reshape_5

        # pd_op.concat: (24276x2xf32) <- ([1156x2xf32, 4624x2xf32, 18496x2xf32], 1xi32)
        concat_3 = paddle._C_ops.concat(combine_10, full_12)
        del combine_10

        # builtin.combine: ([1156x1xf32, 4624x1xf32, 18496x1xf32]) <- (1156x1xf32, 4624x1xf32, 18496x1xf32)
        combine_11 = [full_5, full_8, full_11]
        del full_11, full_5, full_8

        # pd_op.concat: (24276x1xf32) <- ([1156x1xf32, 4624x1xf32, 18496x1xf32], 1xi32)
        ...
        # pd_op.pool2d: (1x768x1x1xf32) <- (1x768x34x34xf32, 2xi64)
        ...
        # pd_op.add: (1x768x34x34xf32) <- (1x768x34x34xf32, 1x768x34x34xf32)
        add_1 = paddle._C_ops.add(swish_0, data_0)

        # pd_op.conv2d: (1x10x34x34xf32) <- (1x768x34x34xf32, 10x768x3x3xf32)
        ...
        # pd_op.add: (1x10x34x34xf32) <- (1x10x34x34xf32, 1x10x1x1xf32)
        ...
        # pd_op.conv2d: (1x88x34x34xf32) <- (1x768x34x34xf32, 88x768x3x3xf32)
        ...
        # pd_op.add: (1x88x34x34xf32) <- (1x88x34x34xf32, 1x88x1x1xf32)
        add_4 = paddle._C_ops.add(conv2d_5, reshape_9)

        # pd_op.sigmoid: (1x10x34x34xf32) <- (1x10x34x34xf32)
        sigmoid_2 = paddle._C_ops.sigmoid(add_2)
        del add_2

        # pd_op.flatten: (1x10x1156xf32) <- (1x10x34x34xf32)
        flatten_0 = paddle._C_ops.flatten(sigmoid_2, 2, 3)

        # pd_op.transpose: (1x1156x10xf32) <- (1x10x1156xf32)
        transpose_0 = paddle._C_ops.transpose(flatten_0, [0, 2, 1])
        del flatten_0

        # pd_op.flatten: (1x88x1156xf32) <- (1x88x34x34xf32)
        flatten_1 = paddle._C_ops.flatten(add_4, 2, 3)

        # pd_op.transpose: (1x1156x88xf32) <- (1x88x1156xf32)
        transpose_1 = paddle._C_ops.transpose(flatten_1, [0, 2, 1])
        del flatten_1

        # pd_op.pool2d: (1x384x1x1xf32) <- (1x384x68x68xf32, 2xi64)
        ...
        # pd_op.add: (1x384x68x68xf32) <- (1x384x68x68xf32, 1x384x68x68xf32)
        add_6 = paddle._C_ops.add(swish_2, data_1)

        # pd_op.conv2d: (1x10x68x68xf32) <- (1x384x68x68xf32, 10x384x3x3xf32)
        ...
        # pd_op.add: (1x10x68x68xf32) <- (1x10x68x68xf32, 1x10x1x1xf32)
        ...
        # pd_op.conv2d: (1x88x68x68xf32) <- (1x384x68x68xf32, 88x384x3x3xf32)
        ...
        # pd_op.add: (1x88x68x68xf32) <- (1x88x68x68xf32, 1x88x1x1xf32)
        add_9 = paddle._C_ops.add(conv2d_11, reshape_13)

        # pd_op.sigmoid: (1x10x68x68xf32) <- (1x10x68x68xf32)
        sigmoid_5 = paddle._C_ops.sigmoid(add_7)
        del add_7

        # pd_op.flatten: (1x10x4624xf32) <- (1x10x68x68xf32)
        flatten_2 = paddle._C_ops.flatten(sigmoid_5, 2, 3)

        # pd_op.transpose: (1x4624x10xf32) <- (1x10x4624xf32)
        transpose_2 = paddle._C_ops.transpose(flatten_2, [0, 2, 1])
        del flatten_2

        # pd_op.flatten: (1x88x4624xf32) <- (1x88x68x68xf32)
        flatten_3 = paddle._C_ops.flatten(add_9, 2, 3)

        # pd_op.transpose: (1x4624x88xf32) <- (1x88x4624xf32)
        transpose_3 = paddle._C_ops.transpose(flatten_3, [0, 2, 1])
        del flatten_3

        # pd_op.pool2d: (1x192x1x1xf32) <- (1x192x136x136xf32, 2xi64)
        ...
        # pd_op.add: (1x192x136x136xf32) <- (1x192x136x136xf32, 1x192x136x136xf32)
        add_11 = paddle._C_ops.add(swish_4, data_2)

        # pd_op.conv2d: (1x10x136x136xf32) <- (1x192x136x136xf32, 10x192x3x3xf32)
        ...
        # pd_op.add: (1x10x136x136xf32) <- (1x10x136x136xf32, 1x10x1x1xf32)
        ...
        # pd_op.conv2d: (1x88x136x136xf32) <- (1x192x136x136xf32, 88x192x3x3xf32)
        ...
        # pd_op.add: (1x88x136x136xf32) <- (1x88x136x136xf32, 1x88x1x1xf32)
        add_14 = paddle._C_ops.add(conv2d_17, reshape_17)

        # pd_op.sigmoid: (1x10x136x136xf32) <- (1x10x136x136xf32)
        sigmoid_8 = paddle._C_ops.sigmoid(add_12)
        del add_12

        # pd_op.flatten: (1x10x18496xf32) <- (1x10x136x136xf32)
        flatten_4 = paddle._C_ops.flatten(sigmoid_8, 2, 3)

        # pd_op.transpose: (1x18496x10xf32) <- (1x10x18496xf32)
        transpose_4 = paddle._C_ops.transpose(flatten_4, [0, 2, 1])
        del flatten_4

        # pd_op.flatten: (1x88x18496xf32) <- (1x88x136x136xf32)
        flatten_5 = paddle._C_ops.flatten(add_14, 2, 3)

        # pd_op.transpose: (1x18496x88xf32) <- (1x88x18496xf32)
        ...
        # builtin.combine: ([1x1156x10xf32, 1x4624x10xf32, 1x18496x10xf32]) <- (1x1156x10xf32, 1x4624x10xf32, 1x18496x10xf32)
        combine_12 = [transpose_0, transpose_2, transpose_4]

        # pd_op.concat: (1x24276x10xf32) <- ([1x1156x10xf32, 1x4624x10xf32, 1x18496x10xf32], 1xi32)
        concat_0 = paddle._C_ops.concat(combine_12, full_13)
        del combine_12

        # builtin.combine: ([1x1156x88xf32, 1x4624x88xf32, 1x18496x88xf32]) <- (1x1156x88xf32, 1x4624x88xf32, 1x18496x88xf32)
        combine_13 = [transpose_1, transpose_3, transpose_5]

        # pd_op.concat: (1x24276x88xf32) <- ([1x1156x88xf32, 1x4624x88xf32, 1x18496x88xf32], 1xi32)
