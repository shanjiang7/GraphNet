...
class GraphModule(paddle.nn.Layer):
    ...
    def forward(
        ...
        # builtin.combine: ([13xf32, 13xf32]) <- (13xf32, 13xf32)
        combine_0 = [scale_1, scale_1]
        del scale_1

        # pd_op.meshgrid: ([13x13xf32, 13x13xf32]) <- ([13xf32, 13xf32])
        meshgrid_0 = paddle._C_ops.meshgrid(combine_0)
        del combine_0

        # builtin.split: (13x13xf32, 13x13xf32) <- ([13x13xf32, 13x13xf32])
        (
            split_0,
            split_1,
        ) = meshgrid_0
        ...
        # pd_op.scale: (13x13xf32) <- (13x13xf32, 1xf32)
        ...
        # builtin.combine: ([13x13xf32, 13x13xf32, 13x13xf32, 13x13xf32]) <- (13x13xf32, 13x13xf32, 13x13xf32, 13x13xf32)
        combine_1 = [scale_2, scale_3, scale_4, scale_5]
        del scale_2, scale_3, scale_4, scale_5

        # pd_op.stack: (13x13x4xf32) <- ([13x13xf32, 13x13xf32, 13x13xf32, 13x13xf32])
        stack_0 = paddle._C_ops.stack(combine_1, -1)
        del combine_1

        # builtin.combine: ([13x13xf32, 13x13xf32]) <- (13x13xf32, 13x13xf32)
        combine_2 = [split_1, split_0]
        del split_0, split_1

        # pd_op.stack: (13x13x2xf32) <- ([13x13xf32, 13x13xf32])
        ...
        # pd_op.reshape: (169x4xf32) <- (13x13x4xf32, 2xi64)
        ...
        # pd_op.reshape: (169x2xf32) <- (13x13x2xf32, 2xi64)
        reshape_1 = paddle._C_ops.reshape(stack_1, full_int_array_1)
        del stack_1

        # pd_op.full: (169x1xf32) <- ()
        ...
        full_6 = paddle._C_ops.full(
            [1], float("26"), paddle.float64, paddle.core.CPUPlace()
        )

        # pd_op.arange: (26xi64) <- (1xf64, 1xf64, 1xf64)
        arange_1 = paddle.arange(full_0, full_6, full_2, dtype="int64")
        del full_6

        # pd_op.cast: (26xf32) <- (26xi64)
        cast_1 = paddle._C_ops.cast(arange_1, paddle.float32)
        del arange_1

        # pd_op.scale: (26xf32) <- (26xf32, 1xf32)
        ...
        # builtin.combine: ([26xf32, 26xf32]) <- (26xf32, 26xf32)
        combine_3 = [scale_7, scale_7]
        del scale_7

        # pd_op.meshgrid: ([26x26xf32, 26x26xf32]) <- ([26xf32, 26xf32])
        meshgrid_1 = paddle._C_ops.meshgrid(combine_3)
        del combine_3

        # builtin.split: (26x26xf32, 26x26xf32) <- ([26x26xf32, 26x26xf32])
        (
            split_2,
            split_3,
        ) = meshgrid_1
        ...
        # pd_op.scale: (26x26xf32) <- (26x26xf32, 1xf32)
        ...
        # builtin.combine: ([26x26xf32, 26x26xf32, 26x26xf32, 26x26xf32]) <- (26x26xf32, 26x26xf32, 26x26xf32, 26x26xf32)
        combine_4 = [scale_8, scale_9, scale_10, scale_11]
        del scale_10, scale_11, scale_8, scale_9

        # pd_op.stack: (26x26x4xf32) <- ([26x26xf32, 26x26xf32, 26x26xf32, 26x26xf32])
        stack_2 = paddle._C_ops.stack(combine_4, -1)
        del combine_4

        # builtin.combine: ([26x26xf32, 26x26xf32]) <- (26x26xf32, 26x26xf32)
        combine_5 = [split_3, split_2]
        del split_2, split_3

        # pd_op.stack: (26x26x2xf32) <- ([26x26xf32, 26x26xf32])
        stack_3 = paddle._C_ops.stack(combine_5, -1)
        del combine_5

        # pd_op.reshape: (676x4xf32) <- (26x26x4xf32, 2xi64)
        reshape_2 = paddle._C_ops.reshape(stack_2, full_int_array_0)
        del stack_2

        # pd_op.reshape: (676x2xf32) <- (26x26x2xf32, 2xi64)
        reshape_3 = paddle._C_ops.reshape(stack_3, full_int_array_1)
        del stack_3

        # pd_op.full: (676x1xf32) <- ()
        ...
        # builtin.combine: ([169x4xf32, 676x4xf32, 2704x4xf32]) <- (169x4xf32, 676x4xf32, 2704x4xf32)
        combine_9 = [reshape_0, reshape_2, reshape_4]

        # pd_op.concat: (3549x4xf32) <- ([169x4xf32, 676x4xf32, 2704x4xf32], 1xi32)
        concat_2 = paddle._C_ops.concat(combine_9, full_12)
        del combine_9

        # builtin.combine: ([169x2xf32, 676x2xf32, 2704x2xf32]) <- (169x2xf32, 676x2xf32, 2704x2xf32)
        combine_10 = [reshape_1, reshape_3, reshape_5]
        del reshape_1, reshape_3, reshape_5

        # pd_op.concat: (3549x2xf32) <- ([169x2xf32, 676x2xf32, 2704x2xf32], 1xi32)
        concat_3 = paddle._C_ops.concat(combine_10, full_12)
        del combine_10

        # builtin.combine: ([169x1xf32, 676x1xf32, 2704x1xf32]) <- (169x1xf32, 676x1xf32, 2704x1xf32)
        combine_11 = [full_5, full_8, full_11]
        del full_11, full_5, full_8

        # pd_op.concat: (3549x1xf32) <- ([169x1xf32, 676x1xf32, 2704x1xf32], 1xi32)
        ...
        # pd_op.pool2d: (2x768x1x1xf32) <- (2x768x13x13xf32, 2xi64)
        ...
        # pd_op.multiply: (2x768x13x13xf32) <- (2x768x13x13xf32, 2x768x1x1xf32)
        multiply_0 = paddle._C_ops.multiply(data_0, sigmoid_0)

        # pd_op.conv2d: (2x768x13x13xf32) <- (2x768x13x13xf32, 768x768x1x1xf32)
        conv2d_1 = paddle._C_ops.conv2d(
            multiply_0, parameter_51, [1, 1], [0, 0], "EXPLICIT", [1, 1], 1, "NCHW"
        )
        del parameter_51

        # pd_op.batch_norm_: (2x768x13x13xf32, 768xf32, 768xf32, 768xf32, 768xf32, -1xui8) <- (2x768x13x13xf32, 768xf32, 768xf32, 768xf32, 768xf32)
        (
            batch_norm__0,
            batch_norm__1,
            batch_norm__2,
        ...
        # pd_op.swish: (2x768x13x13xf32) <- (2x768x13x13xf32)
        swish_0 = paddle._C_ops.swish(batch_norm__0)

        # pd_op.add: (2x768x13x13xf32) <- (2x768x13x13xf32, 2x768x13x13xf32)
        add_1 = paddle._C_ops.add(swish_0, data_0)

        # pd_op.conv2d: (2x10x13x13xf32) <- (2x768x13x13xf32, 10x768x3x3xf32)
        ...
        # pd_op.add: (2x10x13x13xf32) <- (2x10x13x13xf32, 1x10x1x1xf32)
        ...
        # pd_op.conv2d: (2x40x13x13xf32) <- (2x768x13x13xf32, 40x768x3x3xf32)
        ...
        # pd_op.add: (2x40x13x13xf32) <- (2x40x13x13xf32, 1x40x1x1xf32)
        add_4 = paddle._C_ops.add(conv2d_5, reshape_9)

        # pd_op.sigmoid: (2x10x13x13xf32) <- (2x10x13x13xf32)
        sigmoid_2 = paddle._C_ops.sigmoid(add_2)
        del add_2

        # pd_op.flatten: (2x10x169xf32) <- (2x10x13x13xf32)
        flatten_0 = paddle._C_ops.flatten(sigmoid_2, 2, 3)

        # pd_op.transpose: (2x169x10xf32) <- (2x10x169xf32)
        transpose_0 = paddle._C_ops.transpose(flatten_0, [0, 2, 1])
        del flatten_0

        # pd_op.flatten: (2x40x169xf32) <- (2x40x13x13xf32)
        flatten_1 = paddle._C_ops.flatten(add_4, 2, 3)

        # pd_op.transpose: (2x169x40xf32) <- (2x40x169xf32)
        transpose_1 = paddle._C_ops.transpose(flatten_1, [0, 2, 1])
        del flatten_1

        # pd_op.pool2d: (2x384x1x1xf32) <- (2x384x26x26xf32, 2xi64)
        ...
        # pd_op.multiply: (2x384x26x26xf32) <- (2x384x26x26xf32, 2x384x1x1xf32)
        multiply_2 = paddle._C_ops.multiply(data_1, sigmoid_3)

        # pd_op.conv2d: (2x384x26x26xf32) <- (2x384x26x26xf32, 384x384x1x1xf32)
        conv2d_7 = paddle._C_ops.conv2d(
            multiply_2, parameter_33, [1, 1], [0, 0], "EXPLICIT", [1, 1], 1, "NCHW"
        )
        del parameter_33

        # pd_op.batch_norm_: (2x384x26x26xf32, 384xf32, 384xf32, 384xf32, 384xf32, -1xui8) <- (2x384x26x26xf32, 384xf32, 384xf32, 384xf32, 384xf32)
        (
            batch_norm__12,
            batch_norm__13,
            batch_norm__14,
        ...
        # pd_op.swish: (2x384x26x26xf32) <- (2x384x26x26xf32)
        swish_2 = paddle._C_ops.swish(batch_norm__12)

        # pd_op.add: (2x384x26x26xf32) <- (2x384x26x26xf32, 2x384x26x26xf32)
        add_6 = paddle._C_ops.add(swish_2, data_1)

        # pd_op.conv2d: (2x10x26x26xf32) <- (2x384x26x26xf32, 10x384x3x3xf32)
        ...
        # pd_op.add: (2x10x26x26xf32) <- (2x10x26x26xf32, 1x10x1x1xf32)
        ...
        # pd_op.conv2d: (2x40x26x26xf32) <- (2x384x26x26xf32, 40x384x3x3xf32)
        ...
        # pd_op.add: (2x40x26x26xf32) <- (2x40x26x26xf32, 1x40x1x1xf32)
        add_9 = paddle._C_ops.add(conv2d_11, reshape_13)

        # pd_op.sigmoid: (2x10x26x26xf32) <- (2x10x26x26xf32)
        sigmoid_5 = paddle._C_ops.sigmoid(add_7)
        del add_7

        # pd_op.flatten: (2x10x676xf32) <- (2x10x26x26xf32)
        flatten_2 = paddle._C_ops.flatten(sigmoid_5, 2, 3)

        # pd_op.transpose: (2x676x10xf32) <- (2x10x676xf32)
        transpose_2 = paddle._C_ops.transpose(flatten_2, [0, 2, 1])
        del flatten_2

        # pd_op.flatten: (2x40x676xf32) <- (2x40x26x26xf32)
        flatten_3 = paddle._C_ops.flatten(add_9, 2, 3)

        # pd_op.transpose: (2x676x40xf32) <- (2x40x676xf32)
        transpose_3 = paddle._C_ops.transpose(flatten_3, [0, 2, 1])
        del flatten_3

        # pd_op.pool2d: (2x192x1x1xf32) <- (2x192x52x52xf32, 2xi64)
        ...
        # pd_op.multiply: (2x192x52x52xf32) <- (2x192x52x52xf32, 2x192x1x1xf32)
        multiply_4 = paddle._C_ops.multiply(data_2, sigmoid_6)

        # pd_op.conv2d: (2x192x52x52xf32) <- (2x192x52x52xf32, 192x192x1x1xf32)
        conv2d_13 = paddle._C_ops.conv2d(
            multiply_4, parameter_15, [1, 1], [0, 0], "EXPLICIT", [1, 1], 1, "NCHW"
        )
        del parameter_15

        # pd_op.batch_norm_: (2x192x52x52xf32, 192xf32, 192xf32, 192xf32, 192xf32, -1xui8) <- (2x192x52x52xf32, 192xf32, 192xf32, 192xf32, 192xf32)
        (
            batch_norm__24,
            batch_norm__25,
            batch_norm__26,
        ...
        # pd_op.swish: (2x192x52x52xf32) <- (2x192x52x52xf32)
        swish_4 = paddle._C_ops.swish(batch_norm__24)

        # pd_op.add: (2x192x52x52xf32) <- (2x192x52x52xf32, 2x192x52x52xf32)
        add_11 = paddle._C_ops.add(swish_4, data_2)

        # pd_op.conv2d: (2x10x52x52xf32) <- (2x192x52x52xf32, 10x192x3x3xf32)
        ...
        # pd_op.add: (2x10x52x52xf32) <- (2x10x52x52xf32, 1x10x1x1xf32)
        ...
        # pd_op.conv2d: (2x40x52x52xf32) <- (2x192x52x52xf32, 40x192x3x3xf32)
        ...
        # pd_op.add: (2x40x52x52xf32) <- (2x40x52x52xf32, 1x40x1x1xf32)
        add_14 = paddle._C_ops.add(conv2d_17, reshape_17)

        # pd_op.sigmoid: (2x10x52x52xf32) <- (2x10x52x52xf32)
        sigmoid_8 = paddle._C_ops.sigmoid(add_12)
        del add_12

        # pd_op.flatten: (2x10x2704xf32) <- (2x10x52x52xf32)
        flatten_4 = paddle._C_ops.flatten(sigmoid_8, 2, 3)

        # pd_op.transpose: (2x2704x10xf32) <- (2x10x2704xf32)
        transpose_4 = paddle._C_ops.transpose(flatten_4, [0, 2, 1])
        del flatten_4

        # pd_op.flatten: (2x40x2704xf32) <- (2x40x52x52xf32)
        flatten_5 = paddle._C_ops.flatten(add_14, 2, 3)

        # pd_op.transpose: (2x2704x40xf32) <- (2x40x2704xf32)
        ...
        # builtin.combine: ([2x169x10xf32, 2x676x10xf32, 2x2704x10xf32]) <- (2x169x10xf32, 2x676x10xf32, 2x2704x10xf32)
        combine_12 = [transpose_0, transpose_2, transpose_4]

        # pd_op.concat: (2x3549x10xf32) <- ([2x169x10xf32, 2x676x10xf32, 2x2704x10xf32], 1xi32)
        concat_0 = paddle._C_ops.concat(combine_12, full_13)
        del combine_12

        # builtin.combine: ([2x169x40xf32, 2x676x40xf32, 2x2704x40xf32]) <- (2x169x40xf32, 2x676x40xf32, 2x2704x40xf32)
        combine_13 = [transpose_1, transpose_3, transpose_5]

        # pd_op.concat: (2x3549x40xf32) <- ([2x169x40xf32, 2x676x40xf32, 2x2704x40xf32], 1xi32)
