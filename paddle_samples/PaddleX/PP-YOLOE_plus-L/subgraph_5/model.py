...
class GraphModule(paddle.nn.Layer):
    ...
    def forward(
        ...
        # pd_op.pool2d: (8x768x1x1xf32) <- (8x768x14x14xf32, 2xi64)
        ...
        # pd_op.add: (8x768x14x14xf32) <- (8x768x14x14xf32, 8x768x14x14xf32)
        add_1 = paddle._C_ops.add(swish_0, data_0)

        # pd_op.conv2d: (8x4x14x14xf32) <- (8x768x14x14xf32, 4x768x3x3xf32)
        ...
        # pd_op.add: (8x4x14x14xf32) <- (8x4x14x14xf32, 1x4x1x1xf32)
        ...
        # pd_op.conv2d: (8x68x14x14xf32) <- (8x768x14x14xf32, 68x768x3x3xf32)
        ...
        # pd_op.add: (8x68x14x14xf32) <- (8x68x14x14xf32, 1x68x1x1xf32)
        add_4 = paddle._C_ops.add(conv2d_5, reshape_9)

        # pd_op.sigmoid: (8x4x14x14xf32) <- (8x4x14x14xf32)
        sigmoid_2 = paddle._C_ops.sigmoid(add_2)
        del add_2

        # pd_op.flatten: (8x4x196xf32) <- (8x4x14x14xf32)
        flatten_0 = paddle._C_ops.flatten(sigmoid_2, 2, 3)

        # pd_op.transpose: (8x196x4xf32) <- (8x4x196xf32)
        transpose_0 = paddle._C_ops.transpose(flatten_0, [0, 2, 1])
        del flatten_0

        # pd_op.flatten: (8x68x196xf32) <- (8x68x14x14xf32)
        flatten_1 = paddle._C_ops.flatten(add_4, 2, 3)

        # pd_op.transpose: (8x196x68xf32) <- (8x68x196xf32)
        transpose_1 = paddle._C_ops.transpose(flatten_1, [0, 2, 1])
        del flatten_1

        # pd_op.pool2d: (8x384x1x1xf32) <- (8x384x28x28xf32, 2xi64)
        ...
        # pd_op.add: (8x384x28x28xf32) <- (8x384x28x28xf32, 8x384x28x28xf32)
        add_6 = paddle._C_ops.add(swish_2, data_1)

        # pd_op.conv2d: (8x4x28x28xf32) <- (8x384x28x28xf32, 4x384x3x3xf32)
        ...
        # pd_op.add: (8x4x28x28xf32) <- (8x4x28x28xf32, 1x4x1x1xf32)
        ...
        # pd_op.conv2d: (8x68x28x28xf32) <- (8x384x28x28xf32, 68x384x3x3xf32)
        ...
        # pd_op.add: (8x68x28x28xf32) <- (8x68x28x28xf32, 1x68x1x1xf32)
        add_9 = paddle._C_ops.add(conv2d_11, reshape_13)

        # pd_op.sigmoid: (8x4x28x28xf32) <- (8x4x28x28xf32)
        sigmoid_5 = paddle._C_ops.sigmoid(add_7)
        del add_7

        # pd_op.flatten: (8x4x784xf32) <- (8x4x28x28xf32)
        flatten_2 = paddle._C_ops.flatten(sigmoid_5, 2, 3)

        # pd_op.transpose: (8x784x4xf32) <- (8x4x784xf32)
        transpose_2 = paddle._C_ops.transpose(flatten_2, [0, 2, 1])
        del flatten_2

        # pd_op.flatten: (8x68x784xf32) <- (8x68x28x28xf32)
        flatten_3 = paddle._C_ops.flatten(add_9, 2, 3)

        # pd_op.transpose: (8x784x68xf32) <- (8x68x784xf32)
        transpose_3 = paddle._C_ops.transpose(flatten_3, [0, 2, 1])
        del flatten_3

        # pd_op.pool2d: (8x192x1x1xf32) <- (8x192x56x56xf32, 2xi64)
        ...
        # pd_op.add: (8x192x56x56xf32) <- (8x192x56x56xf32, 8x192x56x56xf32)
        add_11 = paddle._C_ops.add(swish_4, data_2)

        # pd_op.conv2d: (8x4x56x56xf32) <- (8x192x56x56xf32, 4x192x3x3xf32)
        ...
        # pd_op.add: (8x4x56x56xf32) <- (8x4x56x56xf32, 1x4x1x1xf32)
        ...
        # pd_op.conv2d: (8x68x56x56xf32) <- (8x192x56x56xf32, 68x192x3x3xf32)
        ...
        # pd_op.add: (8x68x56x56xf32) <- (8x68x56x56xf32, 1x68x1x1xf32)
        add_14 = paddle._C_ops.add(conv2d_17, reshape_17)

        # pd_op.sigmoid: (8x4x56x56xf32) <- (8x4x56x56xf32)
        sigmoid_8 = paddle._C_ops.sigmoid(add_12)
        del add_12

        # pd_op.flatten: (8x4x3136xf32) <- (8x4x56x56xf32)
        flatten_4 = paddle._C_ops.flatten(sigmoid_8, 2, 3)

        # pd_op.transpose: (8x3136x4xf32) <- (8x4x3136xf32)
        transpose_4 = paddle._C_ops.transpose(flatten_4, [0, 2, 1])
        del flatten_4

        # pd_op.flatten: (8x68x3136xf32) <- (8x68x56x56xf32)
        flatten_5 = paddle._C_ops.flatten(add_14, 2, 3)

        # pd_op.transpose: (8x3136x68xf32) <- (8x68x3136xf32)
        ...
        # builtin.combine: ([8x196x4xf32, 8x784x4xf32, 8x3136x4xf32]) <- (8x196x4xf32, 8x784x4xf32, 8x3136x4xf32)
        combine_12 = [transpose_0, transpose_2, transpose_4]

        # pd_op.concat: (8x4116x4xf32) <- ([8x196x4xf32, 8x784x4xf32, 8x3136x4xf32], 1xi32)
        concat_0 = paddle._C_ops.concat(combine_12, full_13)
        del combine_12

        # builtin.combine: ([8x196x68xf32, 8x784x68xf32, 8x3136x68xf32]) <- (8x196x68xf32, 8x784x68xf32, 8x3136x68xf32)
        combine_13 = [transpose_1, transpose_3, transpose_5]

        # pd_op.concat: (8x4116x68xf32) <- ([8x196x68xf32, 8x784x68xf32, 8x3136x68xf32], 1xi32)
