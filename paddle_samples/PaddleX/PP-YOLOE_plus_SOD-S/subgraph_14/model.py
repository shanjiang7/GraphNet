...
class GraphModule(paddle.nn.Layer):
    ...
    def forward(
        ...
        # pd_op.pool2d: (2x384x1x1xf32) <- (2x384x21x21xf32, 2xi64)
        ...
        # pd_op.multiply: (2x384x21x21xf32) <- (2x384x21x21xf32, 2x384x1x1xf32)
        multiply_0 = paddle._C_ops.multiply(data_0, sigmoid_0)

        # pd_op.conv2d: (2x384x21x21xf32) <- (2x384x21x21xf32, 384x384x1x1xf32)
        conv2d_1 = paddle._C_ops.conv2d(
            multiply_0, parameter_51, [1, 1], [0, 0], "EXPLICIT", [1, 1], 1, "NCHW"
        )
        del parameter_51

        # pd_op.batch_norm_: (2x384x21x21xf32, 384xf32, 384xf32, 384xf32, 384xf32, -1xui8) <- (2x384x21x21xf32, 384xf32, 384xf32, 384xf32, 384xf32)
        (
            batch_norm__0,
            batch_norm__1,
            batch_norm__2,
        ...
        # pd_op.add: (2x384x21x21xf32) <- (2x384x21x21xf32, 2x384x21x21xf32)
        add_1 = paddle._C_ops.add(swish_0, data_0)

        # pd_op.conv2d: (2x10x21x21xf32) <- (2x384x21x21xf32, 10x384x3x3xf32)
        ...
        # pd_op.add: (2x10x21x21xf32) <- (2x10x21x21xf32, 1x10x1x1xf32)
        ...
        # pd_op.conv2d: (2x40x21x21xf32) <- (2x384x21x21xf32, 40x384x3x3xf32)
        ...
        # pd_op.add: (2x40x21x21xf32) <- (2x40x21x21xf32, 1x40x1x1xf32)
        add_4 = paddle._C_ops.add(conv2d_5, reshape_9)

        # pd_op.sigmoid: (2x10x21x21xf32) <- (2x10x21x21xf32)
        sigmoid_2 = paddle._C_ops.sigmoid(add_2)
        del add_2

        # pd_op.flatten: (2x10x441xf32) <- (2x10x21x21xf32)
        flatten_0 = paddle._C_ops.flatten(sigmoid_2, 2, 3)

        # pd_op.transpose: (2x441x10xf32) <- (2x10x441xf32)
        transpose_0 = paddle._C_ops.transpose(flatten_0, [0, 2, 1])
        del flatten_0

        # pd_op.flatten: (2x40x441xf32) <- (2x40x21x21xf32)
        flatten_1 = paddle._C_ops.flatten(add_4, 2, 3)

        # pd_op.transpose: (2x441x40xf32) <- (2x40x441xf32)
        transpose_1 = paddle._C_ops.transpose(flatten_1, [0, 2, 1])
        del flatten_1

        # pd_op.pool2d: (2x192x1x1xf32) <- (2x192x42x42xf32, 2xi64)
        ...
        # pd_op.multiply: (2x192x42x42xf32) <- (2x192x42x42xf32, 2x192x1x1xf32)
        multiply_2 = paddle._C_ops.multiply(data_1, sigmoid_3)

        # pd_op.conv2d: (2x192x42x42xf32) <- (2x192x42x42xf32, 192x192x1x1xf32)
        conv2d_7 = paddle._C_ops.conv2d(
            multiply_2, parameter_33, [1, 1], [0, 0], "EXPLICIT", [1, 1], 1, "NCHW"
        )
        del parameter_33

        # pd_op.batch_norm_: (2x192x42x42xf32, 192xf32, 192xf32, 192xf32, 192xf32, -1xui8) <- (2x192x42x42xf32, 192xf32, 192xf32, 192xf32, 192xf32)
        (
            batch_norm__12,
            batch_norm__13,
            batch_norm__14,
        ...
        # pd_op.swish: (2x192x42x42xf32) <- (2x192x42x42xf32)
        swish_2 = paddle._C_ops.swish(batch_norm__12)

        # pd_op.add: (2x192x42x42xf32) <- (2x192x42x42xf32, 2x192x42x42xf32)
        add_6 = paddle._C_ops.add(swish_2, data_1)

        # pd_op.conv2d: (2x10x42x42xf32) <- (2x192x42x42xf32, 10x192x3x3xf32)
        ...
        # pd_op.add: (2x10x42x42xf32) <- (2x10x42x42xf32, 1x10x1x1xf32)
        ...
        # pd_op.conv2d: (2x40x42x42xf32) <- (2x192x42x42xf32, 40x192x3x3xf32)
        ...
        # pd_op.add: (2x40x42x42xf32) <- (2x40x42x42xf32, 1x40x1x1xf32)
        add_9 = paddle._C_ops.add(conv2d_11, reshape_13)

        # pd_op.sigmoid: (2x10x42x42xf32) <- (2x10x42x42xf32)
        sigmoid_5 = paddle._C_ops.sigmoid(add_7)
        del add_7

        # pd_op.flatten: (2x10x1764xf32) <- (2x10x42x42xf32)
        flatten_2 = paddle._C_ops.flatten(sigmoid_5, 2, 3)

        # pd_op.transpose: (2x1764x10xf32) <- (2x10x1764xf32)
        transpose_2 = paddle._C_ops.transpose(flatten_2, [0, 2, 1])
        del flatten_2

        # pd_op.flatten: (2x40x1764xf32) <- (2x40x42x42xf32)
        flatten_3 = paddle._C_ops.flatten(add_9, 2, 3)

        # pd_op.transpose: (2x1764x40xf32) <- (2x40x1764xf32)
        transpose_3 = paddle._C_ops.transpose(flatten_3, [0, 2, 1])
        del flatten_3

        # pd_op.pool2d: (2x96x1x1xf32) <- (2x96x84x84xf32, 2xi64)
        ...
        # pd_op.multiply: (2x96x84x84xf32) <- (2x96x84x84xf32, 2x96x1x1xf32)
        multiply_4 = paddle._C_ops.multiply(data_2, sigmoid_6)

        # pd_op.conv2d: (2x96x84x84xf32) <- (2x96x84x84xf32, 96x96x1x1xf32)
        conv2d_13 = paddle._C_ops.conv2d(
            multiply_4, parameter_15, [1, 1], [0, 0], "EXPLICIT", [1, 1], 1, "NCHW"
        )
        del parameter_15

        # pd_op.batch_norm_: (2x96x84x84xf32, 96xf32, 96xf32, 96xf32, 96xf32, -1xui8) <- (2x96x84x84xf32, 96xf32, 96xf32, 96xf32, 96xf32)
        (
            batch_norm__24,
            batch_norm__25,
            batch_norm__26,
        ...
        # pd_op.swish: (2x96x84x84xf32) <- (2x96x84x84xf32)
        swish_4 = paddle._C_ops.swish(batch_norm__24)

        # pd_op.add: (2x96x84x84xf32) <- (2x96x84x84xf32, 2x96x84x84xf32)
        add_11 = paddle._C_ops.add(swish_4, data_2)

        # pd_op.conv2d: (2x10x84x84xf32) <- (2x96x84x84xf32, 10x96x3x3xf32)
        ...
        # pd_op.add: (2x10x84x84xf32) <- (2x10x84x84xf32, 1x10x1x1xf32)
        ...
        # pd_op.conv2d: (2x40x84x84xf32) <- (2x96x84x84xf32, 40x96x3x3xf32)
        ...
        # pd_op.add: (2x40x84x84xf32) <- (2x40x84x84xf32, 1x40x1x1xf32)
        add_14 = paddle._C_ops.add(conv2d_17, reshape_17)

        # pd_op.sigmoid: (2x10x84x84xf32) <- (2x10x84x84xf32)
        sigmoid_8 = paddle._C_ops.sigmoid(add_12)
        del add_12

        # pd_op.flatten: (2x10x7056xf32) <- (2x10x84x84xf32)
        flatten_4 = paddle._C_ops.flatten(sigmoid_8, 2, 3)

        # pd_op.transpose: (2x7056x10xf32) <- (2x10x7056xf32)
        transpose_4 = paddle._C_ops.transpose(flatten_4, [0, 2, 1])
        del flatten_4

        # pd_op.flatten: (2x40x7056xf32) <- (2x40x84x84xf32)
        flatten_5 = paddle._C_ops.flatten(add_14, 2, 3)

        # pd_op.transpose: (2x7056x40xf32) <- (2x40x7056xf32)
        ...
        # builtin.combine: ([2x441x10xf32, 2x1764x10xf32, 2x7056x10xf32]) <- (2x441x10xf32, 2x1764x10xf32, 2x7056x10xf32)
        combine_12 = [transpose_0, transpose_2, transpose_4]

        # pd_op.concat: (2x9261x10xf32) <- ([2x441x10xf32, 2x1764x10xf32, 2x7056x10xf32], 1xi32)
        concat_0 = paddle._C_ops.concat(combine_12, full_13)
        del combine_12

        # builtin.combine: ([2x441x40xf32, 2x1764x40xf32, 2x7056x40xf32]) <- (2x441x40xf32, 2x1764x40xf32, 2x7056x40xf32)
        combine_13 = [transpose_1, transpose_3, transpose_5]

        # pd_op.concat: (2x9261x40xf32) <- ([2x441x40xf32, 2x1764x40xf32, 2x7056x40xf32], 1xi32)
