...
class GraphModule(paddle.nn.Layer):
    ...
    def forward(
        ...
        transpose_1 = paddle._C_ops.transpose(divide_2, [0, 2, 1])
        del divide_2
        ...
        unsqueeze_0 = paddle._C_ops.unsqueeze(transpose_1, full_int_array_1)
        del transpose_1
        ...
        # pd_op.assign: (32x1x3xf32) <- (32x1x3xf32)
        assign_0 = parameter_3
        del parameter_3
        ...
        # pd_op.unsqueeze: (32x1x1x3xf32) <- (32x1x3xf32, 1xi64)
        ...
        # pd_op.conv2d: (16x32x1x96xf32) <- (16x1x1x98xf32, 32x1x1x3xf32)
        conv2d_0 = paddle._C_ops.conv2d(
            unsqueeze_2, unsqueeze_1, [1, 1], [0, 0], "EXPLICIT", [1, 1], 1, "NCHW"
        )

        # pd_op.squeeze: (16x32x96xf32) <- (16x32x1x96xf32, 1xi64)
        squeeze_1 = paddle._C_ops.squeeze(conv2d_0, full_int_array_3)

        # pd_op.transpose: (16x96x32xf32) <- (16x32x96xf32)
        transpose_2 = paddle._C_ops.transpose(squeeze_1, [0, 2, 1])
        del squeeze_1

        # pd_op.matmul: (16x96x32xf32) <- (16x96x4xf32, 4x32xf32)
        matmul_0 = paddle._C_ops.matmul(data_1, parameter_2, False, False)
        del data_1, parameter_2

        # pd_op.add: (16x96x32xf32) <- (16x96x32xf32, 16x96x32xf32)
        add_0 = paddle._C_ops.add(transpose_2, matmul_0)
        ...
        del data_2, full_int_array_4, full_int_array_5

        # pd_op.add: (16x96x32xf32) <- (16x96x32xf32, 1x96x32xf32)
        add_1 = paddle._C_ops.add(add_0, slice_0)
        ...
        # pd_op.dropout: (16x96x32xf32, 16x96x32xui8) <- (16x96x32xf32, None, 1xf32)
        ...
        # pd_op.transpose: (16x32x96xf32) <- (16x96x32xf32)
        transpose_3 = paddle._C_ops.transpose(dropout_0, [0, 2, 1])
        del dropout_0

        # pd_op.matmul: (16x32x192xf32) <- (16x32x96xf32, 96x192xf32)
        matmul_1 = paddle._C_ops.matmul(transpose_3, parameter_1, False, False)
        del parameter_1

        # pd_op.add: (16x32x192xf32) <- (16x32x192xf32, 192xf32)
        add_2 = paddle._C_ops.add(matmul_1, parameter_0)
        del parameter_0

        # pd_op.transpose: (16x192x32xf32) <- (16x32x192xf32)
