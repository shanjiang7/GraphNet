...
class GraphModule(paddle.nn.Layer):
    ...
    def forward(self, data_0, data_1):
        ...
        # pd_op.unsqueeze: (1x4789x4xf32) <- (4789x4xf32, 1xi64)
        ...
        # pd_op.slice: (1x4789x2xf32) <- (1x4789x4xf32, 1xi64, 1xi64)
        ...
        # pd_op.maximum: (4x4789x2xf32) <- (4x1x2xf32, 1x4789x2xf32)
        maximum_0 = paddle._C_ops.maximum(slice_0, slice_2)

        # pd_op.minimum: (4x4789x2xf32) <- (4x1x2xf32, 1x4789x2xf32)
        minimum_0 = paddle._C_ops.minimum(slice_1, slice_3)

        # pd_op.subtract: (4x4789x2xf32) <- (4x4789x2xf32, 4x4789x2xf32)
        ...
        # pd_op.clip: (4x4789x2xf32) <- (4x4789x2xf32, 1xf32, 1xf32)
        ...
        # pd_op.prod: (4x4789xf32) <- (4x4789x2xf32, 1xi64)
        ...
        # pd_op.scale: (4x4789xf32) <- (4x4789xf32, 1xf32)
        scale_0 = paddle._C_ops.scale(subtract_3, full_2, float("1e-10"), True)
        del full_2, subtract_3

        # pd_op.divide: (4x4789xf32) <- (4x4789xf32, 4x4789xf32)
        divide_0 = paddle._C_ops.divide(prod_0, scale_0)
        del prod_0, scale_0

        # pd_op.full_int_array: (3xi64) <- ()
        full_int_array_6 = [4, -1, 4789]

        # pd_op.reshape: (4x1x4789xf32) <- (4x4789xf32, 3xi64)
        ...
        # pd_op.slice: (4789xf32) <- (4789x4xf32, 1xi64, 1xi64)
        ...
        # pd_op.add: (4789xf32) <- (4789xf32, 4789xf32)
        add_3 = paddle._C_ops.add(slice_8, slice_9)
        del slice_8, slice_9

        # pd_op.scale: (4789xf32) <- (4789xf32, 1xf32)
        ...
        # builtin.combine: ([4789xf32, 4789xf32]) <- (4789xf32, 4789xf32)
        combine_1 = [scale_3, scale_4]
        del scale_3, scale_4

        # pd_op.stack: (4789x2xf32) <- ([4789xf32, 4789xf32])
        stack_1 = paddle._C_ops.stack(combine_1, -1)
        del combine_1

        # pd_op.unsqueeze: (1x4789x2xf32) <- (4789x2xf32, 1xi64)
        unsqueeze_3 = paddle._C_ops.unsqueeze(stack_1, full_int_array_2)
        del full_int_array_2

        # pd_op.subtract: (4x4789x2xf32) <- (4x1x2xf32, 1x4789x2xf32)
        subtract_4 = paddle._C_ops.subtract(unsqueeze_2, unsqueeze_3)
        del unsqueeze_2, unsqueeze_3

        # pd_op.p_norm: (4x4789xf32) <- (4x4789x2xf32)
