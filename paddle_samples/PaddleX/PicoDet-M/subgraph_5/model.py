...
class GraphModule(paddle.nn.Layer):
    ...
    def forward(self, data_0, data_1, data_2, data_3, data_4):
        ...
        # pd_op.topk: (4x1x9xf32, 4x1x9xi64) <- (4x1x3600xf32, 1xi32)
        ...
        full_2 = paddle._C_ops.full(
            [1], float("3600"), paddle.int32, paddle.core.CPUPlace()
        )

        # pd_op.one_hot: (4x1x9x3600xf32) <- (4x1x9xi64, 1xi32)
        ...
        # pd_op.sum: (4x1x3600xf32) <- (4x1x9x3600xf32, 1xi64)
        sum_0 = paddle._C_ops.sum(one_hot_0, full_int_array_0, None, False)
        del one_hot_0

        # pd_op.multiply: (4x1x3600xf32) <- (4x1x3600xf32, 4x1x1xf32)
        multiply_0 = paddle._C_ops.multiply(sum_0, data_4)
        del sum_0

        # pd_op.topk: (4x1x9xf32, 4x1x9xi64) <- (4x1x900xf32, 1xi32)
        ...
        # pd_op.scale: (4x1x9xi64) <- (4x1x9xi64, 1xf32)
        scale_1 = paddle._C_ops.scale(topk_3, full_1, float("3600"), True)

        # pd_op.full: (1xi32) <- ()
        full_3 = paddle._C_ops.full(
            [1], float("900"), paddle.int32, paddle.core.CPUPlace()
        )

        # pd_op.one_hot: (4x1x9x900xf32) <- (4x1x9xi64, 1xi32)
        one_hot_1 = paddle._C_ops.one_hot(
            topk_3 % paddle.cast(full_3, topk_3.dtype), full_3
        )
        del full_3, topk_3

        # pd_op.sum: (4x1x900xf32) <- (4x1x9x900xf32, 1xi64)
        sum_1 = paddle._C_ops.sum(one_hot_1, full_int_array_0, None, False)
        del one_hot_1

        # pd_op.multiply: (4x1x900xf32) <- (4x1x900xf32, 4x1x1xf32)
        multiply_1 = paddle._C_ops.multiply(sum_1, data_4)
        del sum_1

        # pd_op.topk: (4x1x9xf32, 4x1x9xi64) <- (4x1x225xf32, 1xi32)
        ...
        # pd_op.scale: (4x1x9xi64) <- (4x1x9xi64, 1xf32)
        scale_2 = paddle._C_ops.scale(topk_5, full_1, float("4500"), True)

        # pd_op.full: (1xi32) <- ()
        full_4 = paddle._C_ops.full(
            [1], float("225"), paddle.int32, paddle.core.CPUPlace()
        )

        # pd_op.one_hot: (4x1x9x225xf32) <- (4x1x9xi64, 1xi32)
        one_hot_2 = paddle._C_ops.one_hot(
            topk_5 % paddle.cast(full_4, topk_5.dtype), full_4
        )
        del full_4, topk_5

        # pd_op.sum: (4x1x225xf32) <- (4x1x9x225xf32, 1xi64)
        sum_2 = paddle._C_ops.sum(one_hot_2, full_int_array_0, None, False)
        del one_hot_2

        # pd_op.multiply: (4x1x225xf32) <- (4x1x225xf32, 4x1x1xf32)
        multiply_2 = paddle._C_ops.multiply(sum_2, data_4)
        del sum_2

        # pd_op.topk: (4x1x9xf32, 4x1x9xi64) <- (4x1x64xf32, 1xi32)
        ...
        # pd_op.scale: (4x1x9xi64) <- (4x1x9xi64, 1xf32)
        scale_3 = paddle._C_ops.scale(topk_7, full_1, float("4725"), True)
        del full_1
        ...
        # builtin.combine: ([4x1x3600xf32, 4x1x900xf32, 4x1x225xf32, 4x1x64xf32]) <- (4x1x3600xf32, 4x1x900xf32, 4x1x225xf32, 4x1x64xf32)
        combine_0 = [multiply_0, multiply_1, multiply_2, multiply_3]
        del multiply_0, multiply_1, multiply_2, multiply_3

        # pd_op.concat: (4x1x4789xf32) <- ([4x1x3600xf32, 4x1x900xf32, 4x1x225xf32, 4x1x64xf32], 1xi32)
