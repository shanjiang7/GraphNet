...
class GraphModule(paddle.nn.Layer):
    ...
    def forward(
        ...
        # pd_op.pool2d: (2x768x1x1xf32) <- (2x768x10x10xf32, 2xi64)
        ...
        # pd_op.multiply: (2x768x10x10xf32) <- (2x768x10x10xf32, 2x768x1x1xf32)
        multiply_0 = paddle._C_ops.multiply(data_0, sigmoid_0)

        # pd_op.conv2d: (2x768x10x10xf32) <- (2x768x10x10xf32, 768x768x1x1xf32)
        conv2d_1 = paddle._C_ops.conv2d(
            multiply_0, parameter_51, [1, 1], [0, 0], "EXPLICIT", [1, 1], 1, "NCHW"
        )
        del parameter_51

        # pd_op.batch_norm_: (2x768x10x10xf32, 768xf32, 768xf32, 768xf32, 768xf32, -1xui8) <- (2x768x10x10xf32, 768xf32, 768xf32, 768xf32, 768xf32)
        (
            batch_norm__0,
            batch_norm__1,
            batch_norm__2,
        ...
        # pd_op.swish: (2x768x10x10xf32) <- (2x768x10x10xf32)
        swish_0 = paddle._C_ops.swish(batch_norm__0)

        # pd_op.add: (2x768x10x10xf32) <- (2x768x10x10xf32, 2x768x10x10xf32)
        add_1 = paddle._C_ops.add(swish_0, data_0)

        # pd_op.conv2d: (2x1x10x10xf32) <- (2x768x10x10xf32, 1x768x3x3xf32)
        ...
        # pd_op.add: (2x1x10x10xf32) <- (2x1x10x10xf32, 1x1x1x1xf32)
        ...
        # pd_op.conv2d: (2x68x10x10xf32) <- (2x768x10x10xf32, 68x768x3x3xf32)
        ...
        # pd_op.add: (2x68x10x10xf32) <- (2x68x10x10xf32, 1x68x1x1xf32)
        ...
        # pd_op.flatten: (2x1x100xf32) <- (2x1x10x10xf32)
        flatten_0 = paddle._C_ops.flatten(sigmoid_2, 2, 3)

        # pd_op.transpose: (2x100x1xf32) <- (2x1x100xf32)
        transpose_0 = paddle._C_ops.transpose(flatten_0, [0, 2, 1])
        del flatten_0

        # pd_op.flatten: (2x68x100xf32) <- (2x68x10x10xf32)
        flatten_1 = paddle._C_ops.flatten(add_4, 2, 3)

        # pd_op.transpose: (2x100x68xf32) <- (2x68x100xf32)
        transpose_1 = paddle._C_ops.transpose(flatten_1, [0, 2, 1])
        del flatten_1

        # pd_op.pool2d: (2x384x1x1xf32) <- (2x384x20x20xf32, 2xi64)
        ...
        # pd_op.multiply: (2x384x20x20xf32) <- (2x384x20x20xf32, 2x384x1x1xf32)
        multiply_2 = paddle._C_ops.multiply(data_1, sigmoid_3)

        # pd_op.conv2d: (2x384x20x20xf32) <- (2x384x20x20xf32, 384x384x1x1xf32)
        conv2d_7 = paddle._C_ops.conv2d(
            multiply_2, parameter_33, [1, 1], [0, 0], "EXPLICIT", [1, 1], 1, "NCHW"
        )
        del parameter_33

        # pd_op.batch_norm_: (2x384x20x20xf32, 384xf32, 384xf32, 384xf32, 384xf32, -1xui8) <- (2x384x20x20xf32, 384xf32, 384xf32, 384xf32, 384xf32)
        (
            batch_norm__12,
            batch_norm__13,
            batch_norm__14,
        ...
        # pd_op.swish: (2x384x20x20xf32) <- (2x384x20x20xf32)
        swish_2 = paddle._C_ops.swish(batch_norm__12)

        # pd_op.add: (2x384x20x20xf32) <- (2x384x20x20xf32, 2x384x20x20xf32)
        add_6 = paddle._C_ops.add(swish_2, data_1)

        # pd_op.conv2d: (2x1x20x20xf32) <- (2x384x20x20xf32, 1x384x3x3xf32)
        ...
        # pd_op.add: (2x1x20x20xf32) <- (2x1x20x20xf32, 1x1x1x1xf32)
        ...
        # pd_op.conv2d: (2x68x20x20xf32) <- (2x384x20x20xf32, 68x384x3x3xf32)
        ...
        # pd_op.add: (2x68x20x20xf32) <- (2x68x20x20xf32, 1x68x1x1xf32)
        add_9 = paddle._C_ops.add(conv2d_11, reshape_13)

        # pd_op.sigmoid: (2x1x20x20xf32) <- (2x1x20x20xf32)
        sigmoid_5 = paddle._C_ops.sigmoid(add_7)
        del add_7

        # pd_op.flatten: (2x1x400xf32) <- (2x1x20x20xf32)
        flatten_2 = paddle._C_ops.flatten(sigmoid_5, 2, 3)

        # pd_op.transpose: (2x400x1xf32) <- (2x1x400xf32)
        transpose_2 = paddle._C_ops.transpose(flatten_2, [0, 2, 1])
        del flatten_2

        # pd_op.flatten: (2x68x400xf32) <- (2x68x20x20xf32)
        flatten_3 = paddle._C_ops.flatten(add_9, 2, 3)

        # pd_op.transpose: (2x400x68xf32) <- (2x68x400xf32)
        transpose_3 = paddle._C_ops.transpose(flatten_3, [0, 2, 1])
        del flatten_3

        # pd_op.pool2d: (2x192x1x1xf32) <- (2x192x40x40xf32, 2xi64)
        ...
        # pd_op.multiply: (2x192x40x40xf32) <- (2x192x40x40xf32, 2x192x1x1xf32)
        multiply_4 = paddle._C_ops.multiply(data_2, sigmoid_6)

        # pd_op.conv2d: (2x192x40x40xf32) <- (2x192x40x40xf32, 192x192x1x1xf32)
        conv2d_13 = paddle._C_ops.conv2d(
            multiply_4, parameter_15, [1, 1], [0, 0], "EXPLICIT", [1, 1], 1, "NCHW"
        )
        del parameter_15

        # pd_op.batch_norm_: (2x192x40x40xf32, 192xf32, 192xf32, 192xf32, 192xf32, -1xui8) <- (2x192x40x40xf32, 192xf32, 192xf32, 192xf32, 192xf32)
        (
            batch_norm__24,
            batch_norm__25,
            batch_norm__26,
        ...
        # pd_op.swish: (2x192x40x40xf32) <- (2x192x40x40xf32)
        swish_4 = paddle._C_ops.swish(batch_norm__24)

        # pd_op.add: (2x192x40x40xf32) <- (2x192x40x40xf32, 2x192x40x40xf32)
        add_11 = paddle._C_ops.add(swish_4, data_2)

        # pd_op.conv2d: (2x1x40x40xf32) <- (2x192x40x40xf32, 1x192x3x3xf32)
        ...
        # pd_op.add: (2x1x40x40xf32) <- (2x1x40x40xf32, 1x1x1x1xf32)
        ...
        # pd_op.conv2d: (2x68x40x40xf32) <- (2x192x40x40xf32, 68x192x3x3xf32)
        ...
        # pd_op.add: (2x68x40x40xf32) <- (2x68x40x40xf32, 1x68x1x1xf32)
        add_14 = paddle._C_ops.add(conv2d_17, reshape_17)

        # pd_op.sigmoid: (2x1x40x40xf32) <- (2x1x40x40xf32)
        sigmoid_8 = paddle._C_ops.sigmoid(add_12)
        del add_12

        # pd_op.flatten: (2x1x1600xf32) <- (2x1x40x40xf32)
        flatten_4 = paddle._C_ops.flatten(sigmoid_8, 2, 3)

        # pd_op.transpose: (2x1600x1xf32) <- (2x1x1600xf32)
        transpose_4 = paddle._C_ops.transpose(flatten_4, [0, 2, 1])
        del flatten_4

        # pd_op.flatten: (2x68x1600xf32) <- (2x68x40x40xf32)
        flatten_5 = paddle._C_ops.flatten(add_14, 2, 3)

        # pd_op.transpose: (2x1600x68xf32) <- (2x68x1600xf32)
        ...
        # builtin.combine: ([2x100x1xf32, 2x400x1xf32, 2x1600x1xf32]) <- (2x100x1xf32, 2x400x1xf32, 2x1600x1xf32)
        combine_12 = [transpose_0, transpose_2, transpose_4]

        # pd_op.concat: (2x2100x1xf32) <- ([2x100x1xf32, 2x400x1xf32, 2x1600x1xf32], 1xi32)
        concat_0 = paddle._C_ops.concat(combine_12, full_13)
        del combine_12

        # builtin.combine: ([2x100x68xf32, 2x400x68xf32, 2x1600x68xf32]) <- (2x100x68xf32, 2x400x68xf32, 2x1600x68xf32)
        combine_13 = [transpose_1, transpose_3, transpose_5]

        # pd_op.concat: (2x2100x68xf32) <- ([2x100x68xf32, 2x400x68xf32, 2x1600x68xf32], 1xi32)
