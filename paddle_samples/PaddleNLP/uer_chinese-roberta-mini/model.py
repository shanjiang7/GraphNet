...
class GraphModule(paddle.nn.Layer):
    ...
    def forward(
        ...
        # pd_op.dropout: (1x11x256xf32, 1x11x256xui8) <- (1x11x256xf32, None, 1xf32)
        ...
        # pd_op.matmul: (1x11x256xf32) <- (1x11x256xf32, 256x256xf32)
        matmul_0 = paddle._C_ops.matmul(dropout_0, parameter_65, False, False)
        del parameter_65

        # pd_op.add: (1x11x256xf32) <- (1x11x256xf32, 256xf32)
        ...
        # pd_op.reshape: (1x11x4x64xf32) <- (1x11x256xf32, 4xi64)
        reshape_0 = paddle._C_ops.reshape(add_2, full_int_array_1)

        # pd_op.transpose: (1x4x11x64xf32) <- (1x11x4x64xf32)
        ...
        # pd_op.scale: (1x4x11x64xf32) <- (1x4x11x64xf32, 1xf32)
        scale_1 = paddle._C_ops.scale(transpose_0, full_5, float("0"), True)
        del transpose_0

        # pd_op.matmul: (1x4x11x11xf32) <- (1x4x11x64xf32, 1x4x11x64xf32)
        matmul_3 = paddle._C_ops.matmul(scale_1, transpose_1, False, True)

        # pd_op.add: (1x4x11x11xf32) <- (1x4x11x11xf32, 1x1x1x11xf32)
        add_5 = paddle._C_ops.add(matmul_3, unsqueeze_0)

        # pd_op.softmax: (1x4x11x11xf32) <- (1x4x11x11xf32)
        softmax_0 = paddle._C_ops.softmax(add_5, -1)
        del add_5

        # pd_op.dropout: (1x4x11x11xf32, 1x4x11x11xui8) <- (1x4x11x11xf32, None, 1xf32)
        ...
        # pd_op.matmul: (1x4x11x64xf32) <- (1x4x11x11xf32, 1x4x11x64xf32)
        matmul_4 = paddle._C_ops.matmul(dropout_2, transpose_2, False, False)

        # pd_op.transpose: (1x11x4x64xf32) <- (1x4x11x64xf32)
        ...
        # pd_op.reshape: (1x11x256xf32) <- (1x11x4x64xf32, 3xi64)
        ...
        # pd_op.matmul: (1x11x1024xf32) <- (1x11x256xf32, 256x1024xf32)
        ...
        # pd_op.gelu: (1x11x1024xf32) <- (1x11x1024xf32)
        gelu_0 = paddle._C_ops.gelu(add_8, False)

        # pd_op.matmul: (1x11x256xf32) <- (1x11x1024xf32, 1024x256xf32)
        ...
        # pd_op.slice: (1x256xf32) <- (1x11x256xf32, 1xi64, 1xi64)
