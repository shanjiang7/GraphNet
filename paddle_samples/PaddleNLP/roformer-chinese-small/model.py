...
class GraphModule(paddle.nn.Layer):
    ...
    def forward(
        ...
        # pd_op.dropout: (1x8x384xf32, 1x8x384xui8) <- (1x8x384xf32, None, 1xf32)
        ...
        # pd_op.matmul: (1x8x384xf32) <- (1x8x384xf32, 384x384xf32)
        matmul_0 = paddle._C_ops.matmul(dropout_0, parameter_109, False, False)
        del parameter_109

        # pd_op.add: (1x8x384xf32) <- (1x8x384xf32, 384xf32)
        ...
        # pd_op.reshape: (1x8x6x64xf32) <- (1x8x384xf32, 4xi64)
        reshape_0 = paddle._C_ops.reshape(add_1, full_int_array_1)

        # pd_op.transpose: (1x6x8x64xf32) <- (1x8x6x64xf32)
        ...
        # pd_op.slice: (8x32xf32) <- (512x32xf32, 1xi64, 1xi64)
        ...
        # pd_op.strided_slice: (1x6x8x32xf32) <- (1x6x8x64xf32, 1xi64, 1xi64, 1xi64)
        ...
        # pd_op.add: (1x6x8x32xf32) <- (1x6x8x32xf32, 1x6x8x32xf32)
        add_4 = paddle._C_ops.add(multiply_2, multiply_3)

        # builtin.combine: ([1x6x8x32xf32, 1x6x8x32xf32]) <- (1x6x8x32xf32, 1x6x8x32xf32)
        combine_0 = [subtract_0, add_4]

        # pd_op.stack: (1x6x8x32x2xf32) <- ([1x6x8x32xf32, 1x6x8x32xf32])
        stack_0 = paddle._C_ops.stack(combine_0, -1)
        del combine_0

        # pd_op.flatten: (1x6x8x64xf32) <- (1x6x8x32x2xf32)
        ...
        # pd_op.scale: (1x6x8x64xf32) <- (1x6x8x64xf32, 1xf32)
        scale_1 = paddle._C_ops.scale(flatten_0, full_3, float("0"), True)
        del flatten_0

        # pd_op.matmul: (1x6x8x8xf32) <- (1x6x8x64xf32, 1x6x8x64xf32)
        matmul_3 = paddle._C_ops.matmul(scale_1, flatten_1, False, True)

        # pd_op.add: (1x6x8x8xf32) <- (1x6x8x8xf32, 1x1x1x8xf32)
        add_6 = paddle._C_ops.add(matmul_3, unsqueeze_0)

        # pd_op.softmax: (1x6x8x8xf32) <- (1x6x8x8xf32)
        softmax_0 = paddle._C_ops.softmax(add_6, -1)
        del add_6

        # pd_op.dropout: (1x6x8x8xf32, 1x6x8x8xui8) <- (1x6x8x8xf32, None, 1xf32)
        ...
        # pd_op.matmul: (1x6x8x64xf32) <- (1x6x8x8xf32, 1x6x8x64xf32)
        matmul_4 = paddle._C_ops.matmul(dropout_2, transpose_2, False, False)

        # pd_op.transpose: (1x8x6x64xf32) <- (1x6x8x64xf32)
        ...
        # pd_op.reshape: (1x8x384xf32) <- (1x8x6x64xf32, 3xi64)
        ...
        # pd_op.matmul: (1x8x1536xf32) <- (1x8x384xf32, 384x1536xf32)
        matmul_6 = paddle._C_ops.matmul(layer_norm_3, parameter_101, False, False)
        del parameter_101

        # pd_op.add: (1x8x1536xf32) <- (1x8x1536xf32, 1536xf32)
        add_9 = paddle._C_ops.add(matmul_6, parameter_100)
        del parameter_100

        # pd_op.gelu: (1x8x1536xf32) <- (1x8x1536xf32)
        gelu_0 = paddle._C_ops.gelu(add_9, False)

        # pd_op.matmul: (1x8x384xf32) <- (1x8x1536xf32, 1536x384xf32)
        ...
        # pd_op.slice: (1x384xf32) <- (1x8x384xf32, 1xi64, 1xi64)
