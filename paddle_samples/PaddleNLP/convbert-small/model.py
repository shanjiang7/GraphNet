...
class GraphModule(paddle.nn.Layer):
    ...
    def forward(
        ...
        # pd_op.matmul: (1x21x128xf32) <- (1x21x256xf32, 256x128xf32)
        matmul_1 = paddle._C_ops.matmul(add_2, parameter_277, False, False)
        del parameter_277

        # pd_op.add: (1x21x128xf32) <- (1x21x128xf32, 128xf32)
        ...
        # pd_op.assign: (256x1x9xf32) <- (256x1x9xf32)
        ...
        # pd_op.unsqueeze: (256x1x1x9xf32) <- (256x1x9xf32, 1xi64)
        ...
        # pd_op.unsqueeze: (1x1x21x256xf32) <- (1x21x256xf32, 1xi64)
        unsqueeze_2 = paddle._C_ops.unsqueeze(add_2, full_int_array_2)

        # pd_op.depthwise_conv2d: (1x1x21x256xf32) <- (1x1x21x256xf32, 256x1x1x9xf32)
        depthwise_conv2d_0 = paddle._C_ops.depthwise_conv2d(
            unsqueeze_2, unsqueeze_1, [1, 1], [0, 4], "EXPLICIT", 256, [1, 1], "NHWC"
        )

        # pd_op.squeeze: (1x21x256xf32) <- (1x1x21x256xf32, 1xi64)
        squeeze_0 = paddle._C_ops.squeeze(depthwise_conv2d_0, full_int_array_2)

        # pd_op.assign: (128x256x1xf32) <- (128x256x1xf32)
        assign_107 = parameter_267
        del parameter_267

        # pd_op.unsqueeze: (128x256x1x1xf32) <- (128x256x1xf32, 1xi64)
        ...
        # pd_op.conv2d: (1x1x21x128xf32) <- (1x1x21x256xf32, 128x256x1x1xf32)
        conv2d_0 = paddle._C_ops.conv2d(
            unsqueeze_4, unsqueeze_3, [1, 1], [0, 0], "EXPLICIT", [1, 1], 1, "NHWC"
        )

        # pd_op.squeeze: (1x21x128xf32) <- (1x1x21x128xf32, 1xi64)
        ...
        # pd_op.multiply: (1x21x128xf32) <- (1x21x128xf32, 1x21x128xf32)
        multiply_0 = paddle._C_ops.multiply(add_6, add_3)

        # pd_op.matmul: (1x21x18xf32) <- (1x21x128xf32, 128x18xf32)
        matmul_4 = paddle._C_ops.matmul(multiply_0, parameter_266, False, False)
        del parameter_266

        # pd_op.add: (1x21x18xf32) <- (1x21x18xf32, 18xf32)
        ...
        # pd_op.reshape: (42x9x1xf32) <- (1x21x18xf32, 3xi64)
        reshape_0 = paddle._C_ops.reshape(add_7, full_int_array_3)

        # pd_op.softmax: (42x9x1xf32) <- (42x9x1xf32)
        ...
        # pd_op.unsqueeze: (1x21x1x1x128xf32) <- (1x21x128xf32, 2xi64)
        ...
        # pd_op.pad3d: (1x29x1x1x128xf32) <- (1x21x1x1x128xf32, 6xi64)
        pad3d_0 = paddle._C_ops.pad3d(
            unsqueeze_5, full_int_array_5, "constant", float("0"), "NDHWC"
        )

        # pd_op.squeeze: (1x29x128xf32) <- (1x29x1x1x128xf32, 2xi64)
        ...
        # pd_op.slice: (1x21x128xf32) <- (1x29x128xf32, 1xi64, 1xi64)
        ...
        # builtin.combine: ([1x21x128xf32, 1x21x128xf32, 1x21x128xf32, 1x21x128xf32, 1x21x128xf32, 1x21x128xf32, 1x21x128xf32, 1x21x128xf32, 1x21x128xf32]) <- (1x21x128xf32, 1x21x128xf32, 1x21x128xf32, 1x21x128xf32, 1x21x128xf32, 1x21x128xf32, 1x21x128xf32, 1x21x128xf32, 1x21x128xf32)
        combine_0 = [
            slice_0,
            slice_1,
            slice_2,
        ...
        # pd_op.stack: (1x21x128x9xf32) <- ([1x21x128xf32, 1x21x128xf32, 1x21x128xf32, 1x21x128xf32, 1x21x128xf32, 1x21x128xf32, 1x21x128xf32, 1x21x128xf32, 1x21x128xf32])
        ...
        # pd_op.reshape: (42x64x9xf32) <- (1x21x128x9xf32, 3xi64)
        reshape_1 = paddle._C_ops.reshape(stack_0, full_int_array_24)

        # pd_op.matmul: (42x64x1xf32) <- (42x64x9xf32, 42x9x1xf32)
        matmul_6 = paddle._C_ops.matmul(reshape_1, softmax_0, False, False)

        # pd_op.full_int_array: (4xi64) <- ()
        full_int_array_25 = [1, 21, 2, 64]

        # pd_op.reshape: (1x21x2x64xf32) <- (42x64x1xf32, 4xi64)
        reshape_2 = paddle._C_ops.reshape(matmul_6, full_int_array_25)

        # pd_op.full_int_array: (4xi64) <- ()
        full_int_array_26 = [0, 0, 2, 64]

        # pd_op.reshape: (1x21x2x64xf32) <- (1x21x128xf32, 4xi64)
        reshape_3 = paddle._C_ops.reshape(add_3, full_int_array_26)

        # pd_op.transpose: (1x2x21x64xf32) <- (1x21x2x64xf32)
        ...
        # pd_op.matmul: (1x2x21x21xf32) <- (1x2x21x64xf32, 1x2x21x64xf32)
        ...
        # pd_op.scale: (1x2x21x21xf32) <- (1x2x21x21xf32, 1xf32)
        scale_1 = paddle._C_ops.scale(matmul_7, full_5, float("0"), True)
        del matmul_7

        # pd_op.add: (1x2x21x21xf32) <- (1x2x21x21xf32, 1x1x1x21xf32)
        add_9 = paddle._C_ops.add(scale_1, unsqueeze_0)

        # pd_op.softmax: (1x2x21x21xf32) <- (1x2x21x21xf32)
        softmax_1 = paddle._C_ops.softmax(add_9, -1)
        del add_9

        # pd_op.dropout: (1x2x21x21xf32, 1x2x21x21xui8) <- (1x2x21x21xf32, None, 1xf32)
        ...
        # pd_op.matmul: (1x2x21x64xf32) <- (1x2x21x21xf32, 1x2x21x64xf32)
        matmul_8 = paddle._C_ops.matmul(dropout_2, transpose_2, False, False)

        # pd_op.transpose: (1x21x2x64xf32) <- (1x2x21x64xf32)
        ...
        # builtin.combine: ([1x21x2x64xf32, 1x21x2x64xf32]) <- (1x21x2x64xf32, 1x21x2x64xf32)
        combine_1 = [transpose_3, reshape_2]

        # pd_op.concat: (1x21x4x64xf32) <- ([1x21x2x64xf32, 1x21x2x64xf32], 1xi32)
        concat_0 = paddle._C_ops.concat(combine_1, full_6)
        del combine_1

        # pd_op.full_int_array: (3xi64) <- ()
        full_int_array_27 = [0, 0, 256]
        ...
        matmul_10 = paddle._C_ops.matmul(layer_norm_3, parameter_262, False, False)
        del parameter_262

        # pd_op.add: (1x21x1024xf32) <- (1x21x1024xf32, 1024xf32)
        add_12 = paddle._C_ops.add(matmul_10, parameter_261)
        del parameter_261
        ...
        matmul_11 = paddle._C_ops.matmul(gelu_0, parameter_260, False, False)
        del parameter_260

        # pd_op.add: (1x21x256xf32) <- (1x21x256xf32, 256xf32)
        add_13 = paddle._C_ops.add(matmul_11, parameter_259)
        del parameter_259
        ...
        depthwise_conv2d_1 = paddle._C_ops.depthwise_conv2d(
            unsqueeze_7, unsqueeze_6, [1, 1], [0, 4], "EXPLICIT", 256, [1, 1], "NHWC"
        )
        ...
        # pd_op.reshape: (42x9x1xf32) <- (1x21x18xf32, 3xi64)
        reshape_7 = paddle._C_ops.reshape(add_19, full_int_array_3)
        ...
        # pd_op.matmul: (42x64x1xf32) <- (42x64x9xf32, 42x9x1xf32)
        matmul_17 = paddle._C_ops.matmul(reshape_8, softmax_2, False, False)

        # pd_op.reshape: (1x21x2x64xf32) <- (42x64x1xf32, 4xi64)
        reshape_9 = paddle._C_ops.reshape(matmul_17, full_int_array_25)

        # pd_op.reshape: (1x21x2x64xf32) <- (1x21x128xf32, 4xi64)
        reshape_10 = paddle._C_ops.reshape(add_15, full_int_array_26)
        ...
        # pd_op.reshape: (1x21x2x64xf32) <- (1x21x128xf32, 4xi64)
        reshape_11 = paddle._C_ops.reshape(add_16, full_int_array_26)
        ...
        # pd_op.reshape: (1x21x2x64xf32) <- (1x21x128xf32, 4xi64)
        reshape_12 = paddle._C_ops.reshape(add_17, full_int_array_26)
        ...
        # pd_op.matmul: (1x2x21x21xf32) <- (1x2x21x64xf32, 1x2x21x64xf32)
        matmul_18 = paddle._C_ops.matmul(transpose_4, transpose_5, False, True)
        ...
        # pd_op.matmul: (1x2x21x64xf32) <- (1x2x21x21xf32, 1x2x21x64xf32)
        matmul_19 = paddle._C_ops.matmul(dropout_8, transpose_6, False, False)

        # pd_op.transpose: (1x21x2x64xf32) <- (1x2x21x64xf32)
        transpose_7 = paddle._C_ops.transpose(matmul_19, [0, 2, 1, 3])
        del matmul_19
        ...
        # pd_op.matmul: (1x21x256xf32) <- (1x21x256xf32, 256x256xf32)
        matmul_20 = paddle._C_ops.matmul(reshape_13, parameter_248, False, False)
        del parameter_248
        ...
        # pd_op.matmul: (1x21x1024xf32) <- (1x21x256xf32, 256x1024xf32)
        matmul_21 = paddle._C_ops.matmul(layer_norm_9, parameter_239, False, False)
        del parameter_239

        # pd_op.add: (1x21x1024xf32) <- (1x21x1024xf32, 1024xf32)
        add_24 = paddle._C_ops.add(matmul_21, parameter_238)
        del parameter_238
        ...
        # pd_op.matmul: (1x21x256xf32) <- (1x21x1024xf32, 1024x256xf32)
        matmul_22 = paddle._C_ops.matmul(gelu_1, parameter_237, False, False)
        del parameter_237

        # pd_op.add: (1x21x256xf32) <- (1x21x256xf32, 256xf32)
        add_25 = paddle._C_ops.add(matmul_22, parameter_236)
        del parameter_236
        ...
        depthwise_conv2d_2 = paddle._C_ops.depthwise_conv2d(
            unsqueeze_12, unsqueeze_11, [1, 1], [0, 4], "EXPLICIT", 256, [1, 1], "NHWC"
        )
        ...
        # pd_op.reshape: (42x9x1xf32) <- (1x21x18xf32, 3xi64)
        reshape_14 = paddle._C_ops.reshape(add_31, full_int_array_3)
        ...
        # pd_op.matmul: (42x64x1xf32) <- (42x64x9xf32, 42x9x1xf32)
        matmul_28 = paddle._C_ops.matmul(reshape_15, softmax_4, False, False)

        # pd_op.reshape: (1x21x2x64xf32) <- (42x64x1xf32, 4xi64)
        reshape_16 = paddle._C_ops.reshape(matmul_28, full_int_array_25)

        # pd_op.reshape: (1x21x2x64xf32) <- (1x21x128xf32, 4xi64)
        reshape_17 = paddle._C_ops.reshape(add_27, full_int_array_26)
        ...
        # pd_op.reshape: (1x21x2x64xf32) <- (1x21x128xf32, 4xi64)
        reshape_18 = paddle._C_ops.reshape(add_28, full_int_array_26)
        ...
        # pd_op.reshape: (1x21x2x64xf32) <- (1x21x128xf32, 4xi64)
        reshape_19 = paddle._C_ops.reshape(add_29, full_int_array_26)
        ...
        # pd_op.matmul: (1x2x21x21xf32) <- (1x2x21x64xf32, 1x2x21x64xf32)
        matmul_29 = paddle._C_ops.matmul(transpose_8, transpose_9, False, True)
        ...
        # pd_op.matmul: (1x2x21x64xf32) <- (1x2x21x21xf32, 1x2x21x64xf32)
        matmul_30 = paddle._C_ops.matmul(dropout_14, transpose_10, False, False)

        # pd_op.transpose: (1x21x2x64xf32) <- (1x2x21x64xf32)
        transpose_11 = paddle._C_ops.transpose(matmul_30, [0, 2, 1, 3])
        del matmul_30
        ...
        # pd_op.matmul: (1x21x256xf32) <- (1x21x256xf32, 256x256xf32)
        matmul_31 = paddle._C_ops.matmul(reshape_20, parameter_225, False, False)
        del parameter_225
        ...
        # pd_op.matmul: (1x21x1024xf32) <- (1x21x256xf32, 256x1024xf32)
        matmul_32 = paddle._C_ops.matmul(layer_norm_15, parameter_216, False, False)
        del parameter_216

        # pd_op.add: (1x21x1024xf32) <- (1x21x1024xf32, 1024xf32)
        add_36 = paddle._C_ops.add(matmul_32, parameter_215)
        del parameter_215
        ...
        # pd_op.matmul: (1x21x256xf32) <- (1x21x1024xf32, 1024x256xf32)
        matmul_33 = paddle._C_ops.matmul(gelu_2, parameter_214, False, False)
        del parameter_214

        # pd_op.add: (1x21x256xf32) <- (1x21x256xf32, 256xf32)
        add_37 = paddle._C_ops.add(matmul_33, parameter_213)
        del parameter_213
        ...
        depthwise_conv2d_3 = paddle._C_ops.depthwise_conv2d(
            unsqueeze_17, unsqueeze_16, [1, 1], [0, 4], "EXPLICIT", 256, [1, 1], "NHWC"
        )
        ...
        # pd_op.reshape: (42x9x1xf32) <- (1x21x18xf32, 3xi64)
        reshape_21 = paddle._C_ops.reshape(add_43, full_int_array_3)
        ...
        # pd_op.matmul: (42x64x1xf32) <- (42x64x9xf32, 42x9x1xf32)
        matmul_39 = paddle._C_ops.matmul(reshape_22, softmax_6, False, False)

        # pd_op.reshape: (1x21x2x64xf32) <- (42x64x1xf32, 4xi64)
        reshape_23 = paddle._C_ops.reshape(matmul_39, full_int_array_25)

        # pd_op.reshape: (1x21x2x64xf32) <- (1x21x128xf32, 4xi64)
        reshape_24 = paddle._C_ops.reshape(add_39, full_int_array_26)
        ...
        # pd_op.reshape: (1x21x2x64xf32) <- (1x21x128xf32, 4xi64)
        reshape_25 = paddle._C_ops.reshape(add_40, full_int_array_26)
        ...
        # pd_op.reshape: (1x21x2x64xf32) <- (1x21x128xf32, 4xi64)
        reshape_26 = paddle._C_ops.reshape(add_41, full_int_array_26)
        ...
        # pd_op.matmul: (1x2x21x21xf32) <- (1x2x21x64xf32, 1x2x21x64xf32)
        matmul_40 = paddle._C_ops.matmul(transpose_12, transpose_13, False, True)
        ...
        # pd_op.matmul: (1x2x21x64xf32) <- (1x2x21x21xf32, 1x2x21x64xf32)
        matmul_41 = paddle._C_ops.matmul(dropout_20, transpose_14, False, False)

        # pd_op.transpose: (1x21x2x64xf32) <- (1x2x21x64xf32)
        transpose_15 = paddle._C_ops.transpose(matmul_41, [0, 2, 1, 3])
        del matmul_41
        ...
        # pd_op.matmul: (1x21x256xf32) <- (1x21x256xf32, 256x256xf32)
        matmul_42 = paddle._C_ops.matmul(reshape_27, parameter_202, False, False)
        del parameter_202
        ...
        # pd_op.matmul: (1x21x1024xf32) <- (1x21x256xf32, 256x1024xf32)
        matmul_43 = paddle._C_ops.matmul(layer_norm_21, parameter_193, False, False)
        del parameter_193

        # pd_op.add: (1x21x1024xf32) <- (1x21x1024xf32, 1024xf32)
        add_48 = paddle._C_ops.add(matmul_43, parameter_192)
        del parameter_192
        ...
        # pd_op.matmul: (1x21x256xf32) <- (1x21x1024xf32, 1024x256xf32)
        matmul_44 = paddle._C_ops.matmul(gelu_3, parameter_191, False, False)
        del parameter_191

        # pd_op.add: (1x21x256xf32) <- (1x21x256xf32, 256xf32)
        add_49 = paddle._C_ops.add(matmul_44, parameter_190)
        del parameter_190
        ...
        depthwise_conv2d_4 = paddle._C_ops.depthwise_conv2d(
            unsqueeze_22, unsqueeze_21, [1, 1], [0, 4], "EXPLICIT", 256, [1, 1], "NHWC"
        )
        ...
        # pd_op.reshape: (42x9x1xf32) <- (1x21x18xf32, 3xi64)
        reshape_28 = paddle._C_ops.reshape(add_55, full_int_array_3)
        ...
        # pd_op.matmul: (42x64x1xf32) <- (42x64x9xf32, 42x9x1xf32)
        matmul_50 = paddle._C_ops.matmul(reshape_29, softmax_8, False, False)

        # pd_op.reshape: (1x21x2x64xf32) <- (42x64x1xf32, 4xi64)
        reshape_30 = paddle._C_ops.reshape(matmul_50, full_int_array_25)

        # pd_op.reshape: (1x21x2x64xf32) <- (1x21x128xf32, 4xi64)
        reshape_31 = paddle._C_ops.reshape(add_51, full_int_array_26)
        ...
        # pd_op.reshape: (1x21x2x64xf32) <- (1x21x128xf32, 4xi64)
        reshape_32 = paddle._C_ops.reshape(add_52, full_int_array_26)
        ...
        # pd_op.reshape: (1x21x2x64xf32) <- (1x21x128xf32, 4xi64)
        reshape_33 = paddle._C_ops.reshape(add_53, full_int_array_26)
        ...
        # pd_op.matmul: (1x2x21x21xf32) <- (1x2x21x64xf32, 1x2x21x64xf32)
        matmul_51 = paddle._C_ops.matmul(transpose_16, transpose_17, False, True)
        ...
        # pd_op.matmul: (1x2x21x64xf32) <- (1x2x21x21xf32, 1x2x21x64xf32)
        matmul_52 = paddle._C_ops.matmul(dropout_26, transpose_18, False, False)

        # pd_op.transpose: (1x21x2x64xf32) <- (1x2x21x64xf32)
        transpose_19 = paddle._C_ops.transpose(matmul_52, [0, 2, 1, 3])
        del matmul_52
        ...
        # pd_op.matmul: (1x21x256xf32) <- (1x21x256xf32, 256x256xf32)
        matmul_53 = paddle._C_ops.matmul(reshape_34, parameter_179, False, False)
        del parameter_179
        ...
        # pd_op.matmul: (1x21x1024xf32) <- (1x21x256xf32, 256x1024xf32)
        matmul_54 = paddle._C_ops.matmul(layer_norm_27, parameter_170, False, False)
        del parameter_170

        # pd_op.add: (1x21x1024xf32) <- (1x21x1024xf32, 1024xf32)
        add_60 = paddle._C_ops.add(matmul_54, parameter_169)
        del parameter_169
        ...
        # pd_op.matmul: (1x21x256xf32) <- (1x21x1024xf32, 1024x256xf32)
        matmul_55 = paddle._C_ops.matmul(gelu_4, parameter_168, False, False)
        del parameter_168

        # pd_op.add: (1x21x256xf32) <- (1x21x256xf32, 256xf32)
        add_61 = paddle._C_ops.add(matmul_55, parameter_167)
        del parameter_167
        ...
        depthwise_conv2d_5 = paddle._C_ops.depthwise_conv2d(
            unsqueeze_27, unsqueeze_26, [1, 1], [0, 4], "EXPLICIT", 256, [1, 1], "NHWC"
        )
        ...
        # pd_op.reshape: (42x9x1xf32) <- (1x21x18xf32, 3xi64)
        reshape_35 = paddle._C_ops.reshape(add_67, full_int_array_3)
        ...
        # pd_op.matmul: (42x64x1xf32) <- (42x64x9xf32, 42x9x1xf32)
        matmul_61 = paddle._C_ops.matmul(reshape_36, softmax_10, False, False)

        # pd_op.reshape: (1x21x2x64xf32) <- (42x64x1xf32, 4xi64)
        reshape_37 = paddle._C_ops.reshape(matmul_61, full_int_array_25)

        # pd_op.reshape: (1x21x2x64xf32) <- (1x21x128xf32, 4xi64)
        reshape_38 = paddle._C_ops.reshape(add_63, full_int_array_26)
        ...
        # pd_op.reshape: (1x21x2x64xf32) <- (1x21x128xf32, 4xi64)
        reshape_39 = paddle._C_ops.reshape(add_64, full_int_array_26)
        ...
        # pd_op.reshape: (1x21x2x64xf32) <- (1x21x128xf32, 4xi64)
        reshape_40 = paddle._C_ops.reshape(add_65, full_int_array_26)
        ...
        # pd_op.matmul: (1x2x21x21xf32) <- (1x2x21x64xf32, 1x2x21x64xf32)
        matmul_62 = paddle._C_ops.matmul(transpose_20, transpose_21, False, True)
        ...
        # pd_op.matmul: (1x2x21x64xf32) <- (1x2x21x21xf32, 1x2x21x64xf32)
        matmul_63 = paddle._C_ops.matmul(dropout_32, transpose_22, False, False)

        # pd_op.transpose: (1x21x2x64xf32) <- (1x2x21x64xf32)
        transpose_23 = paddle._C_ops.transpose(matmul_63, [0, 2, 1, 3])
        del matmul_63
        ...
        # pd_op.matmul: (1x21x256xf32) <- (1x21x256xf32, 256x256xf32)
        matmul_64 = paddle._C_ops.matmul(reshape_41, parameter_156, False, False)
        del parameter_156
        ...
        # pd_op.matmul: (1x21x1024xf32) <- (1x21x256xf32, 256x1024xf32)
        matmul_65 = paddle._C_ops.matmul(layer_norm_33, parameter_147, False, False)
        del parameter_147

        # pd_op.add: (1x21x1024xf32) <- (1x21x1024xf32, 1024xf32)
        add_72 = paddle._C_ops.add(matmul_65, parameter_146)
        del parameter_146
        ...
        # pd_op.matmul: (1x21x256xf32) <- (1x21x1024xf32, 1024x256xf32)
        matmul_66 = paddle._C_ops.matmul(gelu_5, parameter_145, False, False)
        del parameter_145

        # pd_op.add: (1x21x256xf32) <- (1x21x256xf32, 256xf32)
        add_73 = paddle._C_ops.add(matmul_66, parameter_144)
        del parameter_144
        ...
        depthwise_conv2d_6 = paddle._C_ops.depthwise_conv2d(
            unsqueeze_32, unsqueeze_31, [1, 1], [0, 4], "EXPLICIT", 256, [1, 1], "NHWC"
        )
        ...
        # pd_op.reshape: (42x9x1xf32) <- (1x21x18xf32, 3xi64)
        reshape_42 = paddle._C_ops.reshape(add_79, full_int_array_3)
        ...
        # pd_op.matmul: (42x64x1xf32) <- (42x64x9xf32, 42x9x1xf32)
        matmul_72 = paddle._C_ops.matmul(reshape_43, softmax_12, False, False)

        # pd_op.reshape: (1x21x2x64xf32) <- (42x64x1xf32, 4xi64)
        reshape_44 = paddle._C_ops.reshape(matmul_72, full_int_array_25)

        # pd_op.reshape: (1x21x2x64xf32) <- (1x21x128xf32, 4xi64)
        reshape_45 = paddle._C_ops.reshape(add_75, full_int_array_26)
        ...
        # pd_op.reshape: (1x21x2x64xf32) <- (1x21x128xf32, 4xi64)
        reshape_46 = paddle._C_ops.reshape(add_76, full_int_array_26)
        ...
        # pd_op.reshape: (1x21x2x64xf32) <- (1x21x128xf32, 4xi64)
        reshape_47 = paddle._C_ops.reshape(add_77, full_int_array_26)
        ...
        # pd_op.matmul: (1x2x21x21xf32) <- (1x2x21x64xf32, 1x2x21x64xf32)
        matmul_73 = paddle._C_ops.matmul(transpose_24, transpose_25, False, True)
        ...
        # pd_op.matmul: (1x2x21x64xf32) <- (1x2x21x21xf32, 1x2x21x64xf32)
        matmul_74 = paddle._C_ops.matmul(dropout_38, transpose_26, False, False)

        # pd_op.transpose: (1x21x2x64xf32) <- (1x2x21x64xf32)
        transpose_27 = paddle._C_ops.transpose(matmul_74, [0, 2, 1, 3])
        del matmul_74
        ...
        # pd_op.matmul: (1x21x256xf32) <- (1x21x256xf32, 256x256xf32)
        matmul_75 = paddle._C_ops.matmul(reshape_48, parameter_133, False, False)
        del parameter_133
        ...
        # pd_op.matmul: (1x21x1024xf32) <- (1x21x256xf32, 256x1024xf32)
        matmul_76 = paddle._C_ops.matmul(layer_norm_39, parameter_124, False, False)
        del parameter_124

        # pd_op.add: (1x21x1024xf32) <- (1x21x1024xf32, 1024xf32)
        add_84 = paddle._C_ops.add(matmul_76, parameter_123)
        del parameter_123
        ...
        # pd_op.matmul: (1x21x256xf32) <- (1x21x1024xf32, 1024x256xf32)
        matmul_77 = paddle._C_ops.matmul(gelu_6, parameter_122, False, False)
        del parameter_122

        # pd_op.add: (1x21x256xf32) <- (1x21x256xf32, 256xf32)
        add_85 = paddle._C_ops.add(matmul_77, parameter_121)
        del parameter_121
        ...
        depthwise_conv2d_7 = paddle._C_ops.depthwise_conv2d(
            unsqueeze_37, unsqueeze_36, [1, 1], [0, 4], "EXPLICIT", 256, [1, 1], "NHWC"
        )
        ...
        # pd_op.reshape: (42x9x1xf32) <- (1x21x18xf32, 3xi64)
        reshape_49 = paddle._C_ops.reshape(add_91, full_int_array_3)
        ...
        # pd_op.matmul: (42x64x1xf32) <- (42x64x9xf32, 42x9x1xf32)
        matmul_83 = paddle._C_ops.matmul(reshape_50, softmax_14, False, False)

        # pd_op.reshape: (1x21x2x64xf32) <- (42x64x1xf32, 4xi64)
        reshape_51 = paddle._C_ops.reshape(matmul_83, full_int_array_25)

        # pd_op.reshape: (1x21x2x64xf32) <- (1x21x128xf32, 4xi64)
        reshape_52 = paddle._C_ops.reshape(add_87, full_int_array_26)
        ...
        # pd_op.reshape: (1x21x2x64xf32) <- (1x21x128xf32, 4xi64)
        reshape_53 = paddle._C_ops.reshape(add_88, full_int_array_26)
        ...
        # pd_op.reshape: (1x21x2x64xf32) <- (1x21x128xf32, 4xi64)
        reshape_54 = paddle._C_ops.reshape(add_89, full_int_array_26)
        ...
        # pd_op.matmul: (1x2x21x21xf32) <- (1x2x21x64xf32, 1x2x21x64xf32)
        matmul_84 = paddle._C_ops.matmul(transpose_28, transpose_29, False, True)
        ...
        # pd_op.matmul: (1x2x21x64xf32) <- (1x2x21x21xf32, 1x2x21x64xf32)
        matmul_85 = paddle._C_ops.matmul(dropout_44, transpose_30, False, False)

        # pd_op.transpose: (1x21x2x64xf32) <- (1x2x21x64xf32)
        transpose_31 = paddle._C_ops.transpose(matmul_85, [0, 2, 1, 3])
        del matmul_85
        ...
        # pd_op.matmul: (1x21x256xf32) <- (1x21x256xf32, 256x256xf32)
        matmul_86 = paddle._C_ops.matmul(reshape_55, parameter_110, False, False)
        del parameter_110
        ...
        # pd_op.matmul: (1x21x1024xf32) <- (1x21x256xf32, 256x1024xf32)
        matmul_87 = paddle._C_ops.matmul(layer_norm_45, parameter_101, False, False)
        del parameter_101

        # pd_op.add: (1x21x1024xf32) <- (1x21x1024xf32, 1024xf32)
        add_96 = paddle._C_ops.add(matmul_87, parameter_100)
        del parameter_100
        ...
        # pd_op.matmul: (1x21x256xf32) <- (1x21x1024xf32, 1024x256xf32)
        matmul_88 = paddle._C_ops.matmul(gelu_7, parameter_99, False, False)
        del parameter_99

        # pd_op.add: (1x21x256xf32) <- (1x21x256xf32, 256xf32)
        add_97 = paddle._C_ops.add(matmul_88, parameter_98)
        del parameter_98
        ...
        depthwise_conv2d_8 = paddle._C_ops.depthwise_conv2d(
            unsqueeze_42, unsqueeze_41, [1, 1], [0, 4], "EXPLICIT", 256, [1, 1], "NHWC"
        )
        ...
        # pd_op.reshape: (42x9x1xf32) <- (1x21x18xf32, 3xi64)
        reshape_56 = paddle._C_ops.reshape(add_103, full_int_array_3)
        ...
        # pd_op.matmul: (42x64x1xf32) <- (42x64x9xf32, 42x9x1xf32)
        matmul_94 = paddle._C_ops.matmul(reshape_57, softmax_16, False, False)

        # pd_op.reshape: (1x21x2x64xf32) <- (42x64x1xf32, 4xi64)
        reshape_58 = paddle._C_ops.reshape(matmul_94, full_int_array_25)

        # pd_op.reshape: (1x21x2x64xf32) <- (1x21x128xf32, 4xi64)
        reshape_59 = paddle._C_ops.reshape(add_99, full_int_array_26)
        ...
        # pd_op.reshape: (1x21x2x64xf32) <- (1x21x128xf32, 4xi64)
        reshape_60 = paddle._C_ops.reshape(add_100, full_int_array_26)
        ...
        # pd_op.reshape: (1x21x2x64xf32) <- (1x21x128xf32, 4xi64)
        reshape_61 = paddle._C_ops.reshape(add_101, full_int_array_26)
        ...
        # pd_op.matmul: (1x2x21x21xf32) <- (1x2x21x64xf32, 1x2x21x64xf32)
        matmul_95 = paddle._C_ops.matmul(transpose_32, transpose_33, False, True)
        ...
        # pd_op.matmul: (1x2x21x64xf32) <- (1x2x21x21xf32, 1x2x21x64xf32)
        matmul_96 = paddle._C_ops.matmul(dropout_50, transpose_34, False, False)

        # pd_op.transpose: (1x21x2x64xf32) <- (1x2x21x64xf32)
        transpose_35 = paddle._C_ops.transpose(matmul_96, [0, 2, 1, 3])
        del matmul_96
        ...
        # pd_op.matmul: (1x21x256xf32) <- (1x21x256xf32, 256x256xf32)
        matmul_97 = paddle._C_ops.matmul(reshape_62, parameter_87, False, False)
        del parameter_87
        ...
        # pd_op.matmul: (1x21x1024xf32) <- (1x21x256xf32, 256x1024xf32)
        matmul_98 = paddle._C_ops.matmul(layer_norm_51, parameter_78, False, False)
        del parameter_78

        # pd_op.add: (1x21x1024xf32) <- (1x21x1024xf32, 1024xf32)
        add_108 = paddle._C_ops.add(matmul_98, parameter_77)
        del parameter_77
        ...
        # pd_op.matmul: (1x21x256xf32) <- (1x21x1024xf32, 1024x256xf32)
        matmul_99 = paddle._C_ops.matmul(gelu_8, parameter_76, False, False)
        del parameter_76

        # pd_op.add: (1x21x256xf32) <- (1x21x256xf32, 256xf32)
        add_109 = paddle._C_ops.add(matmul_99, parameter_75)
        del parameter_75
        ...
        depthwise_conv2d_9 = paddle._C_ops.depthwise_conv2d(
            unsqueeze_47, unsqueeze_46, [1, 1], [0, 4], "EXPLICIT", 256, [1, 1], "NHWC"
        )
        ...
        # pd_op.reshape: (42x9x1xf32) <- (1x21x18xf32, 3xi64)
        reshape_63 = paddle._C_ops.reshape(add_115, full_int_array_3)
        ...
        # pd_op.matmul: (42x64x1xf32) <- (42x64x9xf32, 42x9x1xf32)
        matmul_105 = paddle._C_ops.matmul(reshape_64, softmax_18, False, False)

        # pd_op.reshape: (1x21x2x64xf32) <- (42x64x1xf32, 4xi64)
        reshape_65 = paddle._C_ops.reshape(matmul_105, full_int_array_25)

        # pd_op.reshape: (1x21x2x64xf32) <- (1x21x128xf32, 4xi64)
        reshape_66 = paddle._C_ops.reshape(add_111, full_int_array_26)
        ...
        # pd_op.reshape: (1x21x2x64xf32) <- (1x21x128xf32, 4xi64)
        reshape_67 = paddle._C_ops.reshape(add_112, full_int_array_26)
        ...
        # pd_op.reshape: (1x21x2x64xf32) <- (1x21x128xf32, 4xi64)
        reshape_68 = paddle._C_ops.reshape(add_113, full_int_array_26)
        ...
        # pd_op.matmul: (1x2x21x21xf32) <- (1x2x21x64xf32, 1x2x21x64xf32)
        matmul_106 = paddle._C_ops.matmul(transpose_36, transpose_37, False, True)
        ...
        # pd_op.matmul: (1x2x21x64xf32) <- (1x2x21x21xf32, 1x2x21x64xf32)
        matmul_107 = paddle._C_ops.matmul(dropout_56, transpose_38, False, False)

        # pd_op.transpose: (1x21x2x64xf32) <- (1x2x21x64xf32)
        transpose_39 = paddle._C_ops.transpose(matmul_107, [0, 2, 1, 3])
        del matmul_107
        ...
        # pd_op.matmul: (1x21x256xf32) <- (1x21x256xf32, 256x256xf32)
        matmul_108 = paddle._C_ops.matmul(reshape_69, parameter_64, False, False)
        del parameter_64
        ...
        # pd_op.matmul: (1x21x1024xf32) <- (1x21x256xf32, 256x1024xf32)
        matmul_109 = paddle._C_ops.matmul(layer_norm_57, parameter_55, False, False)
        del parameter_55

        # pd_op.add: (1x21x1024xf32) <- (1x21x1024xf32, 1024xf32)
        add_120 = paddle._C_ops.add(matmul_109, parameter_54)
        del parameter_54
        ...
        # pd_op.matmul: (1x21x256xf32) <- (1x21x1024xf32, 1024x256xf32)
        matmul_110 = paddle._C_ops.matmul(gelu_9, parameter_53, False, False)
        del parameter_53

        # pd_op.add: (1x21x256xf32) <- (1x21x256xf32, 256xf32)
        add_121 = paddle._C_ops.add(matmul_110, parameter_52)
        del parameter_52
        ...
        depthwise_conv2d_10 = paddle._C_ops.depthwise_conv2d(
            unsqueeze_52, unsqueeze_51, [1, 1], [0, 4], "EXPLICIT", 256, [1, 1], "NHWC"
        )
        ...
        # pd_op.reshape: (42x9x1xf32) <- (1x21x18xf32, 3xi64)
        reshape_70 = paddle._C_ops.reshape(add_127, full_int_array_3)
        ...
        # pd_op.matmul: (42x64x1xf32) <- (42x64x9xf32, 42x9x1xf32)
        matmul_116 = paddle._C_ops.matmul(reshape_71, softmax_20, False, False)

        # pd_op.reshape: (1x21x2x64xf32) <- (42x64x1xf32, 4xi64)
        reshape_72 = paddle._C_ops.reshape(matmul_116, full_int_array_25)

        # pd_op.reshape: (1x21x2x64xf32) <- (1x21x128xf32, 4xi64)
        reshape_73 = paddle._C_ops.reshape(add_123, full_int_array_26)
        ...
        # pd_op.reshape: (1x21x2x64xf32) <- (1x21x128xf32, 4xi64)
        reshape_74 = paddle._C_ops.reshape(add_124, full_int_array_26)
        ...
        # pd_op.reshape: (1x21x2x64xf32) <- (1x21x128xf32, 4xi64)
        reshape_75 = paddle._C_ops.reshape(add_125, full_int_array_26)
        ...
        # pd_op.matmul: (1x2x21x21xf32) <- (1x2x21x64xf32, 1x2x21x64xf32)
        matmul_117 = paddle._C_ops.matmul(transpose_40, transpose_41, False, True)
        ...
        # pd_op.matmul: (1x2x21x64xf32) <- (1x2x21x21xf32, 1x2x21x64xf32)
        matmul_118 = paddle._C_ops.matmul(dropout_62, transpose_42, False, False)

        # pd_op.transpose: (1x21x2x64xf32) <- (1x2x21x64xf32)
        transpose_43 = paddle._C_ops.transpose(matmul_118, [0, 2, 1, 3])
        del matmul_118
        ...
        # pd_op.matmul: (1x21x256xf32) <- (1x21x256xf32, 256x256xf32)
        matmul_119 = paddle._C_ops.matmul(reshape_76, parameter_41, False, False)
        del parameter_41
        ...
        # pd_op.matmul: (1x21x1024xf32) <- (1x21x256xf32, 256x1024xf32)
        matmul_120 = paddle._C_ops.matmul(layer_norm_63, parameter_32, False, False)
        del parameter_32

        # pd_op.add: (1x21x1024xf32) <- (1x21x1024xf32, 1024xf32)
        add_132 = paddle._C_ops.add(matmul_120, parameter_31)
        del parameter_31
        ...
        # pd_op.matmul: (1x21x256xf32) <- (1x21x1024xf32, 1024x256xf32)
        matmul_121 = paddle._C_ops.matmul(gelu_10, parameter_30, False, False)
        del parameter_30

        # pd_op.add: (1x21x256xf32) <- (1x21x256xf32, 256xf32)
        add_133 = paddle._C_ops.add(matmul_121, parameter_29)
        del parameter_29
        ...
        depthwise_conv2d_11 = paddle._C_ops.depthwise_conv2d(
            unsqueeze_57, unsqueeze_56, [1, 1], [0, 4], "EXPLICIT", 256, [1, 1], "NHWC"
        )
        ...
        # pd_op.reshape: (42x9x1xf32) <- (1x21x18xf32, 3xi64)
        reshape_77 = paddle._C_ops.reshape(add_139, full_int_array_3)
        ...
        # pd_op.matmul: (42x64x1xf32) <- (42x64x9xf32, 42x9x1xf32)
        matmul_127 = paddle._C_ops.matmul(reshape_78, softmax_22, False, False)

        # pd_op.reshape: (1x21x2x64xf32) <- (42x64x1xf32, 4xi64)
        reshape_79 = paddle._C_ops.reshape(matmul_127, full_int_array_25)
        del full_int_array_25

        # pd_op.reshape: (1x21x2x64xf32) <- (1x21x128xf32, 4xi64)
        reshape_80 = paddle._C_ops.reshape(add_135, full_int_array_26)
        ...
        # pd_op.reshape: (1x21x2x64xf32) <- (1x21x128xf32, 4xi64)
        reshape_81 = paddle._C_ops.reshape(add_136, full_int_array_26)
        ...
        # pd_op.reshape: (1x21x2x64xf32) <- (1x21x128xf32, 4xi64)
        reshape_82 = paddle._C_ops.reshape(add_137, full_int_array_26)
        ...
        # pd_op.matmul: (1x2x21x21xf32) <- (1x2x21x64xf32, 1x2x21x64xf32)
        matmul_128 = paddle._C_ops.matmul(transpose_44, transpose_45, False, True)
        ...
        # pd_op.matmul: (1x2x21x64xf32) <- (1x2x21x21xf32, 1x2x21x64xf32)
        matmul_129 = paddle._C_ops.matmul(dropout_68, transpose_46, False, False)

        # pd_op.transpose: (1x21x2x64xf32) <- (1x2x21x64xf32)
        transpose_47 = paddle._C_ops.transpose(matmul_129, [0, 2, 1, 3])
        del matmul_129
        ...
        # pd_op.matmul: (1x21x256xf32) <- (1x21x256xf32, 256x256xf32)
        matmul_130 = paddle._C_ops.matmul(reshape_83, parameter_18, False, False)
        del parameter_18
        ...
        # pd_op.matmul: (1x21x1024xf32) <- (1x21x256xf32, 256x1024xf32)
        matmul_131 = paddle._C_ops.matmul(layer_norm_69, parameter_9, False, False)
        del parameter_9

        # pd_op.add: (1x21x1024xf32) <- (1x21x1024xf32, 1024xf32)
        add_144 = paddle._C_ops.add(matmul_131, parameter_8)
        del parameter_8
        ...
        # pd_op.matmul: (1x21x256xf32) <- (1x21x1024xf32, 1024x256xf32)
        matmul_132 = paddle._C_ops.matmul(gelu_11, parameter_7, False, False)
        del parameter_7

        # pd_op.add: (1x21x256xf32) <- (1x21x256xf32, 256xf32)
        add_145 = paddle._C_ops.add(matmul_132, parameter_6)
        del parameter_6
        ...
        # pd_op.slice: (1x256xf32) <- (1x21x256xf32, 1xi64, 1xi64)
