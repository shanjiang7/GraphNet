...
def main():
    parser = argparse.ArgumentParser(
        prog="python -m graph_net.config", description="Set user config for graph_net"
    )
    parser.add_argument(
        "--global",
        dest="global_",
    ...
    parser.add_argument("--username", required=True, help="Username to set in config")
    parser.add_argument("--email", required=True, help="Email to set in config")

    args = parser.parse_args()

    # Prepare config data
    config_data = {"username": args.username, "email": args.email}
