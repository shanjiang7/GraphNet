...
class DeclareConfigMixin:
    @abc.abstractmethod
    def declare_config(self):
        raise NotImplementedError()

    def init_config(self, config=None, constraint_base_classes=()):
        if config is None:
            config = {}

        self._check_config_declaration_valid(constraint_base_classes)
        self.config = self._make_config_by_config_declare(config)

    def _recursively_check_mixin_declare_config(
        self, base_class, constraint_base_classes
    ):
        if issubclass(base_class, constraint_base_classes):
        ...
        for sub_class in base_class.__bases__:
            self._recursively_check_mixin_declare_config(
                sub_class, constraint_base_classes
            )
    ...
    def _check_config_declaration_valid(self, constraint_base_classes):
        self._recursively_check_mixin_declare_config(
            type(self), constraint_base_classes
        )
